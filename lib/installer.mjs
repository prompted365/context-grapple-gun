...
function verifyPlugin({ target, mode, manifest }) {
  ...
  const installed = listJson ? Boolean(findNamedRecord(listJson, PLUGIN_NAME)) : list.includes(PLUGIN_NAME);
  if (!installed) {
  ...
  return {
    ...
    expected: {
      ...
      agents: mode === 'full' ? manifest.agents : null,
      hooks: mode === 'full' ? manifest.hooks : null,
    },
  };
}
