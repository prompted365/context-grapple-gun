...
function materializeAgents({ packageRoot, target, mode, dryRun }) {
  ...
  // nothing. Materialize the mode's contract agents into that directory
  // (mode-scoped: non-full modes must not ship agents).
...
function verifyPlugin({ target, mode, manifest }) {
  ...
  // An installed record can still carry load errors (e.g. duplicate hooks
  // file) — surface them instead of letting downstream inventory checks
  // fail with a misattributed message.
  ...
  if (!inventoryHas(details, 'Skills', expectedSkills)) {
    throw new Error('Claude Code plugin details did not show an admitted skill.');
  }
  if (mode === 'full') {
    if (!inventoryHas(details, 'Agents', ['mogul', 'ripple-assessor', 'review-execute'])) {
      throw new Error('Full-mode plugin details did not show an admitted agent.');
    }
    if (!inventoryHas(details, 'Hooks', ['SessionStart', 'session-restore-patch', 'hooks.json'])) {
      throw new Error('Full-mode plugin details did not show an admitted hook.');
    }
  }
  return {
    details,
    expected: {
      ...
      agents: mode === 'full' ? 'plugin-root agents/ (materialized from contract)' : null,
      hooks: mode === 'full' ? 'plugin-root hooks/hooks.json (auto-loaded)' : null,
    },
  };
}
...
function printSummary({ mode, scope, target, zoneRoot, version, zoneActions = [] }) {
...
export function install(opts = {}) {
  ...
  heading('Verifying loaded component inventory...');
  ...
  printSummary({ mode, scope, target, zoneRoot, version, zoneActions: appliedZoneActions });
  return receipt;
}
