...
export function buildPluginManifest({ packageRoot, mode, version }) {
  ...
  // plugin-root locations only: hooks/hooks.json auto-loads (an explicit
  // manifest reference to that same file is a fatal duplicate — the WHOLE
  // plugin fails to load), and agents auto-load from the root agents/
  // directory (the manifest "agents" field validates but loads nothing).
  // The installer materializes the mode's contract agents into agents/;
  // assertComponentSources still requires the source files above.
