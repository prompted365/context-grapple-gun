...
export const RELEASE_STATUS = 'release-status.json';
...
export function payloadPathsForMode(mode) {
...
export function hookEventNamesForMode(packageRoot, contract, mode) {
  if (mode !== 'full') return [];
  const hookPath = join(packageRoot, contract.hooks.full.replace(/^\.\//, ''));
  const hookManifest = readJson(hookPath);
  return Object.entries(hookManifest.hooks || {})
    .filter(([, entries]) => Array.isArray(entries) && entries.length > 0)
    .map(([event]) => event)
    .sort();
}

export function expectedInventoryForMode(packageRoot, mode) {
  const contract = loadComponentContract(packageRoot);
  const skills = skillNamesForMode(contract, mode).sort();
  const agents = agentPathsForMode(contract, mode)
    .map((relative) => basename(relative, '.md'))
    .sort();
  const hooks = hookEventNamesForMode(packageRoot, contract, mode);
  return {
    skills: { expected_count: skills.length, expected_ids: skills },
    agents: { expected_count: agents.length, expected_ids: agents },
    hooks: { expected_count: hooks.length, expected_ids: hooks },
  };
}

export function readReleaseIdentity(packageRoot, version) {
  const manifestPath = join(packageRoot, RELEASE_MANIFEST);
...
export function assertComponentSources(packageRoot, contract, mode) {
  ...
  for (const relative of agentPathsForMode(contract, mode)) {
    const agentPath = join(packageRoot, relative.replace(/^\.\//, ''));
    if (!existsSync(agentPath)) missing.push(`agent:${relative}`);
  }
  if (mode === 'full') {
    const hookPath = join(packageRoot, contract.hooks.full.replace(/^\.\//, ''));
...
export function buildPluginManifest({ packageRoot, mode, version }) {
  ...
  // Claude Code >= 2.1.220 loads hooks and agents from standard plugin-root
  // locations. The npm installer materializes only the admitted agents into
  // agents/ and copies hooks/ only in full mode. Manifest fields for these
  // surfaces are intentionally omitted because they are inert or duplicative.
