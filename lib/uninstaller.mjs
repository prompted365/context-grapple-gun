...
export function authorizeManagedTargetRemoval(target, receipt) {
  if (existsSync(join(target, '.git'))) {
    return { authorized: false, reason: 'target is a git source checkout' };
  }
  if (!receipt || receipt.package !== PLUGIN_NAME || receipt.managed_target !== target) {
    return { authorized: false, reason: 'install receipt does not prove CGG ownership' };
  }
  if (receipt.managed_target_kind === 'source_checkout') {
    return { authorized: false, reason: 'receipt classifies target as a source checkout' };
  }
  if (receipt.removal_authorized !== true) {
    return { authorized: false, reason: 'receipt does not explicitly authorize recursive removal' };
  }
  return { authorized: true, reason: 'receipt-owned package copy' };
}
...
export function uninstall(opts = {}) {
  ...
  if (!keepRuntime && existsSync(target)) {
    ...
    const authorization = authorizeManagedTargetRemoval(target, receipt);

    if (!authorization.authorized) {
      warn(`Preserving ${target}: ${authorization.reason}.`);
    } else if (dryRun) {
