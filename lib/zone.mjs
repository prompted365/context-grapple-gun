...
import { checkCommand, readJson, run, writeJson } from './utils.mjs';

const PROTOCOL_MARKER = '<!-- cgg-session-learning-protocol:v5 -->';
const LEGACY_PROTOCOL_HEADING = '## Session Learning Protocol';
...
function inspectExistingTiczone(path) {
  const zone = readJson(path);
  if (!Array.isArray(zone.bands)) {
    throw new Error(`Existing .ticzone has no valid bands array: ${path}`);
  }
  if (zone.bands.includes('PRESTIGE')) {
    throw new Error(
      `Existing .ticzone activates governance-blocked PRESTIGE: ${path}\n` +
      'The installer will not rewrite a constitutional file. Remove PRESTIGE deliberately, then retry.',
    );
  }
  return zone;
}
...
export function applyConvention({ zoneRoot, packageRoot, dryRun = false }) {
  ...
  if (existing.includes(LEGACY_PROTOCOL_HEADING)) {
    throw new Error(
      `An unversioned Session Learning Protocol already exists in ${claudePath}.\n` +
      'Refusing to append a competing protocol. Reconcile that section with the v5 source, then retry.',
    );
  }
  if (!dryRun) {
...
export function bootstrapGovernanceZone({
  ...
  if (!existsSync(ticzonePath)) {
    ...
    inspectExistingTiczone(ticzonePath);
