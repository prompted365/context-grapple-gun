...
**Refinement ray — a headline scalar is the degenerate unspecified window: size 1 at the interval's end (tic 684 → /review 687 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** The entry binds explicit window syntax for cross-frequency QUERIES; this ray binds the PUBLISHED SCALAR: a top-level field that names a whole interval is an unlabelled end-of-interval sample — the degenerate window of size 1 at the interval's end — and an END-OF-INTERVAL sample and the interval's DURATION-UNDER-BREACH are different measurements that routinely disagree about the interval's health. Lived: the economy heartbeat publishes top-level `g_t` = `swarm_final_aggregate_g_t` (economy-heartbeat.py:683/741), computed by one `aggregate_g_t` call AFTER the 1000-tick loop closes — a terminal instant, and not even the value the mint gate reads (the gate consumes the per-tick moving `g`). Across tics 670–684 the endpoint and the breach-duration disagreed about clearing the tau=0.70 floor in 6/15 tics (Pearson r = −0.367): tic 681 published g_t 0.7287 while 932/1000 internal ticks ran mint-halted; tic 682 published 0.5963 with only 46/1000 breached. Compounding: the sibling required field `breach_flags` is presence-only — tics 681 and 684 emit an IDENTICAL flag list for 93.2% vs 33.5% breach duration — and the Mogul reporting contract (mogul-runner.sh:450) requires the endpoint but NOT the arc, so the duration reaches /review only if volunteered. Rule: a scalar published at envelope top-level must either name its temporal scope in the field itself (`g_t_final` vs `g_t_mean`) or ship its interval companion alongside; a presence-only flag must not be the only breach evidence in a reporting contract that also publishes an endpoint. Scope-honest bounds carried from the born: `mint_total` IS required and DOES carry the duration signal indirectly (42× spread) — the claim "the required schema cannot distinguish these tics" would be false and is not made; and the braid IS a correct arc consumer (braid-tic-684 carries `fell_below_tau_ticks=335`) — the gap is the GOVERNANCE REPORTING contract, not the producer artifact. Fix-site FILED: `bk-economy-reporting-contract-temporal-scope`. Family edges: `#temporal-scope-discipline` honestly rejected as home (it binds WHICH CLOCK a duration measures against, not point-vs-interval semantics of a published scalar); sibling to the birth-686 breach-flag OR-saturation row (matures /review 689) — distinct axis, held apart deliberately.

<!-- promoted from cpr_mogul_economy_heartbeat_7deeef43cb93 (birth_tic 684 → /review 687 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, round 1). Source: mogul:economy_heartbeat (mogul_mandate_id tic-683-20260807T105016; no born file — the queue-row lesson is the authoritative body). REFINES this entry along the PUBLISHED-SCALAR axis (headline scalar = degenerate unspecified window of size 1 at interval end). Evidence reproducible from audit-logs/economy/economy-tic-{670..684}.json + economy-heartbeat.py:683. Queue: status extracted → promoted; maturity discharged (684+3=687 — queue truth over the scheduler's 688 stamp). Fix FILED: bk-economy-reporting-contract-temporal-scope. compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: meta. confidence_tier: tentative (n=15-tic measured window, single lane). review_confidence: 0.80. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 529f7bb44f7e724c. -->
...
**Refinement ray — a merge that absorbs a still-PENDING predecessor never terminalizes it: A MERGE MUST TERMINALIZE WHAT IT CONSUMES (tic 688 → /review 691 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** The four defects above all bind `current.json` — write-races, recomputation, concurrency, scope expansion; this ray binds the HISTORY ledger, on the record that gets absorbed. A lifecycle MERGE that absorbs a still-live (never-dispatched, `pending`) predecessor records the absorption ONLY on the successor — a backward-pointing `merged_from` — leaving the absorbed record's own ledger lane permanently non-terminal. The mechanism is structural, not incidental: `mogul-runner.sh` is the sole transition-row writer and writes rows only for the mandate it DISPATCHES, while `mandate-write.py:merge_or_supersede` returns `merged_from` for the NEW mandate and writes nothing at all to the old one — so the absorbed row is structurally unreachable by any terminalizer that exists. The absorption pointer runs successor→predecessor, so a forward reader of the predecessor's own lane sees a row that never resolves. This is DISTINCT from and WIDER than the running-merge double-execution hazard named by the tic-683 F2 finding, which explicitly cleared the pending path as "free": the pending path is free of EXECUTION harm — exactly why it reads as free — yet it produces the identical LEDGER hole. The harm compounds wherever that same history ledger is ALSO the declared tiebreaker for swallowed-writeback detection ("on disagreement the LEDGER is truth"): the defect inverts the protocol's own authority ordering, because the mirror is CORRECT in every instance while the declared arbiter is the surface carrying the false positives. Cure: emit the terminal transition (e.g. `pending_to_merged`) on the absorbed record AT THE MERGE SITE — the only locus in the system that knows the absorption happened. General form: **a merge must terminalize what it consumes**; when an absorption is recorded only on the absorber, the absorbed record's lane can never close.

EVIDENCE — n=3 deterministic at filing, n=6 and 6-for-6 at adjudication. Filed over the transition-row era (tics 670–688) with three instances, each named in a successor's `merged_from` and each carrying ZERO transition rows: `tic-680-20260805T180325`, `tic-684-20260807T160839`, `tic-684-20260807T171957`. Re-verified LIVE at the tic-691 stepper gate-1 pass, where the condition was found not merely unresolved but REPRODUCED three more times in the three tics since birth — `tic-688-review-close-T035839` (absorbed by `tic-689-20260808T040041`), `tic-689-20260809T051516` (absorbed by `tic-690-20260809T052045`), and `tic-690-20260810T040919` (absorbed by `tic-691-20260810T041114`) — the last a SAME-TIC reproduction generated by the adjudicating tic's own cadence mandate. The isolating contrast held in BOTH directions at n=6: every `merged_from` predecessor that HAD been dispatched before absorption still carries its own transitions (`tic-679-20260805T042854` and `tic-683-20260807T105016` both carry `pending_to_running` + `running_to_consumed_detached`), and every supersede-path predecessor is closed by construction (supersede fires only on an already-terminal record). The defect is therefore exactly and only *merged-while-never-dispatched*. Scope honesty carried from the born: a naive whole-history "last row is non-terminal" predicate yields ~319 IDs, but that is overwhelmingly the pre-tic-670 legacy `status:`-row convention, NOT this defect; scoped to the transition-row era the count is exact.

SIBLING, CITED NOT RE-OPENED — `cpr_lifecycle_merge_defaults_to_enumerate_terminal_else_merge_tic534` (skip-with-home @ /review 537) shares the exact fix-site function `mandate-write.py:merge_or_supersede` on an ORTHOGONAL axis: tic534 governs which BRANCH the reconciler takes (enumerate-terminal-else-merge), this ray governs what the taken MERGE branch OMITS. Its standing condition "PROMOTE on 2nd lifecycle-reconciler instance" is MET by this ray and is cited here; its terminal row is not re-opened. The merge itself is not under critique — merge-a-live / supersede-a-terminal is correct and load-bearing (it is what prevents the tic-530 unconsumed-cycle drop); the gap is that it preserves the CYCLES without closing the LANE. Fix-site FILED: `bk-mandate-merge-terminalize-absorbed` (HIGH, three-arm fixture design) — cited here, not duplicated.

<!-- promoted from cpr_mogul_deep_audit_3a7579a80e8e (birth_tic 688 → /review 691 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, round 1). Source: mogul:deep_audit (mogul_mandate_id tic-688-20260808T013054; origin artifact audit-logs/mogul/cycle-reports/reports/2026-08-07T224932-tic-688.report.json — the queue-row lesson is the authoritative body). REFINES this entry along the HISTORY-LEDGER TERMINALIZATION axis: the four prior defects bind current.json (write-race, recomputation, concurrency, scope expansion); this ray binds the absorbed record's own lane. Queue: status enrichment_needed → promoted; maturity discharged (688+3=691 — queue truth, and the mandate stamp AGREED by construction this tic). Fix FILED: bk-mandate-merge-terminalize-absorbed (HIGH, three-arm fixture design). Sibling cited, not re-opened: cpr_lifecycle_merge_defaults_to_enumerate_terminal_else_merge_tic534 (skip-with-home @ /review 537 — same fix-site function, orthogonal axis; its "PROMOTE on 2nd lifecycle-reconciler instance" standing condition met by this ray). compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: structural_defect_with_named_fix_site. confidence_tier: tentative — carried through the tic-691 stepper UNCHANGED by authority discipline (the stepper passes tier upgrades through, it does not compute them); the ratified verdict consumed the n=6 evidence directly. review_confidence: 0.85. signer ent_review_execute (claude-opus-5), boot receipt 06cde3bda44bc485. -->
...
<!-- REFINEMENT EDGE (tic 562→574, /review 574 strike PROMOTE-as-ray 0.80 held-at-gate; Architect batch-ratified tic 575 "approved both"): **A boot-time standing REMINDER is not an access-time GUARANTEE — "look-first" reliability requires a hook fired at the moment the guarded surface is accessed, not a pointer surfaced once at boot.** The federation surfaced "consult NAVIGATION/router/io-map FIRST" as a boot pointer and relied on `refresh-maps.sh`'s internal self-guards — but nothing fired when the map FILES were actually read, so freshness reliance was discipline, not a surface. Closed tic 562 with `nav-freshness-reminder.py`: a PreToolUse hook on `Read|Grep|Glob` that, ONCE PER TIC on first access to io-map/router/NAVIGATION, compares the maps' built-tic to the current tic and names the refresh command when stale. The generalizable shape: a reminder answers "did you remember?"; a gate answers "you cannot proceed without X." A "consult X before Y" discipline is reliable only when a hook fires at the Y access-point — advisory (never a hard gate; reading is legitimate) + once-per-tic (piggybacks the natural first read) + fail-soft. The root cause of the gap was structural: the PreToolUse matcher set had `Edit|Write|Bash` but NO `Read|Grep|Glob` — access-class tools were entirely un-hooked, so no access-time nudge was even possible. Live-fire evidence: nav-freshness hook fired tic 574 (maps stale-at-570 → refresh ran); tic-575 session task-touch + primary-tool-search advisories fired shape-never-gate/fail-soft per this discipline — 4th ray on the gate-labeling quiver (t552 don't-call-it-a-gate + t573 memory-mining 3-entry convergence). Cross-ref: composes Gate Contracts (Not Vibes) (#gate-contracts-not-vibes) — reminder vs gate is a LABEL-ACCURACY question at the enforcement-commitment axis. Band: COGNITIVE. From cpr_b98d8987aa12e2cd (born tic 562, source audit-logs/governance/borns-tic562-cockpit-and-reminder-gate.md; /review 574 strike PROMOTE-as-ray 0.80, held-at-gate Architect AFK; Architect batch-ratified tic 575 "approved both"); refinement edge only, no standalone KI. signer ent_homeskillet (claude-fable-5). -->
...
**Scanner cwd-relative default-roots + destructive-empty-write clobber (refinement).** A scanner that resolves its scan-roots relative to the CURRENT WORKING DIRECTORY (not the script's own anchor) produces a SILENT EMPTY scan when invoked from the wrong directory — and if it writes its output unconditionally, it CLOBBERS the prior good output to empty BEFORE the zero-result is recognized as anomalous. The destructive write races ahead of the anomaly check: the file is already gone when the "0 scanned" warning prints. Two teeth, both needed: (1) resolve default scan-roots against the script's anchor (Path(__file__) / the walked-up repo root), not CWD — or require an explicit --root and refuse to default; (2) guard the output write — never overwrite a NON-EMPTY output artifact with an EMPTY scan result without an explicit --force, because empty-from-mis-invocation is indistinguishable at the write boundary from empty-because-truly-empty. This is the sibling of Hook Path Resolution (hooks discover zone root by walking up from the edited file path, not from cwd) applied to a SCANNER's roots, plus its destructive-clobber complement (the write-guard the hook KI does not name). Evidence: tic 556 — running `io_map.py build` (no `--root`) from its own dir `audit-logs/governance/io-map/` resolved `DEFAULT_ROOTS` relative to CWD, found none of those subdirs there, scanned 0 files, and OVERWROTE io-map.json (2.5MB, 1135 scripts / 8967 edges) with an empty 718-byte 0-everything stub. Caught immediately (the "0 scripts" print + a stat), restored from git, re-ran from the repo root (correct: 1138 scripts, coverage clean). The 3D iomap canvas reads io-map-3d.json derived from io-map.json, so a silent clobber here is a live data-lane breaker for the AK Control Room /iomap-3d surface.

<a id="path-arg-resolution-law-is-per-arg-class-derived"></a>
**Path-argument resolution law is per-ARG class-derived, not per-tool (refinement).** A tool's path-argument resolution/refusal law derives from EACH ARGUMENT's surface class, never from the tool as a whole — and one tool can lawfully carry args of OPPOSITE classes with OPPOSITE refusal directions. Lived instance (intake-crossing.py v2.1): the `--proposal` arg is zone-resident class (canonical-side lane artifact) → relative paths anchor at the ZONE ROOT and an outside-zone path is a typed refusal; the `--journal` arg is membrane-external class (repo-face evidence, deliberately outside the zone) → a RELATIVE path is the typed refusal, because zone-anchoring it would pull evidence inside canonical and violate the membrane (observe-not-couple). A single per-tool rule — "anchor everything at the zone root" — would have CURED one argument and VIOLATED the membrane on the other: the correct-looking uniform rule is the wrong rule, which is exactly why this is a distinct locus and not derivable from the per-tool zone-root law above (that law anchors, single-class) composed with observe-not-couple (which forbids the pull-inside but says nothing about path-arg resolution) — the composition locus is per-ARG classification with opposite refusal directions inside ONE tool, validated at argument INTAKE before any side effect. The intake-timing corollary is load-bearing: the v1 defect's real cost was fail-open-LATE (crash at the journal step AFTER binding/request/adapter writes landed, leaving a half-landed emission). Recurrence is structural: every lane tool touching both canonical-side artifacts and membrane/repo-face evidence carries mixed-class path args.

<!-- promoted from cpr_path_arg_resolution_law_is_per_arg_class_derived_tic662 (birth_tic 662 → /review 665 PROMOTE-as-ray at the CGG rung, ratified in-tic question set 1/1; inscribed as in-entry refinement ray per the born's own recommended_scopes). Source: audit-logs/governance/borns-tic662-path-arg-law-is-per-arg-class-derived.md (tic-662 emitter/crossing path-resolution repair, canonical b3c44561 — EMITTER_MANIFEST_OUTSIDE_ZONE / INTAKE_PROPOSAL_OUTSIDE_ZONE vs INTAKE_JOURNAL_PATH_RELATIVE, 6/6 proofs incl refusal-leaves-zero-side-effects). Evidence at promotion: THREE live exercises of the zone-root-relative class (relative --manifest emissions, tics 663/664/665); the refusal arms remain TEST-PROVEN only (both-arms fixtures; honestly bounded — no live refusal-arm exercise claimed); stepper absorption probe at 665 grepped the composition locus across cgg-ledger + canonical/CLAUDE.md + constitution-ledger: zero hits (non-derivable at the locus). Band: COGNITIVE. lesson_type: pattern. Confidence: 0.85. signer ent_homeskillet (claude-fable-5), boot receipt e25bb83edf32d731. -->

<!-- promoted from cpr_scanner_cwd_relative_roots_clobber_output_on_wrong_cwd_tic556 (tic 557→572). Source: audit-logs/governance/borns-tic556-scanner-cwd-relative-roots-clobber-output-on-wrong-cwd.md. /review 572, Architect-ratified PROMOTE as refinement ray on Hook Path Resolution (CogPR-127) — not a standalone KI. Band: COGNITIVE. Confidence: 0.80. -->
...
**Fail-closed epistemic holds — scope the STOP to the consumers of the held truth (refinement ray).** The graduated-scope law extends from containment wire-cuts to fail-closed epistemic HOLDS: a hold placed at the HEAD of a boot path must derive its stop-scope from the consumer set that actually READS the held surface. A hold answers a question the mechanical lanes never asked — born extraction, enrichment scanning, and mandate-fabric emission consume NONE of the held truth — so an unscoped stop converts one suppressed projection into a total governance blackout that persists as long as the hold does. **Hold ≠ blackout**: suppress the projection-dependent consumers (worldview render, queue-truth banners), let the truth-independent mechanical lanes run. The exit-condition hazard is sharper here than for a containment wire-cut: because corrections are append-only, `effective_record.hydration_view` returns `corrected` (rc=3) PERMANENTLY once any resolved correction differs from base, so an unscoped hold on that status is a blackout with no exit condition at all — every future correction re-arms it. Composes `reason-coded-genuine-vs-known-verifier-split`: a gate that cannot distinguish known legacy noise from genuine unresolved corrections holds forever (the binding driver was ~714 legacy parse-issue rows classified *informational* at the scan lens but *blocking* at the gate lens — a scan-lens/gate-lens disagreement invisible until a boot goes dark, because the gate's refusal consumes the very banner that would report it).

<!-- promoted from cpr_fail_closed_boot_hold_must_scope_stop_to_held_truth_consumers_tic679 (birth_tic 679 → /review 682 PROMOTE-as-refinement-ray, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic679-fail-closed-boot-hold-blast-radius.md. NON-DERIVABILITY RULING (the row's own open question, adjudicated at 682): SAME CENTROID as the parent wire-cut scoping law (scope the stop to what consumes it), NEW MECHANISM CLASS (fail-closed epistemic hold vs containment affordance) — therefore a refinement RAY on this entry, not a standalone entry and not a derivable-SKIP. Composes reason-coded-genuine-vs-known-verifier-split. EVIDENCE: three-tic blackout t676–t679 (extraction + fabric + banner all dark; locus session-restore.sh:156 exit 0 on EFFECTIVE_RECORD_HYDRATION_BLOCKED, ahead of the cpr-extract gate, enrichment scanner, and mandate fabric); same-tic control — the UserPromptSubmit fabric dispatch fired while SessionStart stayed dark, proving the stop was UNSCOPED rather than globally warranted. CURE: legs b1/b2 LANDED t680 (CGG 5e22c08 + cgg-runtime/scripts/lib/effective_record.py + cgg-runtime/hooks/session-restore.sh); LIVE-VERIFIED t681 (boot receipt c3b44cb3e165db46 — born extraction fired at boot exactly as the cgg-evaluate block projected) and RE-VERIFIED t682 (second consecutive clean boot). Leg b3 remains open as bk-worldview-projection-aware-b3 (projection-aware worldview consumption of effective-record row-scoped views). Band: COGNITIVE. Confidence_tier: tentative (n=1 lived blackout; the CURE is live-verified twice, the LAW is not). signer ent_review_execute (claude-opus-5[1m]), boot receipt a0006c4eff5b58bb. -->

**Refinement ray — the DATA-SIDE complement: an unsatisfiable fail-closed equality predicate is cured by a TYPED attestation inside an already-trusted envelope, never by weakening the guard (tic 680 born → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** When a fail-closed guard's equality predicate is structurally unsatisfiable for lawful data (a semantic check assuming a field shape the real corpus does not carry), the cure is a TYPED, repository-controlled attestation carried inside an already-trusted envelope — admitted only when it self-consistently matches the digest-protected evidence it interprets — never weakening or deleting the guard, and never contorting the data to satisfy a predicate that mis-models it. The attestation converts the guard from 'equality or refuse' to 'equality, or an accountable resolution the trust chain already covers.' Lived at tic 680 (corrects_resolution in the effective-record legacy bindings; the tic-658 strict path preserved for back-compat); same-tic siblings: the known/genuine noise split (b1) and the consumer-derived stop-scope (b2) — discriminate-or-scope with typed, receipted structure, never soften fail-closed semantics.

<!-- promoted from cpr_unsatisfiable_equality_guard_cured_by_typed_attestation_tic680 (birth_tic 680 → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: audit-logs/governance/borns-tic680-unsatisfiable-equality-guard-attestation-cure.md. JOINS the tic-682 hold≠blackout ray above as its DATA-SIDE complement — the mechanism half of the same law: that ray scopes the STOP to the consumers of the held truth (blast-radius axis); this one names what to do with the DATUM that tripped the fail-closed guard in the first place (attest, don't weaken; type it, don't contort it). Together they cover the two faces a fail-closed guard presents when its predicate mis-models lawful reality. THREE LIVED INSTANCES: (1) t680 seal-state / corrects_resolution attestation; (2) t682 hold≠blackout (consumer-scoped stop); (3) t683 leg b3 — rc=3 PERMANENT render suppression cured renderer-side by projection-awareness, the guard itself left intact (bk-worldview-projection-aware-b3). Confidence_tier: tentative → REINFORCED on the third instance. compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: invariant_refinement. Confidence: 0.85. signer ent_review_execute (claude-opus-5[1m]), boot receipt a4a5b5661049ce2b. -->
...
**Refinement ray — the DETECTOR-SHAPE law: a fixed threshold on the SIZE of a set that only grows is a countdown to a permanent false alarm — wrong by SHAPE, not by value; calibrate on ARRIVAL, split by LANE when the population is bimodal, keep an absolute ceiling only as a state backstop re-derived when the corpus moves, and record EVERY evaluation (tic 744 born → /review 747, `cpr_size_threshold_on_a_monotone_set_is_a_countdown_calibrate_arrival_by_lane_b744`; PROMOTE-as-refinement-ray at the CGG rung — the rung that owns the detector; the federation-general guard ray DEFERRED measured-before-ruled; Architect-ratified in-tic AskUserQuestion round 2 Q5, recommended option verbatim; confidence measured; provenance friction_born — the crisis-steward ruling at 744).** Authoritative Count Discipline says read the count from authoritative state; this ray says what a PREDICATE over that count may look like. Once a size threshold over a monotone surface is reached it is true forever, and cures that move the number only move the date. The crisis-injection Check 2 proved it twice in one detector: the tic-742 cure fixed the READ (the detector had been dead) and left the SHAPE (`active_count > N` over the active manifold); the tic-744 ruling replaced the shape. Re-derived at /review 747 EXACT from the ruling artifact: 411 measured transitions reconstructed from 412 of 484 manifest revisions (72 unparseable, dropped — a proxy that biases counts DOWN, so the thresholds are conservative); per-tic non-campaign arrival {0:377, 1:24, 2:8, 3:1, 4:1}, p95 = 1, all-time max 4 → arm A1 (≥5 new non-campaign ids since the prior tic) fires 0/411; arm A2 (≥12 new ids from any lane) fires 2/411 = exactly the two campaign dumps (+34 at 492→493, +35 at 572→573); arm A3 (active_count > 90 = 58 + 35 → 90) a state backstop, re-derived when the corpus moves > 20 rows; max non-campaign growth over any 7-day window = 7. The lane split is the part a global number can never see: one lane's campaign dumps and background accretion have categorically different statistics, and a composition-blind counter cries wolf on the dominant lane. The shadow lane records EVERY evaluation — 9 would_inject rows over 742–744 on a bit-identical set, then live rows at 744/745/746/747 each 0 arrivals, silent — because a lane that records only on trip is neither the delta predicate's state store nor its negative control (it cannot tell "nothing happened" from "the detector is dead again"). Tests: 25/25 at CGG 6636450 = 16 (the arrival-predicate file) + 9 (the tic-742 key-shadow file touched in the same commit); 35/35 across three files at 747. Relations: refines the per-predicate-liveness ray under ledger.md#breach-flag-at-saturation-is-a-census-rate-rides-at-flag-altitude (the no-trip row as the RECORDED negative control); composes ledger.md#authoritative-set-readers-must-read-the-manifest-not-aggregate-raw-emissions (composition-aware reading → composition-aware predicate) and ledger.md#presence-observation-fallacy-guard (a watcher declares its scope; its predicate's shape must match its surface's dynamics); sibling F-742-C3 (the dead read — the other half of the same detector's two defects); distinct_from the PHASE ray (oscillation, not monotone growth) and from `cpr_mogul_review_close_check_9646fae378af` (a baseline-selection defect, not a threshold-shape defect). DEFERRED FACETS, declared: the federation-general guard ray on the DETECTOR-SHAPE axis waits for a SECOND measured detector — the crisis-injection sibling checks (mandate pileup > 5/tic, inbox backlog > 20; F-744-CS3) share the uncalibrated-constant vintage and are the first rehydration targets (bk-crisis-injection-sibling-checks-calibration); the windowed/cumulative arm (ruling falsifier F2) has a measured basis and no build. APO: NOT a claim the 53 undrained ladder findings should stay resident; NOT a rate model (the process is campaign-driven); NOT a claim the ceiling 90 is durable; NOT applicable to a set pruned or drained on a cadence (there a size threshold can be honest); NOT a claim the git-history reconstruction is a ledger.

Lock line: *A size threshold over a surface that only grows is a date, not a detector — trip on arrival, by lane, with a re-derived ceiling as backstop, and write the row whether or not it trips.*

<!-- promoted from cpr_size_threshold_on_a_monotone_set_is_a_countdown_calibrate_arrival_by_lane_b744 (friction_born, birth_tic 744 → /review 747 PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 2 Q5, recommended option verbatim). Source: audit-logs/governance/borns-tic744-size-threshold-on-a-monotone-set-is-a-countdown.md; audit-logs/sentinel/crisis-threshold-ruling-tic744.json + .md (crisis-steward seat, boot 5b38c4eedfbf1ffa); crisis-injection.py at CGG 6636450 + e597f33; audit-logs/sentinel/crisis-injection-shadow.jsonl (22 rows at 747). Non-derivability adjudicated: #breach-flag-at-saturation governs the emitter's aggregation altitude; the per-predicate-liveness ray governs whether a predicate ran; Authoritative Count Discipline governs the count's source — none names the predicate's SHAPE over a monotone surface, the arrival-by-lane calibration, or the unconditional lane. Placed at the CGG rung, slice-scoped to one detector; federation-general status is a measurement away, not a ruling away. Queue: extracted → promoted. Signer ent_homeskillet (claude-fable-5), boot receipt 2fce3c49e2a2fc4f. -->
...
**Refinement (grammar-level closure) — the WRITE SIGNAL binds at the FIRST TOKEN AFTER the redirect operator; the classifier keys on THAT token position, never on a path mention elsewhere in the command (tic 638 born → struck /review 641, ratified /review 643):** A free-form-command write-signal classifier must parse the operator's ACTUAL grammar — a shell redirect target is exactly the first unbroken word after `>` / `>>` — never approximate its reach with an unbounded negated character class. The class `[^|&;<>]*` admits spaces AND newlines, so one root produced two distinct failures: (a) OVER-BLOCK — a pure read command containing a `->` or `>=` token plus a governed path anywhere later in the same command false-matched as a redirect write and was gated; (b) CLOCK HIJACK — the same false match let DATA embedded in the command (a born path's tic, present only as a read argument) re-key the operative tic, so a valid current-tic receipt was rejected against a stale historical tic. The prior hardening (tic 520) guarded the arg-POSITION axis of the same footgun but left the whitespace-class hole at the SAME regex sites — sweeping a footgun means sweeping every axis of the pattern's reach (position AND class breadth), not only the axis that fired; the sibling-site law applies within one regex, not just across sibling functions. Cure: bind the target token (exclude whitespace from the class) at every site sharing the pattern, and pin regression fixtures for the exact false-positive shapes (`->` + governed read path; `>=` + governed read path; `->` + born read path must not re-key). This is the grammar-level closure of the parent: the parent says gate the WRITE SIGNAL not the path MENTION; this says the write signal's *extent* is a grammar fact — the first token after the operator — not a character class you may widen for convenience.

<!-- promoted from cpr_redirect_write_signal_binds_first_token_after_operator_tic638 (tic 638 → struck /review 641, ratified /review 643 [Architect in-tic question-set ratification — the answers ARE the judgment]). Source: audit-logs/governance/borns-tic638-redirect-write-signal-first-token.md. PROMOTE-as-refinement under mutation-gate-classifier-gates-write-signal-not-path-mention; extends the tic-518 parameter-from-authoritative-state and tic-520 write-target refinements along the CLASS-BREADTH axis (the whitespace hole those two left at the same regex sites). Composes named-footgun-guard-leaves-sibling-site-unfixed (the sibling site is *within* the regex here) + Syntax-Semantic-Collapse. Band: COGNITIVE. Confidence_tier: tentative. Applied by review-execute at tic 643. -->
...
**Discovery-walk membership-fence refinement (tic 688 — membership is an authority axis; inference disclosure does not qualify it).** A discovery walk that ASSEMBLES a governed chain (the ladder-audit `discover_claude_mds` class) is a review-input producer whose perimeter obligation runs on a third axis the cover/declare disjunction must name explicitly: MEMBERSHIP. Declaring HOW parentage was inferred (an honest heuristic disclosure — "inferred from path nesting, not explicit governance linkage") covers the INFERENCE axis and is SILENT on the membership axis, so a reader who honors the disclosure still inherits a contaminated set. A noise skip-list ({node_modules, .git, dist, …}) is NOT a membership fence: it fences computational noise, not authority — the inbound ASSESSMENT-MEMBRANE surfaces (agent-mailboxes, harpoonTargets, harpoonables, inbound/) hold material that is UNDER assessment and never admitted, and a walk that adopts them emits coherence findings whose cure runs the WRONG DIRECTION (tic 685: "parent does not reference child …/harpoonTargets/queue/overshoot-adapter" — the federation root asked to reference a harpoon assessment target; inbound-material adoption dressed as a coherence repair). The fence must be applied BEFORE findings emit, and membership-exclusion must be an ENUMERATED disclosure — which surfaces were fenced out and why — never an implicit consequence of a noise skip-list. Composes the federation admissibility-membrane law (inbound: evidence enters, authority does not) at the discovery layer. **Lock line:** *a noise skip-list is not a membership fence — fence the assessment membrane out of chain discovery before any finding emits, and enumerate the exclusion.*

<!-- REFINED tic 688 (discovery-walk membership fence) from cpr_mogul_ladder_audit_d3df75aab9fa (PROMOTE-as-refinement, /review 688, Architect-ratified via in-tic question set). Refinement edge (NOT net-new parent): adds the MEMBERSHIP axis to the cover/declare disjunction — inference-axis disclosure does not qualify chain membership; assessment-membrane surfaces are fenced out before findings emit. Cross-ray: federation admissibility membrane (observe-not-couple; inbound authority fence). Lived incident tic 685: 2 of 8 audited files / 31 of 316 rules were un-admitted mailbox artifacts (external Overshoot adapter wrapper + third-party harpoon variant); the single emitted finding proposed the federation root reference the assessment target. Runtime fix-site: bk-ladder-audit-membership-fence. Source: queue cpr_mogul_ladder_audit_d3df75aab9fa (extracted->promoted, birth 685, mogul ladder_audit cycle). Band: STRUCTURAL. signer ent_homeskillet-f5 (claude-fable-5). -->
...
**Refinement (tic 692 → /review 695) — per-gate instrumentation symmetry: an extractor carrying MULTIPLE suppression gates must self-report at EVERY gate, not only the one a prior fix instrumented.** Otherwise a SUPPRESSION-zero is indistinguishable from a DETECTION-zero at the reader, and the reader concludes "nothing found" when the truth is "everything found was already known." The parent obligation — emit explicit diagnostic output, not silence — attaches to each suppression site independently, because a reader cannot discriminate a cause it was never told about. Lived instance: `pattern_miner.py` honors this discipline at its LATE gate (`emit_pattern_envelopes()` prints its `skipped_duplicate` count to stderr with an explicit no-re-flood rationale, citing this very entry) but drops detected recurrences silently at its EARLY gate — `mine_patterns()`, `if existing and new_count <= prev_count: continue` (~line 257). Measured live at tic 692: 57 detected / 57 suppressed, reading as zero on both channels (CLI prints `0`; `--json` emits `{"patterns": [], "count": 0, "envelopes_emitted": 0}`) with no diagnostic on either. Per-gate instrumentation symmetry is the tooth. Composes *Named Footgun Guard Leaves Sibling Site Unfixed* (`#named-footgun-guard-leaves-sibling-site-unfixed`) — fix-site and sibling-site are a closed set, here at the instrumentation axis rather than the guard axis — and *A Recurrence Miner Whose Detection Axis Equals the Upstream Write-Gate's Dedup Axis Measures ~Zero BY CONSTRUCTION* (`#recurrence-miner-axis-must-differ-from-upstream-dedup-axis`) — a zero must be labeled by its cause-axis, never read as absence. Note the discrimination from that sibling: a different DETECTION axis would not cure this one, because count-monotonic suppression would still render standing recurrence as a literal 0.

<!-- refined by cpr_mogul_pattern_mining_f32ba06231fc (tic 695) -->
...
**Refinement ray — a severity clause is itself a falsifiable claim: re-test 'benign' against lived recurrence, and name the CLOCK that labels the evidence artifact (tic 684 → /review 687 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** This entry's own severity assessment — double `review_close_check` execution is "benign per the Artifact-Count-≠-1 Fix-Family but noisy" — was falsified by a lived counter-instance at tic 683: the duplication straddled a TIC BOUNDARY, the second executor's artifact took the tic-684 filename, and no tic-683 artifact was ever written. The duplication is benign only when both executions fall inside the same tic. The mechanism the severity clause missed: a cycle's evidence artifact is named by the EXECUTOR'S clock (resolved from current.json at run time), not by the OBLIGATION'S clock (the mandate's own `tic_context.current_tic`). When a merge carries an obligation across a tic boundary, the evidence lands under the absorbing tic and the ORIGINATING tic reads as never-checked — the artifact-count check is structurally blind to it because the count is exactly 1, just filed under the wrong label. The reader-side Presence/Observation guard ("observed-absence-does-not-prove-breakage") already covers the auditor; nothing covers the writer. Cure direction (fix-site FILED `bk-review-close-check-obligation-clock-naming`, not authorized blind): an obligation-clocked artifact name or a same-tic alias. Family edges: sibling axis to the t685 collapse-key ray above — that ray names WHICH OBSERVATIONS one tic's artifact must discriminate; this ray names WHICH TIC an obligation's evidence must be filed under. Generalizable moral: a doctrine entry that assigns a SEVERITY to the collision it predicts owes that severity a re-test against lived recurrence — a collision judged benign at one cadence position can be materially lossy at another.

<!-- promoted from cpr_mogul_review_close_check_93c2d15aba34 (birth_tic 684 → /review 687 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, round 1). Source: mogul:review_close_check (mogul_mandate_id tic-683-20260807T105016; no born file — the queue-row lesson is the authoritative body). REFINES this entry along the SEVERITY + ARTIFACT-CLOCK axis: the entry predicts the collision and under-grades it; the ray carries the lived falsifier (tic-683 obligation absorbed cross-boundary, evidence filed under tic-684, originating tic never-checked at count=1) + the executor-clock-vs-obligation-clock mechanism. Evidence reproducible from the mandate ledger rows (merged_from), the service log's two tic-684 rows, and the absent tic-683-check.json. Queue: status extracted → promoted; maturity discharged (684+3=687 — queue truth over the scheduler's 688 stamp, per #handoff-docket-projection-must-derive-from-maturity-law). Fix FILED: bk-review-close-check-obligation-clock-naming. Adjacent cure NOT conflated: the t686 bk-review-close-check-observation-key superseded-receipt strike (live-conformed at tic 686) cures un-receipted replacement, not cross-boundary mislabeling. compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: refinement_ray. confidence_tier: tentative (n=1 lived instance). review_confidence: 0.80. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 529f7bb44f7e724c. -->
...
**Refinement ray — the collapse key must discriminate OBSERVATIONS, not merely runs; replacing an informative prior is an un-receipted supersede (tic 682 → /review 685 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** The N=1 cardinality target this family prescribes is CORRECT — and its collapse KEY is a separate, independently-falsifiable choice. review-close-check.py keys its canonical artifact at TIC granularity (`tic-<N>-check.json`, write ladder ~:1626–1648), which correctly collapses redundant same-tic re-runs but cannot distinguish them from the /review Step-8.5 PAIR, which is structural: a routine-cycle check runs PRE-/review and a `-review-close` mandate runs POST-/review within one tic — two DIFFERENT observations of two DIFFERENT queue states, where the pre-/review artifact is the baseline the post-/review check exists to be read against. Measured recurrence: 41 of 329 tics carrying a review_close_check mandate had two DISTINCT mandates run it (12.5%), many with the second literally named `<tic>-review-close-<ts>`. The sharp edge: the script ALREADY COMPUTES the discriminator (`prior_norm != report_norm`, branching skip-vs-replace) — genuinely-redundant runs correctly `skip`, while semantically-distinct runs take `replace`; i.e. the ONE case where the prior artifact was informative is precisely the case where it is overwritten, leaving only an ephemeral stderr INFO line. That replace IS a supersede without a receipt (federation `ledger.md#terminal-state-change-requires-receipt-and-no-signal-goes-dark`), reversible only by luck (tic 682: recoverable only because a commit happened to land between the two runs — not a mechanism). Verified mechanically at birth: the committed artifact at 17:30:38Z carried promoted=652/total=1117; the on-disk artifact at 21:04:02Z carried promoted=654/1119 — the pre-/review baseline for the very pass being closed, overwritten by the close check itself. Cure direction (assess purpose first — NOT prescribed as the fix): key N=1 per OBSERVATION (tic + review-phase discriminator), or preserve the prior under a superseded-receipt carrying justification_class before replace; do NOT revert to per-mandate filenames — that re-opens the N≠1 family this design closed. Fix-site FILED: `bk-review-close-check-observation-key`. Family edges: sharpens `#recurrence-miner-axis-must-differ-from-upstream-dedup-axis` (a collapse axis mismatched to the event class) at the verifier-artifact locus; sibling to `#same-tic-replay-diverges-consumers-declare-source` (the same tic-granularity collision surface, consumer side).

<!-- promoted from cpr_mogul_review_close_check_21084d73fe93 (birth_tic 682 → /review 685 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, round 1). Source: mogul:review_close_check (mogul_mandate_id tic-682-20260806T031408; no born file — the queue-row lesson is the authoritative body). REFINES this entry along the COLLAPSE-KEY axis: the family names the N=1 target and the emit-side fix; this ray names that the collapse key must discriminate semantically-distinct OBSERVATIONS from redundant RUNS, and that replacing an informative prior is an un-receipted supersede owing the receipt discipline at a new locus (composes, does not duplicate, the federation terminal-state receipt law). Queue: status extracted → promoted; maturity discharged (birth 682 + 3 = 685). Fix FILED: bk-review-close-check-observation-key. compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: pattern. Confidence: 0.85. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 745e0d9f876992c4. -->

**Consumer-carry note (tic 779) — the INSTRUMENT-IDENTITY face rides this family's emit side.** The federation ray landed at `constitution-ledger/ledger.md#a-supersession-receipt-types-the-instrument-beside-the-reobservation` (/review 779, from a review_close_check mint): a same-tic supersession receipt must type the INSTRUMENT beside the re-observation — when a ratified cure to the writer lands between two same-tic fires, the pair spans two instrument versions, and an untyped boundary lets attribution machinery read an instrument change as data movement (lived t776: writer mtime strictly between the pair's generated_at stamps, differing key sets). Emit-side cure in `review-close-check.py` same-pass: `compute_producer_identity()` stamps every report {writer_path, writer_sha256_16}; the supersession receipt + sidecar carry `producer_identity` + `prior_producer_identity` (honest null for pre-cure priors); the stamp is occurrence-class and rides `_COMPARE_VOLATILE_KEYS`, so the N=1 collapse key and the skip/replace measured unit are unchanged. This extends the t685 collapse-key ray: the observation-key discriminates WHICH observation; the producer identity discriminates WHICH INSTRUMENT observed. Sibling tic-keyed writers (economy, harmony, contagion, visitor-economy) are UNMEASURED for the same under-typing — hypothesis, not measurement.
...
**Refinement — reason-coding must cover EVERY emitted finding type; a multi-path-reachable status is discriminated by in-row path evidence (Verifier-Split Chapter 3).** A verifier whose genuine-vs-known reason-coding filters to a subset of its own finding types leaves the rest floating unclassified — still flipping consistent:false while the human-facing summary claims all-known, so the sentence and the artifact disagree on the finding universe. And a status reachable by multiple lifecycle paths (enrichment_eligible via /review DEFER writeback vs via ordinary pipeline advancement) must be discriminated by in-row path evidence — enrichment-lineage fields present + review-writeback fields absent means awaiting-first-review, legitimately provenance-free — BEFORE the checker demands path-specific provenance. Fix shape: extend reason-coding to all emitted finding types; add a pipeline_advanced_never_reviewed reason code.
<!-- promoted from cpr_mogul_review_close_check_30ce489cecc4 (tic 602 -> 621, /review 621). Live-confirmed at 621: review-close-check.py dispatch (~L1382) routes enrichment_eligible unconditionally to check_deferred. Build leg: backlog bk-review-close-check-reason-coverage-path-discrimination. Confidence_tier: validated. Band: STRUCTURAL. CGG-rung. -->


**Refinement ray — the SAME-TIC RE-OBSERVATION face: a per-pass counter re-observed within one tic silently becomes TIC-CUMULATIVE while still labelled per-pass, and the decomposition must ride IN the delta block that can go cumulative — not merely be recoverable from a sibling block (tic 744 → /review 747, `cpr_mogul_review_close_check_9646fae378af`; PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 1 Q4, recommended option verbatim; confidence TENTATIVE — n=1 lived instance; the mechanism is structural, stated in the artifact's own note).** `review-close-check.py` excludes its own canonical artifact from baseline selection, so a same-tic re-observation re-measures against the SAME prior-tic baseline rather than against the earlier fire; on a tic whose ENTRY fire already moved, the CLOSE artifact's `verdict_counts_delta.delta` is (entry movement + close movement), not the /review pass's writebacks. Re-derived at /review 747 EXACT from the artifacts: baseline tic-737 promoted 754 / index 786; the 738 entry fire (`superseded/tic-738-check.superseded-1.json`) 757 (+3) / 789 (+3); the 738 close fire 759 (+5) / 792 (+6) — a reader reconciling the /review 738 docket row-count against `delta.promoted = 5` over-attributes by 3, and the index counter split differently (+3/+3 on +6), so the decomposition is NOT derivable from one counter's totals. The window 733–747 carries ten same-tic pairs (733, 734, 735, 736, 738, 739, 740, 742, 744, 746); 738 is the ONE whose entry fire moved, so nine pairs mask the shape by coinciding at entry-delta 0. The artifact DOES preserve the prior fire (`superseded_receipt.preserved_path`, justification_class superseded_by_same_tic_reobservation) and the sibling `inscribed_index_delta.note` DOES state the same-tic consequence — but `verdict_counts_delta.note` buckets the residual as "any out-of-band queue state change", framing a KNOWN, typed, same-artifact-lineage prior observation as unattributable drift. Sibling to the t732 DISCLOSURE-PARITY ray (on `#emitter-rows-must-match-a-reader-predicate`): disclosure that lives in the wrong block is disclosure a single-block reader does not receive; distinct from the t743 TWO-COUNTER face (two counters at one fire) — this is ONE counter across TWO fires of one tic; composes ledger.md#structural-transform-implies-closed-consumer-set-obligation (the single-block reader is a silent-degrade consumer of a disclosure that relocated to a sibling block); distinct_from ledger.md#disagreement-as-evidence (the two blocks AGREE on the number and differ only in disclosure depth, so no disagreement trips the reader). Cure (build row bk-close-check-same-tic-reobservation-delta-decomposition): an additive `prior_same_tic_observation` field on BOTH delta blocks naming the preserved artifact and its delta, and the vc note amended — no counter semantics move, no baseline re-selection; its first non-null live read waits for a two-fire tic (747's close fire is a first fire). APO: NOT a claim the delta is wrong (759−754 = 5 is correct against its declared baseline — the defect is attributional); NOT a claim the baseline-exclusion rule should change (excluding the run's own artifact is what makes re-observation re-measurable); NOT a superseded_receipt defect.

Lock line: *A per-pass delta re-observed in the same tic is a tic delta wearing a per-pass label — the block that can go cumulative must carry its own split, or a single-block reader attributes the whole tic to the pass.*

<!-- promoted from cpr_mogul_review_close_check_9646fae378af (birth_tic 744 → /review 747 PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 1 Q4, recommended option verbatim). Source: mogul:review_close_check (2026-08-27T164447-tic-744.report.json; artifacts tic-737-check.json / tic-738-check.json / superseded/tic-738-check.superseded-1.json; no born file — the queue-row lesson is the authoritative body). The row's recommended_scopes named a non-existent path (audit-logs/governance/constitution-ledger/cgg-ledger.md — handed up by the tic-747 stepper); inscribed at the CGG ledger's real path. Non-derivability adjudicated: the t732 and t743 rays govern what the counter's SENTENCE says and which POPULATION two counters compare; this ray governs one counter's delta across two fires of one tic — a third axis. Queue: extracted → promoted. Signer ent_homeskillet (claude-fable-5), boot receipt 2fce3c49e2a2fc4f. -->
...
**(3) Observability-FIELD parity — the symmetry extends to the field-set, not just state (ray, tic 665→668).** A receipted state-transition writer that thins its own row converts diligence into invisibility. Observed tic 665 (Mogul ladder_audit): `ladder-audit.py` emit-finding writes the full observability shape (kind/band/volume/max_volume), but the reaffirm and resolve writers append receipt-shaped rows carrying only `{signal_id, signal_type, status, retest/resolution fields, summary}` into the latest-per-id active manifest — the thin row supersedes the fat one, and the finding goes dark on band, volume, and heat for band-filtered readers. The perverse structure is the point: the ONLY invisible findings are the ones most correctly stewarded (re-tested with a receipt), and the invisible set grows by one on each correct re-test. A second leg compounds at the reader: the derived v2 projection (`manifest-prune.py:128`) defaults a missing `last_reinforced_tic` to the CURRENT tic, so `raw_age_tics` reads 0 for a hold opened 92 tics earlier — the defaulting rule manufactures the exact property (freshness) the escalation reader filters on, while a sibling reader (the campaign driver) computes staleness correctly from its own store (*Disagreement-as-evidence*, live at a reader pair). The law both legs share: **a state-transition writer or a derived projector may not silently narrow the field-set downstream escalation depends on — the transition must carry the observability contract forward, and a projector must render a missing provenance field as UNKNOWN, never as now.** (Repair filed first-class: `bk-downaudit-observability-field-parity` — reaffirm/resolve manifest rows carry kind/band/volume/max_volume forward; manifest-prune renders missing provenance as unknown.)
...
<!-- promoted from cpr_mogul_ladder_audit_2c67d9ae67cb (tic 665→668, /review 668 PROMOTE-as-ray). Source: Mogul ladder_audit cycle tic 665; both legs spot-check CONFIRMED at source at /review 668 (ladder-audit.py:3065 reaffirm + :2956 resolve thin manifest rows vs :571 full emit shape; manifest-prune.py:128 fallback=current_tic). Ray on this entry: observability-FIELD parity + projector-renders-missing-provenance-as-unknown-never-now. Repair filed first-class: bk-downaudit-observability-field-parity. Confidence_tier: reinforced (source-verified both legs at /review). Band: COGNITIVE. CGG-rung (ladder-audit.py + manifest-prune.py are CGG runtime). -->
...
**Refinement — a COUPLE stacks blocker-types; a couple's readiness is the AND of ALL its types, never the OR of the loud ones (tic 536 born → /review 538 PROMOTE-as-refinement, Architect-gated).** The three-way taxonomy above classifies a SINGLE blocker's TYPE; this adds the COMPOSITION axis for a compound target. A governance COUPLE — wiring one governed component's output INTO another's gate — can carry MULTIPLE blocker-types SIMULTANEOUSLY, and resolving the loud ones (a policy-GATE opened + a CAPABILITY cleared) does NOT clear the couple while a quiet BUILD-type blocker is still owed. The recurring failure is NARRATIVE COLLAPSE onto the resolved layers: "admission landed + capability proven → one ring away" reads the two cleared blocker-types as the whole state and silently drops the unbuilt one. Two net-new rays beyond the parent: (1) a couple's readiness is the AND over its full blocker-set, not the OR of whichever types just cleared; (2) before framing a ring/go as a flip, run a read-only PREFLIGHT that enumerates EVERY blocker-type on the couple and names what the go actually authorizes (flag-flip vs build-lane-open). Discriminator vs the parent: the parent routes ONE blocker by type (adjudicate|provision|build); this governs the COMPOSITION of a target carrying several at once. Sibling to the capability-shelf-life refinement above: that names a CAPABILITY-type verdict DECAYS; this names that clearing capability (or admission) is not clearing the COUPLE. Composes Presence/Observation-Fallacy-Guard (admission-present ≠ couple-wired) + Fix-Then-Present (don't frame a build as a flip). (Validated: tic 527→536 — epoch16 was ADMITTED as the S3 proposal-source [tic 527] and CAPABILITY-proven local [tic 535, NOT_REACHABLE overturned], and the running narrative called the live SP2 couple "the real boom, one ring away"; a read-only preflight found the couple BUILD-gated — the real federation governance quiver un-typed [VERDICT-gated], the org-engine-envelope→SP5-bid adapter unbuilt, and the SP2 gate fixture-only [design_only, no kernel registration] — so the Architect's "yes" would authorize BEGINNING that build, not a flag-flip. The tic-537 BUILD #1 real-quiver typing is the first owed build step, made reviewable at /review 538.) Derivable-from-parents PARTIAL (the AND-over-a-stacked-set + the couple-as-composition target + preflight-before-framing-flip-vs-build are net-new). SKIP≠DISCARD honored.

<!-- promoted from cpr_admission_plus_capability_proven_is_not_a_ring_flip_couple_can_be_build_gated_behind_open_policy_gate_tic536 (audit-logs/cprs/queue.jsonl, extracted->promoted; /review 538 PROMOTE-as-refinement, Architect-gated [AskUserQuestion approval]). Composition axis on Blocker-Type Taxonomy: the parent classifies a SINGLE blocker's type; this names that a COUPLE stacks blocker-types and readiness is the AND over all of them (+ preflight-before-framing-a-ring-as-flip that names flag-flip vs build-lane-open). Composes Presence/Observation-Fallacy-Guard + Fix-Then-Present. Source: audit-logs/governance/borns-tic536-admission-plus-capability-is-not-a-ring-flip.md. Band: COGNITIVE. Confidence_tier: reinforced (validated tic 527→536; sibling to the tic-535 shelf-life refinement, same SP2 build lane). signer ent_homeskillet-c48 (claude-opus-4-8). -->
...
**Refinement ray — hash pins are COMPUTED-AT-WRITE, never transcribed; a truncated hash preview is an invitation to fabricate the tail (tic 628 born → /review 632 PROMOTE-as-refinement).** A hash pin in a hand-authored receipt must be computed at write time — the writing tool hashes the file as it writes the pin — never transcribed from an earlier print or from memory. Two coupled halves: (1) CONSUMER-side, the fail-closed currency gate catches transcription drift only after re-drive cycles are spent localizing it — the first-16-match/tail-divergence signature IS the fabrication fingerprint (honest recomputation of different content cannot collide on 16 leading hex chars while diverging on the tail); (2) GENERATOR-side — the promotable core — the fix belongs at the surface that PRINTS: a preview-truncated hash (sha16) is lineage-display only and must be labeled so, or printed in FULL wherever a downstream writer might pin it. Lived: tic 628 — the lead hand-wrote drain receipts pinning reviews/2026-07-13.jsonl from a sha16 preview; the 48-char tail was fabricated; `_drain_receipt_current` refused fail-closed; three strict re-drives were spent before the first-16-match diff exposed it. Cure: computed re-pin + full-set verification against disk — held as operational law every tic since (the "anchor treadmill"). Refines this entry (the corrector's obligation extends to every pin the corrector writes) and composes producer-seal-is-a-typed-field-aperture (generator-side disclosure discipline) + don't-trust-the-sync-verify-bytes. Not a new parent.

<!-- REFINED tic 632 from cpr_hash_pins_computed_at_write_never_transcribed_tic628 (PROMOTE-as-refinement, /review 632, Architect-ratified via in-tic question set). Source: audit-logs/governance/borns-tic628-hash-pins-computed-at-write-never-transcribed.md. Enrichment: deterministic-lite single-lens baseline (lesson_class=engineering, target=cgg-ledger, proof=source_stable — zero-divergence structural, not cross-lens). Band: COGNITIVE. Confidence_tier: tentative→reinforced (lived as operational law tics 628-632, four consecutive handoffs; anchor-treadmill recurrences at 631/632 validated the discipline). Return-leg push fired same-tic (first live production push). signer ent_homeskillet-f5 (claude-fable-5). -->

**Refinement ray — SELF-authorship is the verification blind spot; probe EACH claimed cause independently at the locus before carrying it to /review (tic 680 born → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** A self-authored root-cause diagnosis must be empirically probed before it is carried to /review — coherence is not admission when the author is oneself. Lived at tic 679: the prior pass's own CogPR (cpr_mogul_review_close_check_79ae89ca3a0a) diagnosed a false GENUINE as TWO compounding resolution defects — (A) an order-of-operations fracture in _collect_targets, and (B) 'no domain-root fallback, so every cgg-runtime/... target is unresolvable.' Re-probing at the locus this tic: (A) reproduces exactly; (B) is FALSE — _resolve_target_path already ends in a bounded suffix-rglob whose own docstring names the domain-relative case, and the SAME run resolved sibling cgg-runtime/... targets correctly and printed their resolved_path. Fed the whole unsplit promoted_to, resolution succeeds; fed the post-split fragment, it fails. (A) is the SOLE live cause. The false half survived because it was COHERENT with the true half and explained the same symptom — the exact intake-gate failure mode CLAUDE.md §7 names for inbound corrections and counsel, here pointed INWARD at a diagnosis the office wrote itself. A self-authored diagnosis is optimized for the author's own compliance surface by construction: it was generated by the same reasoning that will later read it, so it meets no resistance. The gate is structural, not cleverness — before a compound diagnosis is carried to /review, probe EACH claimed cause INDEPENDENTLY at the locus and record the probe, because a compound diagnosis fails open: /review sees one coherent story and authorizes a fix for a defect that does not exist, while the real cause's severity is understated as one-of-two rather than sole. Corollary for verifier repair specifically: distinguish 'mechanism absent' from 'mechanism present but not reached' — claim B's true residue is that _target_exists/_read_target lack the rglob _resolve_target_path has, i.e. resolution is INCONSISTENT ACROSS AXES, not missing.

<!-- promoted from cpr_mogul_review_close_check_6540e0503eaa (birth_tic 680 → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: mogul:review_close_check (mogul_mandate_id tic-680; no born file — queue-row lesson is the authoritative body). REFINES this entry along the SELF-AUTHORSHIP axis: the parent names the corrector's obligation over a DELEGATED patch's predicted metrics; this ray names the same obligation turned inward, where the object under verification is the office's OWN root-cause diagnosis and there is no delegation boundary to supply resistance. EVIDENCE (two independent, one same-pass): (1) the t680 re-probe at the locus — claim A reproduced exactly, claim B FALSIFIED (_resolve_target_path already ends in a bounded suffix-rglob whose docstring names the domain-relative case; the same run resolved sibling cgg-runtime/... targets and printed resolved_path); (2) t683 cpr-stepper finding F2 — the same method class falsified the ALWAYS clause of cpr_mogul_harmony_invoke + deep_audit_f4de0ac773fb LIVE (lag 0 measured, order-dependent not unconditional), which is why that row was DEFERRED to 683+ for joint adjudication rather than fixed on its self-authored story. Composes coherence-is-not-admission (CLAUDE.md §7 inbound altitude gate, pointed inward) + bounded-delegation-surfaces-default-to-masking-bugs (federation). Runtime fix for the surviving claim A is FILED, not authorized blind (bk-review-close-check-resolution-order). compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: invariant_refinement. Confidence: 0.9. signer ent_review_execute (claude-opus-5[1m]), boot receipt a4a5b5661049ce2b. -->

**Refinement ray — a diagnosis's SCOPE QUANTIFIER and its MECHANISM are separately testable claims, and the CURE is chosen by the quantifier (tic 681 born → /review 684 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** A defect diagnosis carries two claims that fail INDEPENDENTLY and must therefore be probed independently: the MECHANISM (what the code actually does, at a named locus) and the SCOPE QUANTIFIER (always / on every run / sometimes / order-dependent). The quantifier is the load-bearing one at the fix boundary, because THE CURE A FIXER REACHES FOR IS CHOSEN BY THE SCOPE CLAIM, NOT BY THE MECHANISM — so an untested quantifier authorizes the WRONG FIX-SHAPE even when the mechanism is exactly right, and the wrongness is invisible at review precisely because the mechanism half audits clean. Lived across tics 681→684 on `cpr_mogul_harmony_invoke + deep_audit_f4de0ac773fb`: its MECHANISM — braid-input-builder.py resolving its tic from the economy current-pointer as PRIMARY authority while its parent harmony-input-builder.py reads conformation `tic_count_physical` — reverified INTACT at the named locus every time it was probed, while its QUANTIFIER ("braid resolves tic N-1 on EVERY cadence run") was falsified LIVE twice. An "always" invites a CONSTANT-OFFSET repair (add 1; treat every braid packet as lag-1); that repair is wrong on exactly the lag-0 tics and would silently mis-stamp them. An order-dependent defect cannot be cured by a constant. Generalizes past this locus: when an artifact derives its own identity from a MUTABLE POINTER that a SIBLING step refreshes, that identity is a property of EXECUTION ORDER — not of the artifact's content, not of its invocation — so the defect's period is set by scheduling, it presents as intermittent-but-mechanical rather than always-on, and a run of confirming observations cannot distinguish "always" from "whenever the order happened to land that way." Sharpens the SELF-authorship ray directly above along the WITHIN-a-claim axis: that ray requires probing EACH claimed CAUSE independently (the across-claims axis); this one splits a SINGLE claim into mechanism and quantifier and requires each to stand on its own evidence.

<!-- promoted from cpr_mogul_memory_mining_e5fa86d6ec02 (birth_tic 681 → /review 684 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: mogul:memory_mining (mogul_mandate_id tic-681-20260805T180514; no born file — the queue-row lesson is the authoritative body). REFINES this entry along the QUANTIFIER axis: the SELF-authorship ray above governs probing each claimed CAUSE independently (across claims); this ray governs the split WITHIN a single claim — mechanism vs scope quantifier — because the fix-shape is selected by the quantifier alone. EVIDENCE: the parent row's ALWAYS clause was falsified LIVE TWICE while its mechanism stood intact both times — (1) t683 cpr-stepper finding F2 measured lag 0 (economy current-pointer generated_at 2026-08-06T23:33:46Z carrying tic 683; braid current-pointer 23:33:51Z carrying tic 683); (2) t684 braid again read the ALREADY-ADVANCED economy pointer (economy-invoke 2026-08-07T16:11:05Z, braid 16:11:12Z; recorded in the t684 Mogul mandate report under cross_cycle_observations key "braid_economy_lag_zero_second_consecutive"). A constant-offset repair would have been wrong on exactly those tics. This is the SECOND lived instance of the joint-adjudication law — the corrector is adjudicated BEFORE the parent's fix is authorized; Pair A (cpr_mogul_review_close_check_6540e0503eaa correcting 79ae89ca3a0a, /review 683) was the first. Its Pair-B parent, "cpr_mogul_harmony_invoke + deep_audit_f4de0ac773fb", was MODIFY+PROMOTED the same pass at cgg-ledger/ledger.md#same-tic-replay-diverges-consumers-declare-source. Queue: status extracted → promoted; maturity discharged (birth 681 + 3 = 684). compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: correction. Confidence: 0.9. signer ent_review_execute (claude-opus-5[1m]), boot receipt 92a60d0448ce0ae8. -->
...
**Refinement — a PURE-POLICY gate's ungate surface should be reduced to ONE source swap (tic 538 born → /review 539 SKIP-with-home → REFINE-into-build-and-gate, Architect-gated [AskUserQuestion approval batch]).** Build-and-gate ships a doctrine-adjacent model with its consumer wired-but-`ratified`-gated; this names the sibling discipline for the class of gate where the "flag" is a bid/input SOURCE, not a boolean. When a live-coupling gate has had EVERY non-policy blocker-type cleared (capability proven + interface sanctioned + the build lane complete — the AND-over-blocker-types from the Blocker-Type Taxonomy composition axis), it is a PURE POLICY gate: nothing technical remains, only the authorizer's bell. The highest-leverage way to build toward such a gate is to collapse its go-live to a SINGLE SOURCE SWAP — build + test the ENTIRE downstream pipeline (adapter → gate → verdict → receipt) on a RECORDED source behind a stable interface, so uning-gating is one flag (`recorded_receipt → live_model`), not a fresh build; everything downstream of the bid source is byte-identical for both sources. Discipline: (1) classify the gate — is any non-policy blocker-type still owed, or is it pure-policy? (2) if pure-policy, locate the single swap point (the source) and build the whole pipeline on the recorded/fixture source so the swap is the ONLY delta; (3) drive the live source through the OWNING office (the egress-router S3_model_proposer DRIVER), never inline in the orchestrator lane, and never fake a live receipt against a recorded source (Blocker-Type Taxonomy no-fake-receipt). Derivable (NOT net-new): composes build-and-gate (gate USE behind a flag) + the Blocker-Type-Taxonomy AND-over-types composition axis (pure-policy = all non-policy cleared) + lane-separation; the source-swap-as-ungate-surface specialization is the durable home. SKIP ≠ DISCARD honored.

<!-- refinement edge from cpr_pure_policy_gate_ungate_is_one_source_swap_tic538 (tic 538 → /review 539, SKIP-with-home → REFINE-into-build-and-gate, Architect-gated [AskUserQuestion approval batch]). Source: audit-logs/governance/borns-tic538-pure-policy-gate-ungate-is-one-source-swap.md. A pure-policy gate (all non-policy blocker-types cleared) should have its ungate reduced to ONE source swap: build the whole downstream pipeline on a recorded source behind a stable interface so go-live is a single flag (recorded_receipt→live_model), not a build. Composes build-and-gate + Blocker-Type-Taxonomy AND-composition-axis + lane-separation. Derivable-from-existing (NOT net-new) → recorded as a refinement ray, durable home. First live conformation = the tic-538→539 SP5 wire (the ungate WAS one source swap; live-couple verdict ADMIT_PROPOSAL_REFUSE_MOVE, gate unchanged). Band: COGNITIVE. signer ent_homeskillet-c48 (claude-opus-4-8). -->
...
**Refinement — enumerate EVERY leak boundary as a closed set {write / speak / search}, and guard at the EARLIEST one (tic 557 born → /review 566 PROMOTE-as-refinement, Architect-plan-authorized).** A footgun guard must cover EVERY boundary through which its error can leak — not just the first one instrumented — and the leak boundaries are a CLOSED enumerable set: **write / speak / search**. The tic-548 tool-absence write-gate (PreToolUse on Edit|Write|Bash) blocks INSCRIBING a "primitive not-found" conclusion, but the same false conclusion can leak by being SPOKEN to the human, or can FORM at the SEARCH itself — and both slipped the write-gate at tic 557 (a ToolSearch primary-tool no-match was reported in prose as the Agent primitive being unreachable, never touching Edit/Write; #10 recurrence of the Agent-tool-presence-probe class). The durable complement is a guard EARLIER in the chain: a PreToolUse hook on ToolSearch that injects the primary-tool roster + lite-specs the moment a primary tool is searched, so the false not-found signal never forms (primary-tool-search-guard.py + primary-tools-registry.json, CGG 5276936, built + live-fire proven). General shape: **instrument at the EARLIEST leak boundary, and enumerate ALL leak boundaries as a closed set rather than patching one at a time.** This is the leak-BOUNDARY axis of the same closed-consumer-set logic this KI names on the SIBLING-SITE axis; it refines `#footgun-guard-at-perception-layer-warns-after-the-footgun-already-fired` (the write-gate is itself a late guard for the spoken/search leak) and composes the tool-absence-claim family + engine-content separation (registry = content, hook = engine). Discriminator: "does this guard cover every door the error can exit by — write / speak / search — and does it sit at the earliest one, or only the first door I happened to instrument?"

<!-- promoted from cpr_footgun_guard_must_cover_every_leak_boundary_not_just_the_first_tic557 (tic 558, /review 566 PROMOTE-as-refinement, Architect-plan-authorized [tic-566 plan named this a mature PROMOTE; interactive gate timed out; conservative refinement-edge form, git-reversible]). Source: tic 557 — ToolSearch primary-tool no-match reported in prose as Agent-primitive unreachable, bypassing the tic-548 write-gate (#10 recurrence feedback_agent-tool-presence-probe); fix primary-tool-search-guard.py + primary-tools-registry.json BUILT+LIVE, CGG 5276936. Refines footgun-guard-at-perception-layer (leak-boundary axis: write/speak/search closed set + earliest-boundary instrumentation) + extends this KI's closed-consumer-set from sibling-SITES to leak-BOUNDARIES; composes tool-absence-claim family + engine-content-separation. Non-derivability: net-new is the closed leak-boundary set {write/speak/search} + earliest-boundary instrumentation; recorded as a refinement edge, not a parent. Band: COGNITIVE. Confidence_tier: reinforced (n=10 recurrence class, cure live-fire proven). signer ent_homeskillet-opus48. -->
...
**Refinement — dispatch-instruction text is harness perimeter (tic 552 born → /review 555 PROMOTE-as-refinement).** The perimeter this KI re-earns at every harness bump extends beyond schema couplings (settings keys, hook events) to INSTRUCTION TEXT the model reads at runtime: boot injections, mandate prompts, skill bodies, protocol blocks. Protocol text that hardcodes one harness's tool name converts a harness-variance question into a false capability-absence — and the instruction survives every harness bump because it reads as doctrine. The durable phrasing is role-shaped, not tool-shaped: "dispatch the ⟨office⟩ steward — an entity at the appropriate state, via the lead harness's active dispatch surface" (one harness's binding may be given as an example, never as the protocol). Exemption: detector test-corpora that must MATCH harness-specific phrases (claim gates) quote the phrase to catch it, not to command it.
...
**Verifier-Split Chapter 4 — the RESOLUTION layer beneath classification: if `promoted_to` never resolves to a file, NO classification axis can fire (tic 679 born → /review 683 MODIFY+PROMOTE-as-ray, Architect-ratified in-tic question set).** A verification pipeline's RESOLUTION layer fails silently ahead of its CLASSIFICATION layer, and that failure compounds into a false GENUINE. Chapters 1–3 above all work the CLASSIFICATION axis (which reason-code a finding earns). Tic 679 surfaced the layer beneath: if `promoted_to` never resolves to a file at all, NO classification axis can fire, and the finding falls through as GENUINE-missing for an inscription that is demonstrably present and executing. The live defect, empirically reproduced: **ORDER-OF-OPERATIONS** — `_collect_targets` calls `_split_compound_targets` on the RAW `promoted_to` before scope-hint stripping, so a parenthetical scope hint that legitimately contains `' + '` is fractured mid-parenthesis; the resulting component carries an UNCLOSED paren, and `_SCOPE_HINT_RE` (anchored `r'^(.*?)\s+\(.*\)$'`, requiring a closing paren at end-of-string) then cannot strip it, so the mangled string is passed to the filesystem as a path. **Normalization must precede decomposition:** strip scope hints FIRST, split compounds SECOND, or split only on `' + '` occurring OUTSIDE parentheses. Measured blast radius over the 641 promoted rows: this defect affects 4. The generalization that survives: a verifier whose redundant axes (provenance-comment index, cpr_id literal, lesson snippet) mask its resolution failures reports its own blind spot as the system's defect, and the false GENUINE lands on whichever record the surviving axes happen not to rescue — an arbitrary victim, not a signal. **MODIFY note — the born's claim (B) is STRUCK.** The born asserted a second, independent defect: "RUNG-AMBIGUOUS RELATIVE PATH … `_read_target`/`_target_exists` … have NO domain-root fallback; every target recorded relative to the CGG domain root is therefore unresolvable." That claim was FALSIFIED at the locus by this row's own corrector (`cpr_mogul_review_close_check_6540e0503eaa`, promoted same pass): `_resolve_target_path` already ends in a bounded suffix-rglob whose docstring names the domain-relative case, and the same run resolved sibling `cgg-runtime/...` targets and printed their `resolved_path`. Claim B's true residue is narrower and is carried by the corrector ray, not here: resolution is INCONSISTENT ACROSS AXES (`_target_exists`/`_read_target` lack the rglob `_resolve_target_path` has), not missing. The compound-victim framing the born built on top of B ("trips BOTH defects at once") falls with it.

<!-- promoted from cpr_mogul_review_close_check_79ae89ca3a0a (birth_tic 679 → /review 683 MODIFY+PROMOTE-as-ray, Architect-ratified in-tic question set; DEFERRED at /review 682 under the anchored maturity law — advanced_tic 682 + maturity_window_tics 1 — pending its corrector's maturity, and adjudicated JOINTLY with that corrector this pass exactly as the 682 DEFER required). Source: mogul:review_close_check (mogul_mandate_id tic-679; no born file — queue-row lesson is the authoritative body). Verifier-Split CHAPTER 4: extends the Chapter-1–3 classification axis DOWNWARD to the resolution layer. MODIFY APPLIED: the lesson is narrowed to claim (A) ONLY; claim (B) ("no domain-root fallback") is STRUCK as falsified by cpr_mogul_review_close_check_6540e0503eaa — the t682 anchored-DEFER law honored in full (the correction was admitted BEFORE any fix authorization, never after). RUNTIME FIX NOT AUTHORIZED BLIND: the `_collect_targets` ordering repair at review-close-check.py:368 is FILED as a backlog row (bk-review-close-check-resolution-order, filed by the orchestrator), not executed by this inscription. `partial_falsification_pending` from the 682 row is hereby DISCHARGED (the falsification resolved: B struck, A stands). Gate1 staleness reverified at 683 — both Pair-A loci present (review-close-check.py:368 `_collect_targets`, :1009 `_resolve_target_path`). compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: invariant_refinement. Confidence: 0.85. signer ent_review_execute (claude-opus-5[1m]), boot receipt a4a5b5661049ce2b. -->
...
## A Status Name Asserting an Unmet Need With NO Rename Owner Goes Semantically Stale the Moment the Need Is Met — Count the FIELD, Not the LABEL
<a id="need-asserting-status-name-needs-rename-owner-count-field-not-label"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=678 | promoted_tic=682 | relations=sibling:status-value-reader-disagreement-sticky-masks-reactivated-item,sibling:silent-transition-deadlock-gate-checks-next-state-output,composes:named-footgun-guard-leaves-sibling-site-unfixed,composes:presence-observation-fallacy-guard(federation),composes:authoritative-set-readers-must-read-the-manifest(federation) | confidence_tier=tentative -->

A lifecycle status whose NAME asserts an unmet need (`enrichment_needed`) but whose rename is owned by NO actor goes semantically stale the moment the need is met — and every downstream reader that counts the LABEL instead of the FIELD then reports **phantom backlog** where the substrate holds a ripe docket. The failure is not the queue: **it is that the label survives its own predicate.**

**Ownership void** (structurally re-probed at tic 682, not inferred): `cpr-gate-advance.py::find_advanceable` gates strictly on `status == 'tic_gated'` (:120) and writes only `enrichment_needed` (:170); `cpr-enrichment-scanner.py` declares `HOLDING_STATUSES = {'enrichment_needed','enrichment_eligible'}` (:36) and gathers evidence into `enrichment[]` but writes NO status; `cpr-stepper.md:74` explicitly disclaims the transition (it owns `extracted → tic_gated` after the tic-470 deadlock fix, and everything downstream of `enrichment_eligible`). So no actor owns `enrichment_needed → enrichment_eligible`. At tic 678 all 13 `enrichment_needed` rows (born t650–t658, maturity_tics 3, ripe since t653–t661) carried populated `enrichment[]` (2–3 entries each, `no_evidence_reason` None) and were already docket-visible to the bench-packet reader whose `PENDING_STATUSES` includes `enrichment_needed` — a ripe docket for /review 679, not a starved lane.

**Two coupled cures, both cheap; either closes it:** (1) the actor that SATISFIES a named need OWNS the rename — or the status name must not assert a need it cannot retire; (2) any report, banner, handoff, or audit citing "N `enrichment_needed`" must state whether it counted LABELS or EVIDENCE FIELDS — a label-count and an evidence-count are different measurements and may not be reported interchangeably.

**Compounding hazard — the fix-site set is WIDER than first filed** (widened at /review 682): TWO ambient boot banners assert the unowned transition as automatic, not one. `session-restore.sh:447` (line drifted from :434 as the file grew; the claim is intact) emits *"Completion updates queue.jsonl status. No action needed — enrichment runs automatically"*; and — NET-NEW at 682 — `session-restore.sh:885` emits *"[CPR LIFECYCLE v1: pending CogPRs advance through: extracted -> enrichment_needed -> enrichment_eligible -> promotable -> promoted]"*. Both assert an automation the `HOLDING_STATUSES` constant structurally prevents, and the reassurance suppresses the only escalation path the transition actually has. Fix-site and sibling-site are a closed set (`named-footgun-guard-leaves-sibling-site-unfixed`).

**Corroboration — the edge is never walked in practice:** all 13 rows of the cited birth-650..658 cohort had reached terminal by tic 682 (9 promoted / 3 skipped / 1 absorbed) and NOT ONE traversed `enrichment_eligible`; each went from `enrichment_needed` directly to a terminal /review verdict. This confirms the central claim twice over — the cohort was a ripe docket rather than a starved lane, AND the `enrichment_needed → enrichment_eligible` edge is dead in practice.

**Honest provenance:** the mining pass that produced this lesson produced the WRONG first read (*starvation*) from the label plus that banner, and was overturned only by reading `enrichment[]` per row.

<!-- promoted from cpr_mogul_memory_mining_a92f52e29300 (birth_tic 678 → /review 682 MODIFY+PROMOTE as a refinement ray in the status-label family, Architect-ratified in-tic via question set). Source: mogul:memory_mining cycle (mandate tic-678-20260805T021950). MODIFICATION applied at ratification: the fix-site set carried by the born row was WIDENED — the row cited only the session-restore.sh:434 banner (now :447, line drifted, claim intact); a SECOND ambient boot banner at session-restore.sh:885 asserts the same unowned transition as automatic. Cohort corroboration added: 13/13 of the birth-650..658 cohort terminal at 682 with zero traversals of enrichment_eligible. Sibling of status-value-reader-disagreement-sticky-masks-reactivated-item (that entry is READER disagreement across two readers; this is single-reader LABEL/FIELD divergence) and of silent-transition-deadlock-gate-checks-next-state-output (the ADJACENT transition one step upstream — unlike the deadlock there is no gate here at all, only an ownership void declared by a constant). Adjacency-not-absorption checked at 682 against cpr_promotion_asserting_automation_must_cite_observed_fire_tic651 (promoted to constitution-ledger#presence-observation-fallacy-guard guard-8), which scopes to /review PROMOTIONS asserting automation and reaches neither the ambient boot-banner locus nor the primary half. Band: STRUCTURAL. Confidence_tier: tentative (single mining pass; stability window discharged 678→682 with zero contradictory evidence and four independent corroborating probes). signer ent_review_execute (claude-opus-5[1m]), boot receipt a0006c4eff5b58bb. -->

---

## Zero-Divergence In A Single-Lens Baseline Is Structural, Not Cross-Lens Agreement
<a id="zero-divergence-single-lens-baseline-structural"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | promoted_tic=570 | first_appearance_tic=566 | refines=presence-observation-fallacy-guard(federation, guard-2 artifact-existence-is-not-semantic-fulfillment) | relations=sibling:cpr_cogpr_baseline_metadata_decoupled_from_full_enrichment_tic427,composes:mogul-frame-fidelity | confidence_tier=tentative -->
...
<!-- reinforced_by: cpr_mogul_review_close_check_0ddacf1f8896 (tic 743, source: mogul:review_close_check, absorbed-as-reinforcement /review 743 Q6 into the t732 DISCLOSURE-PARITY ray — the TWO-COUNTER face). Independent rediscovery / down-audit reinforce-signal — resilience evidence (the KI re-derived from rung friction, not merely mentioned). Stamped by review-promote-writeback (Stage 5). -->

**Refinement ray — the READER-SIDE closure corollary: a reader's dispatch set must be CLOSED over the emitter's verdict-shape universe; an uncovered shape closes un-verified and the artifact cannot say so (tic 696 born → /review 699 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** The parent names the emit side (rows must match a reader predicate); this ray names the read-side inversion: a consumer that dispatches on a predicate SET must have that set closed over the emitter's actual shape universe, with an unknown shape raising a TYPED finding rather than falling through silently. Lived: review-close-check.py dispatches on status ∈ {promoted, deferred/enrichment_eligible, skipped} plus lifecycle_state ∈ LIFECYCLE_SETTLED_STATES, and emits verdict_counts over {promoted, deferred, skipped}; the `absorbed` status (the MERGE verdict shape) is in NEITHER set — at tic 695, 223 of 1139 latest-per-id rows (19.6%) fell through every branch, zero checks run, zero counter emitted, and the tic-695 MERGE closed with the check structurally blind to it. A MERGE therefore cannot produce a promoted_text_missing or orphaned_promotion finding no matter how broken the absorption is — the silence is structural, not evidence of health (zero-by-blindness at the verdict-shape axis; composes the federation four-state zero taxonomy). Coupled obligation: any cross-reference the checker must resolve (the MERGE carrier) must be a STRUCTURED machine-resolvable id (`absorbed_into`), never prose-only inside `absorbed_reason`.

<!-- promoted from cpr_mogul_review_close_check_d0c2e2d0867c (birth_tic 696 → /review 699 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: mogul:review_close_check (mogul_mandate_id tic-695-20260810T135620 — the check ran under the t695 mandate, the born minted at 696; CORRECTED tic 700 from queue truth, was "tic-696", Architect-ratified in-tic — drill-discovered divergence, c3-fidelity-drill-tic700; no born file — queue-row lesson is the authoritative body). REFINES this entry along the READER-SIDE axis: the parent obligates the emitter to match a reader predicate; this ray obligates the reader's predicate/dispatch set to be closed over the emitter's shape universe, with unknown-shape → typed finding. Cross-cites constitution-ledger four-state-zero-taxonomy (zero-by-blindness) as FIELD. NOT a MERGE with sibling f5a8f2a39d37 (assessed: siblings by fix-site, not centroid — coverage/blindness vs question/delta). FIX-SITE FILED FROM THIS VERDICT, NOT EXECUTED HERE: bk-review-close-check-verdict-shape-closure (MED; arms: dispatch closure + structured absorbed_into in queue-lifecycle-writeback + the delta arm from the sibling ray). compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: coverage_gap. Confidence_tier: observed. Confidence: 0.85. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 6ad8d0ab6340bbae. -->

**Refinement ray — the QUESTION-PREDICATE corollary: a lane dispatched to answer a per-tic question must emit the per-tic answer (the delta) as a first-class field, not whole-universe absolutes (tic 696 born → /review 699 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** The dispatcher's question is itself a reader predicate. A close check dispatched BY a review mandate is asked "did THIS tic's verdicts land clean?" — but an artifact that emits only whole-universe absolutes (total_findings, consistent, verdict_counts) cannot be matched against that question; the per-tic answer is only recoverable by hand-diffing the current artifact against the prior tic's, which the script never reads. Lived at tic 695: the whole-universe verdict read `consistent: false` with 14 findings — unchanged in count from tic 694 — while the actual close signal was in the delta: finding set byte-identical ID-for-ID (0 new, 0 cleared) as promoted advanced 674→675, which is what proved the tic-695 PROMOTE's inscription verified. Delta-reading is already known-load-bearing outside the artifact (the script's own :1571 comment records a dead-check discovery via a 35-vs-36 counter delta; the operator practice "Nth consecutive genuine-zero" is the same read done by hand) — a lane whose purpose is per-tic close verification must emit that delta as a first-class field.

<!-- promoted from cpr_mogul_review_close_check_f5a8f2a39d37 (birth_tic 696 → /review 699 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: mogul:review_close_check (mogul_mandate_id tic-696; no born file — queue-row lesson is the authoritative body). SECOND ray on this entry, distinct axis from d0c2e2d0867c (coverage/blindness vs question/delta — assessed NOT a merge; same fix-site). The dispatcher's per-tic question treated as the reader predicate the emission must match. Tentative tier honored: n=1 lived instance (t695) + the :1571 comment as prior-art-internal + the standing genuine-zero operator practice. Delta-emission arm rides bk-review-close-check-verdict-shape-closure (MED). compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: observability_gap. Confidence_tier: tentative. Confidence: 0.70. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 6ad8d0ab6340bbae. -->

**Refinement ray — the OPEN-VOCABULARY corollary: a reader predicate anchored on a CLOSED verb alternation over an OPEN authoring vocabulary silently sheds every new emitter shape — the predicate owes generative matching or a loud unmatched-shape residue counter (tic 716 born → /review 719 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic question set).** The parent obligates the emitter to match a reader predicate at emit time; the first two rays close the reader's dispatch set and its per-tic question. This ray names the TEMPORAL direction: when the emitting vocabulary is OPEN (the /review lane mints new verdict-head shapes freely — a governance feature, not drift) and the reader predicate is a CLOSED, hand-maintained lexeme alternation anchored at the comment head, every new shape the authoring lane invents drops out of the index while the inscription itself is perfectly valid. The failure is silent and INVERTED-SAFE in the worst way: the doctrine landed, so no finding fires — only the COUNTER goes quiet, and a flat counter reads as "nothing was inscribed" rather than "the inscription was unreadable to me." Lived at tic 716: the /review-716 promotion of cpr_mogul_review_close_check_07c597566b16 landed (commit 5e847af4), the mandate predicted a positive inscribed_index delta, and the measured delta was 0 — the comment opens `PROMOTE-AS-REFINEMENT`; measurable ONLY because the counter's unit had been declared one cure earlier. The cure is to stop matching a hand-maintained lexeme list against a vocabulary another lane authors: match a verb STEM/prefix family or register the verdict vocabulary ONCE at a single source both lanes read — and in ALL cases carry a loud, non-gating unmatched-provenance-shape residue counter so an un-admitted head surfaces beside the index instead of silently shedding (a new verdict shape must not be able to enter doctrine without either entering the matcher or entering the residue report). Deliberately scoped: a SKIP head is NOT an inscription witness and its exclusion is CORRECT — the defect covers only heads that genuinely witness an inscription.

<!-- promoted from cpr_mogul_review_close_check_3a40ab346adb (birth_tic 716 → /review 719 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic question set, reviewer ent_homeskillet). Source: mogul:review_close_check (mogul_mandate_id tic-716-review-close-20260815T1722; no born file — the queue-row lesson is the authoritative body). THIRD ray on this entry, distinct axis from d0c2e2d0867c and f5a8f2a39d37 (emit-vocabulary EVOLUTION vs dispatch-set closure vs question-delta — no merge; the author's own routing note named this parent and it held at adjudication). Composes #predicate-retirement-needs-reader-consumer-sweep (predicate-axis closed-consumer-set) and the federation /review-713 de-enumeration ray on structural-transform-implies-closed-consumer-set-obligation (a closed enumeration rotting against an open family — same centroid, reader-predicate locus). FIRST-CONSUMER PATCH EXECUTED SAME PASS (feedback_patch-first-consumer-not-inscribe-around): review-close-check.py _PROVENANCE_VERB_RE admitted the t716-parked census-residue heads (inscribed, review-executed — the adjudication the tic-716 comment reserved them for) + the row's measured unmatched witness heads (conditional-promoted, merge, reinforced), and build_inscribed_index grew the non-gating unmatched-provenance-shape loud counter (count + samples in diagnostics + stderr). Fixtures: 6 new (admitted heads, novel-head-fires-counter-not-index, skip-head-not-counted, reserved-only-not-counted, tokenless-not-counted, merged-longest-first), suites 28/28 green. First live dry-run: inscribed_index 727→732, +5 ATTRIBUTED to verb admission (token unit, NOT inscription events — the delta declares its cause per 77b1's baseline-side law); unmatched counter first fire = 28 residue comments made visible (landed-from, C5 REINFORCE:, mid-comment verbs — future-adjudication food, surfaced not silently admitted). compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: mechanism_correction. Confidence: 0.80. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 3b1db38684f08e44. -->

**Refinement ray — the OBLIGATION-SCOPE corollary: a per-pass delta obligation landed on one counter attaches to EVERY counter on the same artifact surface — the rider's cure is surface-wide, never counter-local (tic 725 born → /review 728 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic question set).** The question-delta ray obligates the lane to emit its per-tic answer as a first-class delta; this ray names the SCOPE of that obligation once it lands: the cure attaches to the artifact SURFACE, not to the single counter whose defect prompted the rider. Lived at tic 725, the first baseline-bearing pass of the delta emitter (/review-724 RIDER 2): `inscribed_index_size` carries a full delta block (units, current, delta, baseline artifact + selector, collapse flag, delta_baseline_absent) while its sibling `verdict_counts` {promoted, deferred, skipped} sits in the SAME report as bare scalars — no unit declaration, no per-pass delta. A consumer can predict the index counter in its own units pass-over-pass and cannot do the same for the verdict counters without holding the prior artifact by hand — the exact hand-diff the question-delta ray exists to retire, reintroduced one counter over. The asymmetry also strands a free cross-counter predicate: at that pass, promoted moved 721→724 (+3) and delta_tokens moved 750→753 (+3) — two independently derived counters agreeing on the /review-725 PROMOTE count, with nothing in the artifact comparing them. Deliberately fenced (the row's own APO, held at adjudication): this is NOT a claim that delta_tokens MUST equal the promoted-verdict delta — known divergence routes exist (a MODIFY-and-merge promotion adds no new provenance comment; a doctrine-surface edit narrating a sibling id adds a token with no promotion behind it); the divergence is a typed question worth DISCLOSING, never an invariant worth asserting. NOT the collapse flag's job (that discloses the index counter's own two units moving identically); NOT a finding that the observed pass was inconsistent (genuine_count was 0). Same centroid as the federation closed-consumer-set obligation at the counter-obligation locus: a rider discharged only at the defect-prompting member leaves its siblings dark, and the darkness reads as "nothing to disclose."

Lock line: *A delta obligation lands on the surface, not the counter — every sibling counter ships with units, baseline, and delta, or the hand-diff the rider retired comes back one column over.*

<!-- promoted from cpr_mogul_review_close_check_c209995ad848 (birth_tic 725 → /review 728 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic via AskUserQuestion question set, recommended option verbatim; confidence 0.83). Source: mogul:review_close_check (no born file — the queue-row lesson is the authoritative body, inscribed near-verbatim above). FOURTH ray on this entry, distinct axis from the emit-vocabulary, dispatch-set-closure, and question-delta rays (this one scopes the OBLIGATION once landed — surface-wide, never counter-local; no merge). Composes the federation closed-consumer-set obligation at the counter-obligation locus and the question-delta ray it extends. FIRST-CONSUMER PATCH EXECUTED SAME PASS (feedback_patch-first-consumer-not-inscribe-around): review-close-check.py verdict_counts now ships a verdict_counts_delta block (same shape as inscribed_index_delta: units, current, per-key delta, baseline artifact + selector, delta_baseline_absent) + a derived cross_counter_disclosure field (promoted-verdict delta vs provenance-token delta, typed disclosure with divergence routes named, never an asserted invariant). DISTINCT from bk-close-check-counter-disposition-split — the over-admission disposition cure stays owed in its own backlog lane; this patch adds disclosure surface, it does not touch admission. deferred_facets carried from the row: PLE declared absent (no complementary mechanic identified at mint). Dual clamp carried: cost_of_action small (both counters already exist in the same computed report); cost_of_inaction: verdict counters stay unpredictable pass-over-pass and the promoted/inscribed agreement — the exact axis the checker guards — is recomputed by hand at each /review. Queue: status extracted → promoted; maturity discharged (725+3=728, adjudicated at the tic it matured; the docket fence parked this row at both the t727 and t728 stepper passes — fence sole holder, maturity satisfied). compact_root_status: ledger_only. lanes: [signal_and_queue_manifold, verification_and_proof_discipline, measurement-integrity]. Band: STRUCTURAL. lesson_type: obligation_scope_correction. Confidence_tier: tentative. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 336e645ac6646638. -->

**Refinement ray — the DISCLOSURE-PARITY corollary: making a residue VISIBLE and EXPLAINING it are separate obligations, and discharging the first makes the second look discharged — parity applies PER SIDE of the boundary (tic 732 born → /review 735 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** When a boundary sheds material, the standard cure is a loud counter — and it correctly ends the silence. But the PROSE that travels with the counter is what a downstream reader acts on, and a single-cause narrative over a MULTI-CAUSE population is a false diagnosis wearing an honest number. Disclosure parity must therefore be applied PER SIDE of the boundary: enumerating admission routes on the over-inclusion side while giving the under-inclusion side one integer and one sentence leaves the un-enumerated side reading as UNDERSTOOD *because* it is COUNTED. MEASURED at tic 732 on review-close-check.py's inscribed index: the over-admission side carries 10 enumerated routes with per-route machine-countability flags and live occurrence counters (/review 724 RIDER 1); the under-admission side carries `unmatched_provenance_shaped_count=28` plus one stderr sentence ("fail the inscription-verb alternation … a new verdict head must enter the matcher to enter the index"). Re-implementing the counter's own predicate reproduced 28 EXACTLY, so the decomposition is exhaustive rather than sampled: **15** are auto-memory `<!-- --agnostic-candidate -->` born-blocks (WRONG OBJECT CLASS — a birth record is not a failed inscription witness, and it lives on the surface t570 decoupled from the extraction lane); **9** are comments whose verb IS in the alternation but is not head-anchored ("Non-derivability criterion promoted from CogPR-110", "C5 REINFORCE: absorbed narrow residual from …", 3× "ledger-tags: … | promoted_" where the match is a tag KEY); only **4** ("landed-from", "home-pointer from") are the vocabulary gap the sentence describes. The stated cure is correct for 4 of 28 and ACTIVELY WRONG for 15 — registering a new verb can never admit a non-head-anchored "promoted", and must never admit a born-block. Corollary, independently measured: the memory root is genuinely DUAL-ROLE (it contributes 12 matched breadcrumbs / 14 distinct tokens to the index while hosting 15 born-blocks), so the cure is ROLE-DISCRIMINATION on the surface, not verb registration in the matcher.

Lock line: *A counter ends the silence; the sentence beside it starts the diagnosis — and an un-enumerated side reads as understood because it is counted.*

<!-- promoted from cpr_mogul_review_close_check_835c9e0f1165 (birth_tic 732 → /review 735 PROMOTE-as-refinement-ray, Architect-ratified in-tic via AskUserQuestion question-set rounds 1-2, every recommended option verbatim; confidence_tier measured). Source: mogul:review_close_check (mogul_mandate_id tic-732-20260824T130253; no born file — the queue-row lesson is the authoritative body, inscribed near-verbatim above). FIFTH ray on this entry, promoted to the row's OWN recommended scope (cgg-ledger) — distinct axis from the dispatch-set-closure, question-delta, open-vocabulary, and obligation-scope rays: those four govern WHETHER a shape is matched/counted; this one governs what the counter's PROSE may CLAIM about the population it counts. APOPHATIC PERIMETER, load-bearing: NOT "the counter is wrong" (it reproduced 28/28 exactly — the integer is the trustworthy part); NOT "stop scanning auto-memory" (that surface contributes 12 real matched breadcrumbs; removing it would lose true witnesses); NOT the closed-list/open-vocabulary lesson banked at /review 719 (that one is why the counter EXISTS; this is about what the counter SAYS); NOT a gating proposal (disclosure parity is DISCLOSURE, never admission). Cross-rays: ledger.md#presence-observation-fallacy-guard (a counter is an observation; the cause-narrative beside it is an INTERPRETATION, and they do not carry the same confidence) · composes ledger.md#structural-transform-implies-closed-consumer-set-obligation (a DECOUPLING is a structural transform too — t570 closed the extraction lane and never enumerated the surface's other readers) · distinct_from ledger.md#artifact-language-must-not-exceed-its-declared-confidence-classification (that governs a claim against its OWN declared class; this governs an EXPLANATION against the POPULATION it explains). SLICE SCOPE, carried verbatim from the row: observed at tic 732 on the review-close-check inscribed index and its /review-724 disclosure block; the general shape is PROPOSED, not established beyond this observer-indexed slice. Dual clamp carried: cost_of_action = one cause-taxonomy added beside an existing counter, mirroring a disclosure pattern the same file already implements on its other side; cost_of_inaction = a downstream reader registers "landed-from", watches 28 drop to 24, and reads the residual 24 as the same solved class — the wrong-object-class half becomes permanently invisible behind a number that moved. CHECKER-SIDE ENUMERATION IS A FOLLOW-ON, NOT SAME-PASS (explicit docket instruction — this pass inscribes the law only). compact_root_status: ledger_only. lanes: [signal_and_queue_manifold, verification_and_proof_discipline, measurement-integrity]. Band: STRUCTURAL. lesson_type: disclosure_discipline. signer ent_review_execute (claude-opus-4-8), boot receipt 68b793d87b761886. -->

**Reinforcement — the TWO-COUNTER face (ABSORB-as-reinforcement, tic 740 → /review 743, `cpr_mogul_review_close_check_0ddacf1f8896`; Architect-ratified in-tic AskUserQuestion round 2 Q6, recommended option verbatim; confidence measured): a CROSS-COUNTER disclosure must declare the POPULATION DIFFERENCE between the two counters it compares, not merely each counter's own population — `promoted_delta` counts one of eleven queue statuses while `token_delta` admits an inscription witness landed by ANY verdict class (absorbs included), so `agree=false` is BY CONSTRUCTION whenever an absorb-as-reinforcement lands its seat-mandated `reinforced_by:` breadcrumb, and the counter it moved is not present on the verdict side to be compared. Its second half rides the t713 accretion ray on `constitution-ledger#structural-transform-implies-closed-consumer-set-obligation`: the disclosure's static `divergence_routes` list (review-close-check.py:2747, transcribed from the b730 obligation-scope ray's APO perimeter, authored /review 728 — BEFORE the breadcrumb discipline existed, ratified /review 739) is a closed consumer set that a newly ratified verdict class silently invalidates; the operative route at t739, t740 and t742 (promoted_delta 1 / token_delta 2, identical shape) was named by none of the three. A typed disclosure whose typing is wrong reads as a diagnosis, and a wrong diagnosis costs more than a bare unknown. FIRST CONSUMER PATCHED same pass: the route list gains `absorbed_reinforcement_breadcrumb_adds_token_without_promotion` and the disclosure declares `populations` (verdict side: `status == promoted`; index side: any verdict class landing a witness). APOPHATIC: NOT a claim `agree=false` is a defect (the parent ray fenced it; the defect is the ROUTE LIST and the undeclared population, never the divergence); NOT a claim the pass was inconsistent (genuine_count 0, universe_classified true). Derivability weighed at adjudication (stepper A7-743): both halves are entailed by rays already on the record — the population-prose claim by THIS ray, the route-list rot by the t713 accretion ray — so the landing is reinforcement, and what is carried is the two-counter form and the patched consumer.**

**Refinement ray — the PER-LEG FRESHNESS corollary: a freshness/divergence disclosure field is an emitter row whose reader predicate is "every payload leg this packet carries" — a detector scoped to one leg makes the empty-flags state assert cleanliness exactly when it has stopped measuring (tic 733 mint → /review 736 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic question set, recommended option verbatim).** The parent obligates an emitter's rows to match a reader predicate; the disclosure-parity ray splits visibility from explanation per side. This ray names the LEG axis on a disclosure field itself: when a packet carries N payload legs and its staleness/divergence disclosure is computed from ONE leg, the disclosure's implicit claim ("nothing diverged") silently narrows to "the one leg I still watch agreed" — and the narrowing is invisible precisely because improving the watched leg's freshness EMPTIES the flags. Measured at tic 733 in the braid input builder: `clock_divergence` computed exclusively from the economy pointer leg (braid-input-builder.py:157-175); the harmony leg presence-checked only (:209), never freshness-checked; across braid inputs for tics 722–733 the harmony leg lagged the braid tic by exactly 1 in 12/12 tics (deterministic by-design: harmony-invoke runs the braid at step 0 and writes disposition-current.json at step 3) — and at exactly the two tics where the economy heartbeat ran first (731, 733), `clock_divergence` went null and `honest_flags` went empty, asserting clock cleanliness while the harmony leg was stale at both. The correct reading is NOT "the lag is wrong" (it is by-design) but "the lag is UNDECLARED, and the disclosure reports cleanest exactly when it has stopped measuring." Two teeth: (1) a freshness detector must be scoped to EVERY payload leg the packet carries, not the leg whose divergence originally motivated it; (2) an empty honest_flags array must mean "all legs checked and current," never "the checked subset agreed" — which requires the artifact to disclose WHICH legs were checked (a freshness_checked_legs enumeration), so the empty state is scope-welded. A by-design lag is DECLARED as such, not flagged as staleness — declaration and alarm are different verbs.

Lock line: *A disclosure field that watches one leg of an N-leg packet reports cleanest at the exact moment it stops measuring — scope the detector to the payload, and weld the checked-legs enumeration to the empty state.*

<!-- promoted from cpr_mogul_harmony_invoke_37ac2699f535 (birth_tic 733, review_tic 736 declared at mint → /review 736 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic via AskUserQuestion question-set round 1, recommended option verbatim). Source: mogul:harmony_invoke (mogul_mandate_id tic-733; no born file — the queue-row lesson is the authoritative body, inscribed near-verbatim above). SIXTH ray on this entry, distinct axis from the dispatch-set-closure, question-delta, open-vocabulary, obligation-scope, and disclosure-parity rays: those govern shape matching, per-tic answers, vocabulary evolution, obligation scope, and per-side prose; this one governs a disclosure FIELD's coverage over the payload legs of the packet that carries it. Severity LOW-MED scope-honest per the row's own note: the braid is fail-soft non-citable shaping input and per-tic harmony dispositions are durable on disk — an attribution/observability defect, not a correctness one; but it inverts reader confidence precisely at the tics where the packet looks cleanest. Composes bk-braid-tic-clock-inheritance (DONE, /review 684) — this is the residue its INHERIT-arm choice left behind, not a re-litigation of that ratified cure (the divergence detector the other arm would have built was silently retired for every leg it used to cover). Cross-rays: federation ledger.md#presence-observation-fallacy-guard guard 6 (watcher-scope-must-be-declared) + the affirmative-health-verdict ray (green means clean-against-a-stated-scope) — this is the same duty landed on a single FIELD inside a multi-leg packet. FIRST-CONSUMER PATCH EXECUTED SAME PASS (feedback_patch-first-consumer-not-inscribe-around): braid-input-builder.py freshness check extended to the harmony leg with the by-design lag-1 DECLARED (expected_lag field + harmony_leg_stale flag only when lag exceeds it) + freshness_checked_legs enumeration welded beside honest_flags. Queue: status extracted → promoted (declared review_tic 736, adjudicated at the tic it named; docket-parked by the stepper's declared fence half the same tic). compact_root_status: ledger_only. lanes: [signal_and_queue_manifold, verification_and_proof_discipline]. Band: STRUCTURAL. lesson_type: disclosure_scope_correction. Confidence_tier: measured (12/12 tics deterministic lag + the two null-divergence tics both reproduced from artifacts on disk). signer ent_homeskillet-f5 (claude-fable-5), boot receipt e3c91131c1f7bedb. -->

**Refinement ray — the WRITE-TIME corollary: an active admission vocabulary is one contract with two faces — the author of a NEW inscription witness owes its head an admitted verb at authoring time, checked pre-fire in the same pass as the inscription (tic 737 born → /review 740 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, recommended option verbatim).** The parent obligates the emitter to match a reader predicate at emit time; the six prior rays govern the READER and its disclosures (dispatch-set closure, question-delta, open-vocabulary evolution, obligation scope, disclosure parity, per-leg freshness). This ray points the same contract at the WRITER as a STANDING PER-INSTANCE obligation under an ALREADY-ACTIVE vocabulary: whoever authors a new inscription witness (a provenance HTML comment) checks its HEAD against the instrument's declared admission window BEFORE the artifact lands. The read-side remedy classes (vocabulary_gap / head_anchor_gap / wrong_object_class) type defects AFTER they exist; the write-side check prevents the writer from minting the defect its own banked prediction forbids. The locus that makes this non-decorative: the seat that BANKS the checker's next-fire prediction is the same seat AUTHORING the comments that could break it — the writer is a watcher of its own prediction (composes the presence-observation guard family's declared-scope guard). Lived at tic 737: a ratified inscription's comment head ("direction-reversed ray from …") carried no admitted verb; caught and corrected PRE-FIRE, and the second fire then read unmatched 24 UNCHANGED with index +3 EXACT against the banked shape. Derivability disclosed per the via-negativa precedent: derivable-IN-STRUCTURE from the parent's emit-time sentence; the net-new is the standing-obligation FORM (the activation-order law cpr_new_gate_axis_wires_writers_first_b734 governs wiring order of a NEW demanded axis — distinct: this governs every instance authored under an ACTIVE one) plus the self-watching-writer locus. APO perimeter: NOT a new mechanical gate (adopted as standing /review practice + this ray; an absent pre-fire check refuses nothing by machinery); NOT the writers-first activation law; NOT a recommended-expression shape (it is an obligation on inscription witnesses specifically, not a style). Slice scope: one instrument (review-close-check's inscribed index), one lived near-miss (t737); the generalization to any fail-closed reader with a declared admission vocabulary is stated as the candidate form, held at this ray's altitude.

Lock line: *The admission vocabulary binds the writer at write time, not only the reader at fire time.*

<!-- promoted from cpr_inscription_witness_head_owes_admitted_verb_at_write_time_b737 (birth_tic 737, friction_born, cpr-extract-hook, explicit id; docket-parked by the stepper via docket_fence_derived_maturity_current [7th live fire, 737+3=740] and adjudicated at /review 740 — PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion question set round 1, recommended option verbatim). Source: audit-logs/governance/borns-tic737-an-inscription-witness-head-owes-the-admitted-vocabulary-at-write-time.md. SEVENTH ray on this entry, distinct axis from all six (writer-obligation vs reader/disclosure axes — no merge; the row's own review_hints named the writers-first entailment check and it held: adjacent, distinct). Gate-2 context consumed: A6-740 (the named home at cgg-ledger ch.3 obligates writers to STAMP a provenance comment, not that the stamp's head be vocabulary-conformant — residual genuine). Relations from mint held: refines the /review-736 remedy-class procedural home (the write-side face of the same vocabulary); sibling cpr_new_gate_axis_wires_writers_first_b734 (adjacent, distinct: activation order vs standing authoring obligation); composes presence-observation guard 6 (declared watcher scope). PRACTICE ADOPTED SAME PASS: pre-fire head check on every new provenance comment authored at /review — this pass's own two inscription comments are the first conforming instances, verified against the live _PROVENANCE_VERB_RE + relaxation before commit. compact_root_status: ledger_only. lanes: [verification_and_proof_discipline, measurement-integrity]. Band: COGNITIVE. lesson_type: contract_two_faces. confidence_tier: tentative (n=1 near-miss). signer ent_homeskillet-f5 (claude-fable-5), boot receipt f1263aad9a149c87. -->

**Refinement ray — the MEMBERSHIP corollary: a disclosure that ships a divergence-route CATALOG while persisting only the MAGNITUDE of each counter cannot discriminate its own catalog — it hands attribution back to the hand-work its mechanization was minted to retire; persist the SET (or a sorted digest) so the delta is a set difference and the moved ids are NAMED, sign-partition the routes by polarity, and type a zero/zero agreement as VACUOUS (tic 745 → /review 748, `cpr_mogul_review_close_check_64cb21a969f9`; PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 1, recommended option verbatim).** The OBLIGATION-SCOPE ray above (the /review-728 mint `c209995ad848`) landed `cross_counter_disclosure` because "each /review recomputed the agreement by hand" (review-close-check.py:2692-2693). It mechanized the COMPARISON — `agree: true|false` — and shipped four named divergence routes. It did not mechanize the ATTRIBUTION, and as persisted it cannot: `delta_tokens = current_tokens - prior_tokens` (:2603) is integer subtraction against the prior artifact's `inscribed_index_size`, and no close-check artifact persists the token set (the largest list in tic-744, tic-747 and tic-748-check.json is the 25-item `unresolved_against_queue_sample`; the run holds the full ~815-token set in memory to count it, then discards it). Lived at 745: agree=false with promoted_delta 0 / token_delta 1 and all four routes printed; attributing the divergence to route 4 (`absorbed_reinforcement_breadcrumb_adds_token_without_promotion`) and witnessing the moved token as `cpr_mogul_economy_heartbeat_64367ac313d3` at constitution-ledger/ledger.md:5008 took a hand grep by the Mogul seat — the machine had already computed and discarded the set that answers it. SECOND FACE, needing no membership: the catalog is POLARITY-BLIND — `modify_and_merge_promotion_adds_no_provenance_comment` pushes promoted_delta ABOVE token_delta and therefore cannot be a SOLE explanation of a token-side excess, yet it is printed unchanged beside one at 743 (4 vs 6) and 745 (0 vs 1); the script sign-partitions nothing. THIRD FACE, observed the morning of this ruling: at the tic-748 ENTRY fire the field emitted agree=true from 0 == 0 — a tautology that corroborates nothing, typed identically to the real corroboration at 747 (5 == 5) — `cpr_mogul_review_close_check_45c12f41fc1e` (b748, fence 751; noted forward, adjudicated at its own fence). Cure, bounded and forward-only: persist the token set (or sorted digest) in the artifact so from the next pass the delta is a true SET DIFFERENCE and the moved ids are named — collapsing the route catalog from four hypotheses to one witness with no re-derivation of the prior index; partition the routes by the sign of the divergence they can produce; emit `agree_vacuous: true` when both deltas are zero. Apophatic perimeter from the mint: not a genuine-consistency finding (genuine=0; the checker is clean); not a claim about queue-side verdict counting (the promoted counter is correct); not the t724 over-admission / disposition-split lane (bk-close-check-counter-disposition-split stays its own row). Relations held: refines `c209995ad848` (the OBLIGATION-SCOPE ray above); sibling `0ddacf1f8896` (the t743 TWO-COUNTER face, absorbed /review 743); sibling `cpr_a_statistic_without_its_membership_set_…_b745` (the federation principle this row instances — promoted the same pass on constitution-ledger#artifact-language-must-not-exceed-its-declared-confidence-classification, the RE-DERIVABILITY axis; an ORDERING relation, ruled not-twins at t746 by the mint-time edge); distinct_from `…_b730` (how a prediction is BANKED vs what the artifact PERSISTS). First consumer: `bk-close-check-unresolved-membership-persisted` WIDENED to both checker membership sets + polarity-partitioned routes + vacuous typing — one fence (review-close-check.py + its check artifact schema), one instrument. Dual clamp: cost_of_action one bounded field (a sorted list of ~815 short strings, or its digest) + a set difference at delta time + a sign table; the benefit arrives the pass AFTER the first persisted set; cost_of_inaction every future agree=false reproduces the tic-745 hand grep and the route catalog stays a hypothesis set a reader may mistake for an attribution. Slice: counters that ship a divergence-route catalog (observed on cross_counter_disclosure across 743/744/745/747/748), not delta counters generally.

<!-- promoted from cpr_mogul_review_close_check_64cb21a969f9 (birth_tic 745 → /review 748 PROMOTE-as-refinement-ray, Architect-ratified in-tic via AskUserQuestion question set round 1, recommended option verbatim — the FOURTEENTH consecutive tic; confidence measured). Source: mogul:review_close_check (mogul_mandate_id tic-745-20260828T075734; source report audit-logs/mogul/cycle-reports/reports/2026-08-28T035735-tic-745.report.json; no born file — the queue-row lesson is the authoritative body, inscribed near-verbatim above with the 747/748 faces added). Every mechanical claim RE-DERIVED by the orchestrator at adjudication by direct read: :2603 subtraction, :2692-2693 docstring, :2747-2755 route catalog, persisted-list census at 744/747/748, the cross_counter_disclosure blocks at 743 (4v6 false) / 745 (0v1 false) / 747 (5v5 true) / 748 entry (0v0 true), no polarity handling in the script. The row's recommended_scopes named the script path and its note left the ledger anchor to /review — ruled HERE, the lineage parent of the /review-728 mint it refines; EIGHTH ray on this entry. Queue: status extracted → promoted through queue-lifecycle-writeback.py (split stamp). compact_root_status: ledger_only. lanes: [signal_and_queue_manifold, verification_and_proof_discipline, measurement-integrity]. Band: STRUCTURAL. lesson_type: mechanism_refinement. Confidence_tier: measured. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 0ecbc8c5a4340914. -->

**Refinement ray — the MIRROR-FACE corollary: a reader predicate BROADER than the emitter's claim drains its own finding class for reasons external to the rows it counts — split the predicate from the claim: narrow the message to the scanned set and type the match by ROUTE, so a mention-cleared finding is reported as reclassified-by-sibling-narration, never as verification progress (tic 748 born → /review 751 PROMOTE-as-refinement-ray + first-consumer patch, Architect-ratified in-tic AskUserQuestion round 1 Q4, recommended option verbatim; confidence measured).** The parent obligates the emitter's rows to match a reader predicate; the earlier rays close the reader's dispatch set, its per-tic question, its vocabulary evolution, its obligation scope, its disclosure parity, its per-leg freshness, its write-time face and its membership. This ray names the MIRROR: a reader whose PREDICATE is broader than its CLAIM. review-close-check.py's orphaned_promotion finding does both halves at once — its message reads "marked promoted in queue but text not found in any governance file" while the predicate scans exactly three fixed paths (CLAUDE.md, constitution-ledger/ledger.md, ~/.claude/CLAUDE.md, plus the row's own promoted_to and path-shaped recommended_scopes), and it clears on `cpr_id in content or cpr_ref in content or snippet in content` anywhere in that bounded set, with no requirement that the match be the promotion's own LANDING. Lived at tic 748: the b730 units ray's orphan finding cleared between the ENTRY fire (known 17) and the CLOSE fire (known 16) although its promotion neither landed nor changed — a SIBLING ray (the b745 re-derivability anchor, inscribed at /review 748) named b730's full cpr_id in a `distinct_from:` relation edge, and that third-party mention satisfied the predicate. Verified by arm: the exact cpr_id is present in ledger.md (True); the loose CogPR-730 ref is not (False); the 50-char lesson snippet is not (False). By tic 751 a SECOND third-party mention exists (the 750 SAMPLING-INSTANT clause's `refines` relation) — the drain the row predicts, in progress. The forward hazard is structural, not incidental: the six-facet strike form asks every mint to carry `relations` (sibling:/refines:/composes:/distinct_from:) and `apophatic_exclusions` naming nearest excluded neighbors — so as rays increasingly name sibling ids in doctrine prose, this predicate drains the orphan class monotonically, and the checker's most universe-shaped finding type decays toward zero for reasons that have nothing to do with promotions landing. CURE: the message names the scanned set; the match carries a route (landing_at_declared_target | third_party_mention); a mention-cleared orphan is reported as reclassified, never silently vanished from the count. Composes the federation's artifact-language invariant (the "any governance file" message is language exceeding the predicate's actual class) and the presence/observation guard (a mention PRESENT is not a promotion LANDED).

Lock line: *A reader that claims a universe it does not scan, and clears on a mention it does not type, retires findings for reasons the rows never caused — name the set, type the route.*

<!-- promoted from cpr_mogul_review_close_check_7fb07b4e6323 (birth_tic 748 → /review 751 PROMOTE-as-refinement-ray + first-consumer patch in the tic-751 CGG batch (route-typed match + narrowed message in check_orphans — the row named the function find_orphaned_promotions; the finding type is orphaned_promotion), Architect-ratified in-tic via AskUserQuestion question set round 1 Q4, recommended option verbatim — the SEVENTEENTH consecutive tic; confidence measured). Source: mogul:review_close_check (mogul_mandate_id tic-748-20260828T221408; source report audit-logs/mogul/cycle-reports/reports/2026-08-28T181409-tic-748.report.json; no born file — the queue-row lesson is the authoritative body, inscribed near-verbatim above). NINTH ray on this entry, on its own axis (the reader's CLAIM vs its PREDICATE — assessed no merge with the prior rays; the seat's first draft mis-counted the rays as six and was corrected by measuring the entry before writing). Relations held from mint: sibling bk-review-close-check-provenance-axis-patch (match-axis direction; a done row); composes ledger.md#artifact-language-must-not-exceed-its-declared-confidence-classification and ledger.md#presence-observation-fallacy-guard; distinct_from the b745 re-derivability anchor (attribution DIRECTION, not re-derivability) and bk-close-check-same-tic-reobservation-delta-decomposition (the two-observation split within one tic). Apophatic exclusions held from mint: NOT a claim the b730 clear was wrong; NOT a claim genuine_count is compromised (the streak stands); NOT a claim the cross-counter mis-fired (it held 3==3 — a prose mention is not a provenance-comment witness); NOT a demand that relation edges stop naming sibling ids. Re-derived EXACT by the lead at 751 (:1599-1610, :1647, :1657; the b730 arms True/False/False; the second mention at constitution-ledger :3455). signer ent_homeskillet-f5 (claude-fable-5), boot receipt 68ba9ebda60729c1. -->
...
**Refinement ray — the READER-SIDE corollary: source AUTHORITY and predicate CURRENCY are ORTHOGONAL checks; a retired predicate applied to the authoritative source still lies (tic 680 born → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set).** Reading the authoritative manifest is NECESSARY but NOT SUFFICIENT — a RETIRED predicate applied to the correct source still lies, and it lies quietly because both numbers look plausible. Lived at tic 680: the conformation artifact carries two counters over the same substrate, and they disagree by 2. counts.active_signals = 57, computed with the shared is_active_ray predicate (structurally live, or carried/dimmed with heat above floor). governance_query_enrichment.manifold_summary.active = 55, computed by filtering on the RAW STATUS ENUM — the predicate the federation has explicitly RETIRED. The delta is fully accounted with zero residue: exactly the 2 rays whose raw status is 'acknowledged' (sig_2026-04-05_vpl_composite_rollback_gap, sig_economy_economy_fetch_failed_84fb94e38d7a), both structurally live. This refines the existing authoritative-set-readers invariant, which was written against readers that aggregate RAW EMISSIONS instead of the curated manifest. Here BOTH readers read the curated manifest correctly and still disagree — the divergence is in the PREDICATE, one layer below sourcing. So predicate retirement carries the same closed-consumer-set obligation that a structural transform carries: retiring a predicate silently obligates an update on every reader that still applies it, and a reader inside the SAME artifact as the correct one is the easiest to miss precisely because the artifact looks internally authoritative. Disagreement-as-evidence held: the 2-count gap was the visible symptom of a stale predicate, not a counting bug to arbitrate.

<!-- promoted from `cpr_mogul_signal_scan + deep_audit_4b0fefebb476` (birth_tic 680 → /review 683 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set). Source: mogul:signal_scan + deep_audit (mogul_mandate_id tic-680; no born file — queue-row lesson is the authoritative body). REFINES this entry with the READER-SIDE half: the parent names the SWEEP obligation on the retiring actor; this ray names the CHECK obligation on every reader — source AUTHORITY (am I reading the curated manifest?) and predicate CURRENCY (is the predicate I apply to it still the live one?) are ORTHOGONAL, and passing the first says nothing about the second. Sharpens federation authoritative-set-readers-must-read-the-manifest (written against raw-emission aggregation; here BOTH readers read the manifest correctly and still disagree) and composes federation Disagreement-as-evidence. RECURRED UNCHANGED at t683 (cpr-stepper finding F5) — same two acknowledged-but-live rays, same 2-count gap, one tic later: the divergence is standing, not transient. FIX-SITE FILED, NOT EXECUTED HERE: the conformation emitter's adoption of lib/signal_active.py is a backlog row filed by the orchestrator; this pass inscribes the law only. compact_root_status: ledger_only. Band: COGNITIVE. lesson_type: invariant_refinement. Confidence: 0.85. signer ent_review_execute (claude-opus-5[1m]), boot receipt a4a5b5661049ce2b. -->
...
## A Bare Invocation Is an ACTION, Not a Probe — a `x sub || x` Fallback Converts Read Intent Into the Tool's Default Mutation
<a id="bare-invocation-is-an-action-not-a-probe"></a>
<!-- ledger-tags: authority_class=mandate_and_cadence_ops | rung=domain | domain=context-grapple-gun | born_tic=605 | promoted_tic=608 | refines=syntax-semantic-collapse-agent-environments(federation) | composes=footgun-guard-at-perception-layer-warns-after-the-footgun-already-fired,wrapper-must-discriminate-instrument-exit-code-semantics-crash-vs-verdict | confidence_tier=tentative -->
...
## A Same-Tic Replay Lawfully Diverges From the Series Row — Consumers Pinning Exact Numerics Must DECLARE Their Source
<a id="same-tic-replay-diverges-consumers-declare-source"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=605 | promoted_tic=608 | refines=authoritative-count-discipline | composes=disagreement-as-evidence(federation),artifact-language-must-not-exceed-declared-confidence(federation) | confidence_tier=tentative -->

A per-tic heartbeat re-dispatched within the same tic (e.g. after an in-tic incident revert) runs as a series-guarded REPLAY: its recomputed values (g_t=0.762982, mint_total=21990.20 at tic 605) legitimately diverge from the authoritative series row (g_t=0.749105, mint_total=40360.87) because the motion-engine state moved between runs. The anti-clobber design is correct — the replay snapshot lands beside the series row, the pointer stays on the series — but any consumer contract that pins exact numeric fields (a mandate schema demanding g_t/mint_total, a dashboard, a verifier) must DECLARE which source it reports (series row vs invocation/replay record), else the same tic carries two 'true' values and readers silently disagree. The net-new residue beyond disagreement-as-evidence: BOTH values are lawful and CURRENT (neither is stale state to sweep) — the cure is source-declaration in the consumer contract, not manifest-pruning. Family: disagreement-as-evidence / authoritative-set-readers, on the same-tic dual-lawful-value axis. Distinct from cpr_mogul_economy_heartbeat_bd0c499910c6 (exit-code breach encoding — a different mechanism).

<!-- promoted from cpr_mogul_economy_heartbeat_ae4c01af1180 (tic 605→608, /review 608 PROMOTE, Architect-ratified in-tic via AskUserQuestion ratification set). Enrichment baseline: engineering/cgg-ledger (deterministic-lite). Evidence: observed live at tic 605 after the receipted accidental-downbeat revert forced a same-tic re-dispatch; both records preserved side-by-side by the series guard. Band: STRUCTURAL. Confidence_tier: tentative (n=1 lived). signer ent_homeskillet (claude-fable-5), boot receipt 7a9316aa5895dd25. -->

**Refinement ray — a chained sub-step that resolves its tic from a DIFFERENT authority than its parent makes the CHAIN's tic identity ORDER-DEPENDENT within the tic (tic 680 born → /review 684 MODIFY+PROMOTE-as-ray, Architect-ratified in-tic question set; the born's ALWAYS clause STRUCK at ratification — see provenance).** When one lane chains a sub-step, the sub-step must not resolve the tic from a DIFFERENT authority than its parent, or the chain's own tic identity stops being a property of the tic and becomes a property of EXECUTION ORDER. Lived at the harmony→braid chain: `harmony-invoke.sh` chains `braid-invoke.sh`, but the two input builders resolve the tic from different sources — harmony-input-builder.py reads conformation `tic_count_physical` (:133), while braid-input-builder.py reads the ECONOMY current-pointer as PRIMARY authority (:131-133, `tic = pointer.get("tic")`) with conformation only as an `if tic is None` FALLBACK (:145-146). The MECHANISM is real and was re-verified INTACT at the named locus at both tic 683 and tic 684. What it produces is ORDER-DEPENDENCE, not a constant lag: lag 1 when braid reads before the economy pointer advances (the cadence cycle order runs harmony_invoke at position 3, ahead of economy_heartbeat at position 10), and lag 0 whenever wall-clock ordering lands the economy write first — observed live at tic 683 (economy 23:33:46Z, braid 23:33:51Z, both resolving 683) and again at tic 684 (economy 16:11:05Z, braid 16:11:12Z, both resolving 684). The cure is therefore CLOCK INHERITANCE — the parent passes its resolved tic down to the sub-step — or a FIRST-CLASS DIVERGENCE DECLARATION, the sub-step emitting its own resolved clock as an output field so the divergence is visible rather than silent. It is NEVER a constant offset: "add 1" / "treat braid packets as uniformly lag-1" is wrong on exactly the lag-0 tics and would silently mis-stamp them. Corollary retained from the born: a fallback source that is ALWAYS reachable never exercises the fallback, so the "safe" branch never runs. Ray on this entry along the INTRA-TIC ORDERING axis — the parent governs two lawful VALUES for one tic arising from a REPLAY (a re-dispatch), cured by source-declaration in the consumer contract; this ray governs two lawful tic IDENTITIES arising from ORDERING WITHIN a single pass, cured at the producer by clock inheritance or divergence declaration.

<!-- promoted from "cpr_mogul_harmony_invoke + deep_audit_f4de0ac773fb" (birth_tic 680, advanced_tic 683 → /review 684 MODIFY+PROMOTE-as-ray, Architect-ratified in-tic question set). Source: mogul:harmony_invoke + deep_audit (mogul_mandate_id tic-680-20260805T113323; no born file — the queue-row lesson is the authoritative body). MODIFY APPLIED AT RATIFICATION: the row's ALWAYS clause — "braid resolves tic N-1 on EVERY cadence run", asserted as verified "consistently across tics 677/678/679/680" — is STRUCK as FALSIFIED by its corrector cpr_mogul_memory_mining_e5fa86d6ec02 (birth 681), admitted BEFORE any fix authorization. That sequence is the exact law the /review 683 anchored DEFER established; Pair A (6540e0503eaa correcting 79ae89ca3a0a at /review 683) is the sibling precedent. Only the honest scope is inscribed above: the mechanism (re-verified intact — braid-input-builder.py:131-133 pointer-primary with :145-146 conformation fallback; harmony-input-builder.py:133 conformation tic_count_physical) and the ORDER-DEPENDENCE it actually produces. EVIDENCE for the strike: lag 0 measured twice — t683 (economy current-pointer generated_at 2026-08-06T23:33:46Z carrying tic 683; braid 23:33:51Z carrying tic 683; cpr-stepper finding F2) and t684 (economy-invoke 2026-08-07T16:11:05Z, braid 16:11:12Z; t684 Mogul mandate report cross_cycle_observations key "braid_economy_lag_zero_second_consecutive"). RUNTIME FIX NOT AUTHORIZED BLIND — filed by the orchestrator as backlog row bk-braid-tic-clock-inheritance; this pass inscribes doctrine only and touches no runtime. Queue: status enrichment_eligible → promoted; maturity_window_tics 1 anchored on advanced_tic 683, window returned at 684; pending_class evidence_insufficient cleared and the typed re_eval_condition DISCHARGED — its named dependency (corrector e5fa86d6ec02 matures at 684 and the pair adjudicates JOINTLY) is MET, the corrector having been adjudicated FIRST and PROMOTED this same pass at cgg-ledger/ledger.md#the-corrector-inherits-the-verification-obligation. compact_root_status: ledger_only. Band: STRUCTURAL. lesson_type: runtime_defect_with_named_cure. Confidence: 0.85. signer ent_review_execute (claude-opus-5[1m]), boot receipt 92a60d0448ce0ae8. -->

---

## The Worldview Renderer and the Memory Lane Are DISTINCT Inscription Lanes — Choose by Reach and Trigger, Not by Nearest Surface
<a id="worldview-renderer-distinct-inscription-lane-from-memory"></a>
<!-- ledger-tags: authority_class=memory_and_inscription_hygiene | rung=domain | domain=context-grapple-gun | born_tic=493 | promoted_tic=608 | composes=boot-seam-duality-primary-sessionstart-citizens-subagentstart,budget-exempt-closure-framing-and-unit-safe-truncation | confidence_tier=tentative -->

Orientation/posture that must steady EVERY boot — regardless of model generation or whether a session is "the same model" — belongs in the worldview RENDERER (office-worldview.py: a budget-exempt block reaching BOTH boot seams via the shared is_citizen branch), NOT the memory lane. Memory ≠ worldview prose: they are distinct inscription lanes with distinct REACH and TRIGGER. Memory is per-namespace continuity — read/trust/add-to, loaded when relevant, namespace-scoped; it does NOT fire at boot for all citizens. The boot worldview prose is the UNIVERSAL frame that fires every tic for every citizen + the orchestrator BY CONSTRUCTION (the shared renderer both seams call). So when the directive is "put this frame in the boot, across citizen and orchestrator," a memory write is necessary-but-INSUFFICIENT — the renderer edit is the actual deliverable, and the budget-exempt placement (sibling to THE LADDER) is what keeps it from being clipped by the --max-chars body budget. Discriminator before acting: does this need to fire for everyone every boot (→ renderer, budget-exempt, both seams), or is it continuity for one namespace to rehydrate when relevant (→ memory)? The federation has multiple inscription lanes (compact-root, ledger bodies, memory, boot worldview prose, spec files); they are not interchangeable — the lane is chosen by WHO must receive it and WHEN, not by which surface is closest to hand. Correction cost was one round: a memory file was written first; the Architect corrected the lane ("memory is not the same as the worldview prose i want updated"); the fix was render_identity_frame() synced byte-parity, surviving --max-chars 2600, verified on the installed primary-seam binary.

<!-- promoted from cpr_worldview_renderer_is_a_distinct_inscription_lane_from_memory_tic493 (tic 493→608, /review 608 PROMOTE, Architect-ratified in-tic via AskUserQuestion ratification set). Body recovered from audit-logs/governance/borns-tic493-worldview-renderer-distinct-from-memory-lane.md at the 608 pass (queue row was name-only, deferred pending body-fill). Net-new per the born's own non-derivability note: the explicit LANE DISTINCTION (reach × trigger) beyond boot-seam-duality + budget-exempt-framing + memory-is-an-index. Evidence: tic-493 Architect live correction + CGG 888b749 (render_identity_frame, dual-seam, byte-parity verified). Band: COGNITIVE. Confidence_tier: tentative (n=1 lived, Architect-corrected). signer ent_homeskillet (claude-fable-5), boot receipt 7a9316aa5895dd25. -->

---

## Admission Is Orthogonal to Runtime-Authorization at /review — Conflating Them Produces Over-Conservative DEFER
<a id="admission-orthogonal-to-runtime-authorization"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=500 | promoted_tic=608 | refines=promotion-scope-discipline | composes=build-and-gate-ratified-flag-gated-consumer,fixture-green-is-not-trainer-green | confidence_tier=reinforced -->

At /review, the admission decision (is the content valid? what CLASS of doctrine/evidence does it belong to?) is ORTHOGONAL to the runtime-authorization decision (may it WIRE into the live runtime?). Conflating them produces over-conservative DEFER: a refinement/evidence lesson gets parked because its RUNTIME WIRING is blocked, even though the lesson is preservable NOW and the wiring is a separate, separately-gated question. Two concrete shapes of the conflation, both caught at tic 500 by an Architect counter-strike that OVERRODE a reviewer's DEFER×4: (1) EVIDENCE-VS-AUTHORIZATION — a cross-model measurement lesson deferred because live coupling was un-rung, though the measurement itself was admissible evidence; (2) REFINEMENT-VS-WIRING — a refinement ray parked behind an unrelated runtime gate. The discipline: admit what is admissible at its true class NOW; let the wiring question ride its own gate (build-and-gate's ratified-bit is the mechanized form: admission of the mechanism, authorization as a separate human-gated flag-flip). Structural conformations across rungs since birth: the SP2 gate returns ADMIT_PROPOSAL_REFUSE_MOVE (admission and movement authority separated by construction); ladder-feedback-push is registered-but-dormant with /review flipping ratified (admission landed, authorization gated); the C3/C4 hold discipline (admissibility judged while wiring stays held). This is the third face of Promotion Scope Discipline's "two decisions, not one" — validity, class, AND wiring are three separable questions.

<!-- promoted from cpr_review_admission_not_conflated_with_runtime_authorization_tic500 (tic 500→608, /review 608 PROMOTE, Architect-ratified in-tic via AskUserQuestion ratification set). The tic-500 defer condition ("gather 2nd conformation") discharged by cross-rung structural conformations: SP2 ADMIT_PROPOSAL_REFUSE_MOVE + build-and-gate ratified-bit + C3/C4 hold discipline. Refinement ray on Promotion Scope Discipline per the born's recommended scope. Band: COGNITIVE. Confidence_tier: reinforced (Architect counter-strike + 3 structural conformations). signer ent_homeskillet (claude-fable-5), boot receipt 7a9316aa5895dd25. -->

---

## Ratification Freezes Embedded Volatile State — the Ratify-Time Volatile-Sweep Is the Missing Intake Tooth
<a id="ratification-freezes-embedded-volatile-state-ratify-time-volatile-sweep"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=607 | promoted_tic=611 | refines=volatility-handling-timestamps-ttl(federation) | composes=single-owner-discipline,spec-runtime-alignment-by-accident-is-structural-drift(federation) | confidence_tier=tentative -->

When a data surface is ratified as an authoritative baseline (the 19-office civic orientation office-lanes.json, RATIFIED /review 427 as "authoritative live boot terrain"), the ratification freezes not only the intended orientation content but every VOLATILE value embedded in it at ratification time — computed-per-tic gate lines, due markers, counts. Those copies then age silently UNDER THE RATIFICATION'S AUTHORITY: four offices (ent_mogul, ent_crisis_steward, ent_cbux_steward, ent_egress_router) carried a baked "/review 427 due tic 427" active_arcs entry for ~180 tics while the live obligation line was computed per-tic by office-worldview.py from tic_context; any of those offices booting via its own worldview would have rendered a 180-tic-stale obligation as a live YOURS-act lane. The cure has a LOCUS: at RATIFICATION INTAKE, sweep the candidate surface for embedded volatile values and either strip them (when a computed producer owns the value — single-owner discipline: if a value is rendered dynamically anywhere, its static copies elsewhere are latent contradictions, not documentation) or stamp them with last-verified/TTL per the Volatility-Handling law. Ratification-review checks orientation CONTENT; nothing checked for volatile-state stowaways — the ratify-time volatile-sweep is the missing intake tooth, the Case-2 wire-the-check-at-the-locus move rather than new law.

<!-- promoted from cpr_ratified_orientation_baselines_freeze_embedded_volatile_state_tic607 (tic 607→611, /review 611 PROMOTE-as-refinement, Architect-ratified in-tic via AskUserQuestion ratification set). Refinement ray under the federation Volatility-Handling law (timestamps/TTL), NOT new law by the lesson's own classification (Case-2 wire-the-check-at-the-locus). Enrichment baseline: engineering/cgg-ledger (deterministic-lite, proof source_stable). Evidence: the ~180-tic stale active_arcs cohort found and cured at tic 607 (born borns-tic607-ratified-baselines-freeze-embedded-volatile-state.md); mechanization of the intake sweep registered as backlog bk-ratify-intake-volatile-sweep. Band: COGNITIVE. Confidence_tier: tentative (n=1 lived cohort, 4 offices). signer ent_homeskillet (claude-fable-5), boot receipt 4cc70b4b94c32189. -->

---

## During a Receipted Refinement Window the Queue Rules STATUS and the Source File Rules BODY — Id-Dedup at the Write Boundary Cannot Carry a Pending-Entry Body Refinement
<a id="queue-rules-status-source-file-rules-body-refinement-window"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=611 | promoted_tic=614 | composes=ratification-freezes-embedded-volatile-state,review-execute-atomic-writeback-completeness | confidence_tier=tentative -->
...
## A Close/Consistency Receipt Is Only as Strong as Its Artifact Verification — Not Tautological, Not Prose-Relabeled, Not a Mounted-Bear Helper
<a id="close-consistency-receipt-only-as-strong-as-artifact-verification"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=635 | promoted_tic=638 | composes=presence-observation-fallacy-guard(federation),can-it-eat-dataflow-liveness-predicate(federation),reason-coded-genuine-vs-known-verifier-split | confidence_tier=tentative -->

A review-close / consistency / activation receipt is only as strong as the ARTIFACT VERIFICATION behind it — and the exact failure modes are: (1) HIDING a checker's real result — piping an authoritative verifier through `| tail` without `set -o pipefail`, or relabeling its `consistent:false` as GREEN from a prose line ("genuine=0") without proving from the checker's CODE that the state is closure-safe (its exit code may be unconditional; the closure contract lives in the report fields — genuine==0 AND universe_classified AND known==total — not the rc); (2) TAUTOLOGIES masquerading as checks — `len(hardcoded_dict)==2`, `17+8+1==26`, receipt-existence-as-validity, `promoted_to` non-emptiness as anchor RESOLUTION, `pending_class` as a durable landing, bodies checked for only one object class; (3) a MOUNTED BEAR verification helper — a function that only CONSTRUCTS A DICT and asserts a field is nonblank is not a round-trip: it never drives the real writer, appends to a temp surface, rematerializes, or asserts lineage/version/hash. The cure is structural: run the authoritative checker fail-loud and read its closure contract from source; derive the cohort identity set from the frozen input and prove exact set-equality against final dispositions; parse every receipt for full SHA256 + authority namespace + recomputed hashes + source-pointer currency; enumerate the exact appended events (not "N/N checks"); prove every credential RESOLVES to a real surface; materialize every parent AND split child. Harden the close BEFORE commits make the weak proof canonical.

<!-- promoted from cpr_close_receipt_must_be_artifact_verified_not_tautological_or_prose_relabeled_tic635 (birth_tic 635, friction_born — Architect corrections to the /review-635 end-to-end execution), promoted at /review 638. confidence 0.80. Source: audit-logs/governance/borns-tic635-review-execution-discipline.md. Architect-ratified in-tic via AskUserQuestion question set (reviewer ent_homeskillet). -->

**Refinement ray — a gate bundle must be independently replayable from the bundle alone (tic 636 born → /review 639 merged).** A merge/verification gate bundle must be independently replayable from the bundle alone: per-command receipts carrying command, cwd, exact HEAD, TRUE return code, log sha256, and CI run reference — with the raw logs preserved durably, never only in tmp/scratchpad. Two concrete traps this session: (1) 'cmd 2>&1 | tail; echo rc=$?' records TAIL's rc, not the command's — capture rc from the command itself (redirect to file, then echo $?); (2) a hash-only manifest whose referenced logs live in /private/tmp is a pointer to evidence that dies with the session. Synthetic fixtures in a bundle must be explicitly labeled synthetic (world-kind honesty) or they read as live execution evidence downstream. Refines this entry by naming the bundle-level replayability requirement and the two concrete traps (tail-rc, hash-only-manifest-to-tmp) as failure modes of artifact verification. Composes feedback_no-tmp-for-load-bearing-artifacts + this entry's own producer-seal typed aperture. Not a new parent.

<!-- MERGED tic 639 from cpr_gate_evidence_must_be_self_replayable_tic636 (birth_tic 636, /review 639, MERGE-as-refinement-ray, Architect-ratified in-tic via AskUserQuestion question set, reviewer ent_homeskillet). Source: audit-logs/governance/borns-tic636-merge-gate-recovery.md. tic-636 session: initial csl clippy 'rc=0' masked a real (out-of-native-gate) compile error via the tail-rc bug -- all gates re-run with true rc capture; Architect verification flagged the 10-hash+3-stray-line manifest and tmp-only logs; replaced by gate-receipts.json (36 receipts) + logs/ (33 raw logs committed) + FIXTURES.md at canonical acf91671. Composes feedback_no-tmp-for-load-bearing-artifacts + cgg-ledger producer-seal typed aperture. Band: COGNITIVE. Confidence_tier: tentative. -->

---

## Inscribe BEFORE the Queue Write That Cites Its Anchor — a Placeholder Anchor Is a Dead Execution Credential
<a id="inscribe-before-the-queue-write-that-cites-its-anchor"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=635 | promoted_tic=638 | composes=id-form-divergence-voids-cross-surface-writeback,review-execute-atomic-writeback-completeness | confidence_tier=tentative -->

A promotion has TWO coupled writes — the doctrine-body inscription (which mints the anchor slug) and the queue/source-surface credential (`promoted_to: surface#anchor`). If the queue write is emitted BEFORE the inscription, its anchor is a PLACEHOLDER the author guessed, and the inscriber almost never uses the guessed slug — so the credential points at a non-existent anchor: a dead execution credential, the exact credential-drift class review-close exists to catch. Sequence inscription FIRST (it produces the real anchor), then write the credential; OR, if the queue write must precede (e.g. to drive a physics-gated writeback), treat the anchor as provisional and run a mandatory reconciliation pass afterward — re-emit the credential (and every dual-surface stamp the writeback produced: born files, inline markers) to the REAL inscribed anchor, then prove every final credential resolves. A dual-surface promote-writeback fired by the queue write will stamp the source surfaces with whatever placeholder was in the queue row; those stamps are idempotent on status but NOT self-correcting on the anchor, so they need direct correction too. Verify anchor RESOLUTION with alphanumeric-normalized matching (hyphen/space regex fragility produces false-negatives that waste passes).

<!-- promoted from cpr_inscribe_before_the_queue_write_that_cites_its_anchor_tic635 (birth_tic 635, friction_born — Architect corrections to the /review-635 end-to-end execution), promoted at /review 638. confidence 0.80. Source: audit-logs/governance/borns-tic635-review-execution-discipline.md. Architect-ratified in-tic via AskUserQuestion question set (reviewer ent_homeskillet). This entry's own ordering law honored: inscribed BEFORE the queue credential that cites this anchor. -->

---

## A Build Script Embedding VCS Identity Must Register VCS State Files as Rerun-Trigger Inputs
<a id="build-script-vcs-identity-requires-rerun-keys"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=636 | promoted_tic=639 | confidence_tier=tentative -->

A build script that embeds VCS identity (source commit) into an artifact must register the VCS state files themselves — Git HEAD, the symbolic-ref target file, and packed-refs — as rebuild-trigger inputs (cargo rerun-if-changed or equivalent), or incremental builds silently ship stale identity across merges while looking current. The embedded identity must also be TEST-asserted equal to live rev-parse HEAD, and the incremental case proven (build -> move HEAD -> rebuild -> identity followed). Discovering HEAD at build time without watching it is the exact half-implementation that passes every clean-build check and fails only incrementally.

<!-- promoted from cpr_build_script_vcs_identity_requires_rerun_keys_tic636 (tic 636->639). Source: audit-logs/governance/borns-tic636-merge-gate-recovery.md. Architect verification of the tic-636 gate bundle: sealed.json carried @c32de90 while merged csl main was 046c46a (identity correct for its worktree build, but the law gap real: csl splat-cli/build.rs discovered HEAD with zero rerun keys while canonical-mount's build.rs carried the full law). Fixed csl 2fa5266 with tests/runtime_identity.rs; incremental proof: green at 046c46a -> empty commit a287c66 -> green on incremental rebuild. Architect-ratified in-tic via AskUserQuestion question set this session (reviewer ent_homeskillet), /review 639. Band: COGNITIVE. Confidence_tier: tentative. -->

---

## A Handoff That Projects a /review Docket Must DERIVE Adjudicability From the Queue's Maturity Law, Never From Born Existence or Counts
<a id="handoff-docket-projection-must-derive-from-maturity-law"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=637 | promoted_tic=640 | relations=refinement-ray:conductor-score-runtime-parity(federation),sibling:gate-example-template-must-teach-complete-pass-state,sibling:close-consistency-receipt-only-as-strong-as-artifact-verification,composes:inscribe-before-the-queue-write-that-cites-its-anchor,refined-by:maturity-scope-fence-tic679 | confidence_tier=tentative -->

A handoff that projects a /review docket ('N borns + M pending are adjudicable at entry') must DERIVE decision-readiness from the queue's maturity law (birth_tic + maturity_tics vs entry tic, per-entry), never from born existence or counts. Borns extracted at tic W are temporally immature until W+maturity_tics (friction_born standing = 3); a handoff written at tic-W close that books them for /review at W+1 books work the queue physics will refuse — the honest entry-review is then a hold-pass, and the next session should not read that as breakage or a missed obligation. The projection formula is mechanical: adjudicable-at-E = entries with status decision-ready AND (E - birth_tic) >= maturity_tics. A ray on Conductor-Score-Runtime Parity (the score/handoff names a docket the runtime gate does not admit) — likely refinement, not net-new law.

**Scope-fence ray (tic 676→679):** the maturity law this entry names is the epistemic hold for UNCERTAINTY (friction-born inductions climbing toward a dehydrated principle) and for LOWER-RUNG candidates climbing the ladder — it is NOT a universal adjudicability test over every queue/ledger item type, and citing it wider is the over-application the Architect corrected live at tic 676. Both keying axes are non-operative in the lived queue: provenance_class is declared-never-inferred and no author has ever declared `construction_authoritative` (0 of 2,570 rows across 1,105 ids — 100% of queue history matures under the friction_born uncertainty default whether or not it is uncertain; the t673 born, self-diagnosed with its cure fixture- and live-proven both arms, still held 3 tics as if unsure); `birth_rung` is stamped on every row but no maturity predicate reads it (ripple-assessor.py:414–422 + cpr_steppable.py:61 read maturity_tics only); and the one populated uncertainty field (`confidence_tier`) feeds no gate. Ladder Case 2 — correctly-scoped law with un-wired rehydration at the AUTHORING locus: the born template never prompts for provenance_class, so the declaration affordance built at /review 615 has never once fired. Build home: `bk-born-authoring-provenance-class-prompt` (wire the prompt at the authoring locus; assess confidence_tier + birth_rung as gate keys with Architect design input — engine-content separation, the keys are content; fix the ARCHITECTURE.md:930 maturity_tics default-0 vs runtime default-3 contradiction that governs the 1,084 ids carrying no provenance_class).

<!-- promoted from cpr_handoff_review_docket_projection_must_derive_from_maturity_law_tic637 (birth_tic 637, friction_born → /review 640 PROMOTE, Architect-ratified in-tic via AskUserQuestion question set, reviewer ent_homeskillet; confidence 0.80). Source: audit-logs/governance/borns-tic637-drain-campaign-and-docket-physics.md. PATTERN: a handoff/score surface projecting a docket the runtime maturity gate will not admit. CONSTRAINT: Conductor-Score-Runtime Parity (federation) — the score names a docket the runtime does not admit; composes cgg-ledger inscribe-before-the-queue-write (both govern the /review execution seam). EXPLANATION: non-derivable — it names the QUEUE MATURITY LAW as THE derivation source for docket adjudicability, demoting handoff projections to advisory; no existing entry states where adjudicability comes FROM. EVIDENCE: the tic-639→640 handoff projected '2 borns + 5 re-evals adjudicable'; at tic 640 both the cpr-stepper and the session lead INDEPENDENTLY re-derived adjudicability from queue maturity physics (birth_tic/advanced_tic + per-entry window) rather than from the projection — and the bench packet's pending predicate excludes mid-enrichment states, so the packet alone cannot reconstruct the docket (bench-packet-prep.py:732–765 gap, registered in bk-cpr-maturity-envelope-conformance-audit). Prior instance: /review 637 discharged as a zero-mutation hold-pass after the tic-636 handoff booked 7 immature borns. Band: COGNITIVE. Confidence_tier: tentative. signer ent_review_execute (claude-opus-5[1m]), boot receipt 9c587dcf2f86db3e. -->

<!-- scope-fence ray promoted from cpr_maturity_keys_to_uncertainty_both_triggers_nonoperative_tic676 (birth_tic 676, friction_born, origin = Architect mid-tic correction at t676 + field-survey verification → /review 679 PROMOTE-as-ray at first mature window [676+3=679], ratified in-tic question set; the born's review_hints self-assessed "likely refinement ray, not net-new" and the adjudication agreed). Source: audit-logs/governance/borns-tic676-maturity-keys-to-uncertainty-not-universally.md. EVIDENCE verified at source: queue field-presence survey 0/2570 rows construction_authoritative (105 friction_born, remainder absent); cpr-extract.py PROVENANCE_CLASSES frozenset + _resolve_provenance_class declaration path built /review 615, never exercised; ripple-assessor.py:414-422 + cpr_steppable.py:61 read maturity_tics only — neither reads birth_rung nor confidence_tier. Build half routes to bk-born-authoring-provenance-class-prompt (unblocked by this landing), NOT inscribed as doctrine. Band: COGNITIVE. lesson_type: invariant_refinement. Confidence: 0.85. signer ent_homeskillet (claude-fable-5), boot receipt 65da194e48d3fee7. -->

---

## An Anomaly Filed Against a Producer Must First REPRODUCE the Claimed Emission From the Producer's Actual Output Surface
<a id="anomaly-filing-must-reproduce-claimed-emission-from-producer"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=660 | promoted_tic=663 | relations=refinement-ray:presence-observation-fallacy-guard(federation),refinement-ray:emitter-rows-must-match-a-reader-predicate | confidence_tier=tentative -->

An anomaly that attributes a defective emission to a producer must first REPRODUCE the claimed emission from the producer's actual output surface — a consumer's guessed resolution of an ambiguous prescription is not the producer's emission, and filing the guess as the emission plants a wrong locus that survives into the repair queue. Two teeth: (1) before filing a filed-against-producer anomaly, grep/render the producer and quote its VERBATIM output in the anomaly row (claim-vs-emission, not claim-vs-memory); (2) an ambiguous prescription (bare command, relative pointer, unresolvable reference) is itself the finding — file the ambiguity as the defect, not a guessed resolution of it. A ray at this rung on the federation Presence/Observation Fallacy Guard (guard 5: observed-success/observed-content does not prove validity without a fingerprint — here, an anomaly claim does not prove the emission without reproduction) and on emitter-rows-must-match-a-reader-predicate (the anomaly row is a reader's claim about an emitter and must match the emitter's actual rows).

<!-- promoted from cpr_anomaly_diagnosis_must_reproduce_claimed_emission_from_producer_tic660 (birth_tic 660 → /review 663 PROMOTE-as-ray at the CGG rung, ratified in-tic question set round-1 3/3; the born's own recommended_scopes self-flagged "likely a RAY, not net-new" and the adjudication agreed). Source: audit-logs/governance/borns-tic660-anomaly-diagnosis-must-reproduce-claimed-emission.md. Band: COGNITIVE. lesson_type: process. Confidence: 0.75. signer ent_homeskillet (claude-fable-5), boot receipt a504706d67c858b1. -->

---

## Queue-Lane Law: the Canonical Row for a Re-Activated Item Is the Latest Non-Terminal Row WITH Visible Lineage
<a id="canonical-row-for-reactivated-items-latest-nonterminal-with-lineage"></a>
<!-- ledger-tags: authority_class=queue_and_state | rung=domain | domain=context-grapple-gun | born_tic=645 | promoted_tic=663 | relations=composes:terminal-state-valve-pattern,composes:inscribe-before-the-queue-write-that-cites-its-anchor | confidence_tier=tentative -->

When an item re-activates after a terminal write, the row a governance reader treats as canonical is the LATEST NON-TERMINAL row — but only when that row carries explicit lineage: a re-activation appends a NEW row with `prior_terminal_ref` naming the terminal row it succeeds. Readers keep latest-entry-per-id semantics; the terminal valve yields ONLY to an explicit successor row. A later non-terminal row WITHOUT `prior_terminal_ref` is a silent re-activation and stays non-canonical (the terminal row continues to rule) — re-activation is a receipted act, never an accident of append order. Home: the passthrough-repairs family (`bk-queue-field-passthrough-repairs`); this law is the M1 ruling from the tic-645 maturity-envelope conformance audit (§5), decision-routed to /review per the civil report-don't-repair constraint.

<!-- R1 ruling, /review 663 (audit-t645 §7 routing; staged legible at tic 662 in review-663-decision-riders-tic662.md; ratified in-tic question set round-2). Unblocks M5 (with the R7/A2 ruling below). The vocabulary/compile tail (queue_state_compile classification rename + lesson_type enum widening per O1-663) rides bk-queue-field-passthrough-repairs as post-ruling mechanical work. Band: COGNITIVE. Confidence: 0.7. signer ent_homeskillet (claude-fable-5), boot receipt a504706d67c858b1. -->

---

## Ingest-Lane Dedup Law: Id-Keyed Dedup-at-Write Is the Lane's Sufficient Discipline — Content-Hash Stamping Stays the Extract Lane's
<a id="ingest-lane-dedup-law-id-keyed-dedup-at-write-sufficient"></a>
<!-- ledger-tags: authority_class=queue_and_state | rung=domain | domain=context-grapple-gun | born_tic=651 | promoted_tic=663 | relations=composes:terminal-state-valve-pattern | confidence_tier=reinforced -->

cogpr-ingest rows enter WITHOUT a content `dedup_hash` by design: the ingest lane's dedup discipline is ID-KEYED DEDUP-AT-WRITE (an envelope is never re-ingested if its canonical id already exists in the queue's latest-entry projection, any status), and that is SUFFICIENT for the lane — the federation's dedup-at-write invariant keys on canonical identity, not content hash. Content-hash stamping (colon-form `SHA256(source:lesson)[:16]`) remains the EXTRACT lane's discipline; the asymmetry between the two intake lanes is documented law, not a seam defect. Cross-check valve: a mechanical office (the cpr-stepper) MAY compute advisory content hashes for ingest rows under an explicitly non-authoritative field (`dedup_check` with `authoritative: false`) so collisions stay observable without minting the field — advisory visibility, never silent stamping. Evidence: the 651 census (6/12 stamped, 0 collisions), the A6 retirement census at 662 (colon 173 / plain 6 all-terminal / 0 collisions / 0 new plain mints since 644), and the stepper's A2-663 advisory check this tic (both civil rows collision-free, correctly NOT minted pending this ruling).

<!-- R7 ruling, /review 663 (A2-651 classify-list carry; staged legible at tic 662; ratified in-tic question set round-3 — "Sufficient — document + close", no emitter patch). Closes the A2-651 rider. Band: COGNITIVE. Confidence: 0.7 at ruling, reinforced by three independent censuses (651, 662, 663). signer ent_homeskillet (claude-fable-5), boot receipt a504706d67c858b1. -->

---

## A Correction Can Land on a Third Surface the Pair's Marker Vocabulary Cannot Reach — Pair-Agreement Is Not Evidence Currency
<a id="third-surface-correction-pair-agreement-is-not-evidence-currency"></a>
<!-- ledger-tags: authority_class=queue_and_state | rung=domain | domain=context-grapple-gun | born_tic=661 | promoted_tic=664 | relations=refinement-ray:disagreement-as-evidence(federation),composes:reader-side-divergence-cure-t611,composes:append-only-never-retcon | confidence_tier=tentative -->

The t611 reader-side divergence cure gives marker vocabulary for exactly TWO loci — the queue row and its source file — but the append-only-never-retcon law GUARANTEES that a correction is appended where it happened, which is structurally often a THIRD surface (e.g. a `record_correction` row in a reviews ledger) that is neither of the two. When the row and its source file agree with each other, no divergence marker can fire — the agreement itself is the trap (Disagreement-as-evidence, INVERTED: two monitored surfaces agree on evidence a third surface has disproven), and the docket reader inherits a disproven claim with zero signal. This recurs BY DESIGN, not incidentally: every never-retcon correction to an already-extracted claim lands outside the row/source pair. Cure class (ruled SIBLING at promotion — NOT folded into the t611-covenant body-refresh mechanism, whose axis is the row↔source pair; folding would silently widen the /review-615-gated build): a typed correction back-reference — the correcting row names the corrected queue id — resolved by a deterministic reconciler pass that mints the reader-side staleness marker on the affected row; the author bodies stay untouched. Until the reconciler exists, third-surface corrections are found only by hand-probing — exactly the un-wired-rehydration failure mode (Case 2). Build home: `bk-third-surface-correction-reconciler` (registered at promotion, sequenced after `bk-review-663-amendment-tranche`).

<!-- promoted from cpr_correction_can_land_on_a_third_surface_beyond_marker_vocabulary_tic661 (birth_tic 661 → /review 664 PROMOTE-as-ray at the CGG rung, ratified in-tic question set; SIBLING ruling closes the fold-vs-sibling question two successive shadow proposals held open by design). Source: audit-logs/governance/borns-tic661-correction-can-land-on-a-third-surface.md (stepper pass 661-cpr-step anomaly A1-661). Evidence: the t658 record_correction narrowing the t653 row while row+source agreed (hand-probed at 661; marker staleness.source_claim_narrowed_by_external_correction minted by hand) + the 664 stepper's own record_correction (fresh same-tic third-surface instance — row and source still agree, no marker can fire). Band: COGNITIVE. lesson_type: doctrine_gap. Confidence: 0.85. signer ent_homeskillet (claude-fable-5), boot receipt 58bbde08d2a5f2c6. -->

---

## Install-Parity's Reference Is the Sole-Writer Lane, Not the Resident Tree — Shared-Reference Agreement Blinds Drift Exactly When the Reference Moves
<a id="install-parity-reference-is-sole-writer-lane-not-resident-tree"></a>
<!-- ledger-tags: authority_class=sync_and_install_parity | rung=domain | domain=context-grapple-gun | born_tic=673 | promoted_tic=676 | relations=refinement-ray:state-agreement-is-not-truth-unless-lifecycle-reachable(federation),refines:runtime-sync-parity-verification,composes:claimed-install-state-requires-auditable-sync-log-proof | confidence_tier=tentative -->

Install-parity syncs mirror the RESIDENT BRANCH's working tree, and the drift check compares installed bytes against that SAME working tree — so a checkout to any non-main branch silently regresses installed runtime semantics while drift reads 0. The canary is structurally blind exactly when the reference is wrong, because reader and writer share ONE reference (the resident tree) instead of the sole-writer lane (main). Agreement (installed == working-tree) was true throughout the lived window; the TRUTH condition — installed matches the sole-writer lane — was false. There was NO disagreement to read: both readers read the same wrong substrate, the missing-independent-reader shape (Disagreement-as-evidence has nothing to bite on). The law: parity must be DEFINED against the sole-writer lane, never the resident tree; branch residence needs a physics-layer guard at the write boundary (refuse/override-with-lineage on non-main sync), and every sync row carries its `reference_branch` so the reference is auditable, never ambient. A next-fire hazard rider attaches: any scheduled consumer of a silently-regressed script re-manufactures the exact defect the regression reverted before anyone looks. Cure landed as implementation (not doctrine) at tic 675: `runtime-sync.py branch_residence_guard` — cmd_sync/cmd_auto_sync refuse non-main CGG residence BEFORE any copy or signal-resolution side effect, `--allow-non-main` overrides WITH lineage, refusal rows + `reference_branch` stamps land in `cgg-sync-log.jsonl`; fixture-first 13 arms red→green; live-proven both arms (the landing commit's own hook synced through the guard; a throwaway-branch drill refused with zero copies).

<!-- promoted from cpr_branch_residence_regresses_installed_runtime_tic673 (birth_tic 673 → /review 676 PROMOTE dual-surface, ratified in-tic question set). Source: audit-logs/governance/borns-tic673-branch-residence-regresses-installed-runtime.md (the tic-673 PR#18-gate residence window: 5 installed surfaces regressed — manifest-prune age_unknown 6→0 reverting the tic-671 honesty repair, init-governance SKILL, cpr-stepper agent, ladder-audit.py, cgg-doctor.sh — drift 0 throughout; restored from CGG main a5f4c8b, cmp OK ×5). Sibling half inscribed same pass: federation constitution-ledger#state-agreement-is-not-truth-unless-lifecycle-reachable refinement (moved-reference agreement = the third lying shape). Band: COGNITIVE. lesson_type: invariant_refinement. Confidence: 0.85. signer ent_homeskillet (claude-fable-5), boot receipt 261ae5d913788074. -->

---

## Membrane Gate Numbers Require a Train/Eval Disjointness Attestation to Cross With Them
<a id="membrane-gate-number-requires-train-eval-disjointness-attestation"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=650 | promoted_tic=680 | relations=refinement-ray:calibration-population-validity-measure-against-the-acted-on-population(federation),composes:envelope-pattern-is-the-standard-capability-surface-contract(federation) | confidence_tier=reinforced -->

A score produced on the far side of an evidence membrane is admissible as a GATE-PASS only if the train/eval disjointness attestation crosses WITH it. Absent that field, the receiving governance surface cannot distinguish a capability gate from a retention check, and will re-cite the number as proof of a capability the number never measured. Refines the evidence-membrane admission family: the train/eval disjointness attestation must CROSS the membrane WITH the number, else a retention check is re-cited as a capability gate — the epoch14 mean-1.0 S5 wall-refusal, t650. The systems-layers hoist's most-trusted number (the epoch14 self-foreclosure gate, "mean 1.0, 8/8, 0 hard-fails") was scored on 8 eval records whose exact prompt shape and literal answer-key tokens were replayed into that branch's training set by explicit design (428 of 524 training rows keyed to the 8 eval ids); the self-foreclosure eval universe has been the same 8 fixed records since 2026-06-17 with no held-out set at any epoch. The obligation attaches at the CROSSING, not at the measurement: a number admitted across a membrane as evidence must arrive with a provenance envelope naming (a) the eval population, (b) its disjointness from the training population, (c) whether a held-out set exists, and (d) what the number can and cannot discriminate. A number without that envelope may be recorded as an observation; it may never be re-cited as a gate-pass.

<!-- promoted from cpr_membrane_gate_number_requires_train_eval_disjointness_attestation_tic650 (birth_tic 650 → /review 680 PROMOTE-as-ray, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic650-membrane-gate-numbers-need-train-eval-disjointness-attestation.md; membrane evidence read READ-ONLY (observe-not-couple, canonical sole-writer, zero membrane writes) from /Volumes/T7 Shield/models/organization-engine-lora/. Band: COGNITIVE. Confidence_tier: reinforced. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Finite-Supply Eligibility Lanes Must Type Aggregate Supply State — the Window Is Perishable and Consumed From Both Ends
<a id="supply-state-law-aggregate-typing-and-perishable-window"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=653 | promoted_tic=680 | relations=refinement-ray:emission-granularity-is-the-leak-not-the-obligation,merges:cpr_supply_window_consumed_by_route_completion_tic657 | confidence_tier=reinforced -->

A finite-supply eligibility lane must type its AGGREGATE supply state (per-request typed refusals leave lane-level truth untyped and booked instructions fail silently); the supply window is PERISHABLE and consumed from BOTH ends — admission-absence starves it (the t653 edge) and route COMPLETION closes it (the t657 edge). Two teeth from the consumption-edge complement: (1) within-tic ordering — when one tic owes both a supply-consuming transition (route close, terminal writeback) and a supply-drawing act (emission/crossing), the drawing act sequences FIRST, or the transition's receipt must NAME the window it is closing as an accepted cost; (2) restored-supply is not banked — a window that sits unconsumed across tics while other work takes lawful precedence expires silently at the next terminal transition, so the lane owning the draw must either consume promptly or carry a visible expiry-risk marker, never assume the window persists. Tier reinforced (two births, one shared centroid, opposite edges of the same perishable window).

<!-- MERGE: carrier cpr_intake_admission_supply_exhaustion_is_lane_level_state_tic653 (birth_tic 653) + absorbed cpr_supply_window_consumed_by_route_completion_tic657 (birth_tic 657, consumption-edge complement) → /review 680 MERGE+PROMOTE, Architect-ratified in-tic via question set. Sources: audit-logs/governance/borns-tic653-admission-supply-and-authority-park-resurfacing.md + audit-logs/governance/borns-tic657-supply-window-consumed-by-route-completion.md. Refinement-ray on emission-granularity-is-the-leak-not-the-obligation (the emit side is per-item emissions flooding a manifold; this centroid is the REFUSAL side — per-request typed refusals hiding an aggregate supply exhaustion). Band: COGNITIVE. Confidence_tier: reinforced (n=2 independent births, opposite edges of one window). signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Pin-Ordering Discipline for Self-Mutated Surfaces — a Receipt Must Not Pin What Its Own Persistence Step Then Mutates
<a id="pin-ordering-discipline-self-mutated-surfaces"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=654 | promoted_tic=680 | relations=composes:the-corrector-inherits-the-verification-obligation | confidence_tier=tentative -->

A verification receipt that hash-pins a surface its OWN persistence step then mutates will always self-invalidate: the pin records pre-write bytes, the landing mutates them, and the next reader lawfully rejects the receipt as contradicted — by design, every time, not probabilistically. Cure: pin-ordering discipline — exclude self-mutated surfaces from the pin set, or compute their pins LAST after all persistence writes land. Distinct from ordinary drift-repin (external field motion): here the receipt author IS the drift source, deterministically. Ray on the receipt-discipline cluster — adjacent to, not a twin of, the-corrector-inherits-the-verification-obligation's "hash pins are COMPUTED-AT-WRITE, never transcribed" refinement (that ray governs TRANSCRIPTION-fabrication drift; this one governs a receipt's own persistence step invalidating its own pin by construction).

<!-- promoted from cpr_self_referential_pin_race_in_receipt_persistence_tic654 (birth_tic 654 → /review 680 PROMOTE-as-ray, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic654-self-referential-pin-race-in-receipt-persistence.md; tic-654 §14 re-drain: drain-receipts/bk-board-live-backlog-compiler.json pinned route-metadata.json, then the re-drain's repin_history mutated route-metadata.json, dropping conformation/execution to fail-closed nulls until the pin was recomputed post-mutation. Band: COGNITIVE. Confidence_tier: tentative. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Author a Governance Act From the Consumer's Contract, Not the Act's Own Lane Vocabulary
<a id="authoring-from-consumer-contract"></a>
<!-- ledger-tags: authority_class=pipeline_and_integration | rung=domain | domain=context-grapple-gun | born_tic=655 | promoted_tic=680 | relations=refinement-ray:conductor-score-runtime-parity-cgg-application | confidence_tier=tentative -->

When authoring a governance act (admission, ratification, promotion) that a fail-closed resolver will consume, author the act's structured record FROM the consumer's contract — read the resolver's enum values and typed-field schemas BEFORE writing the block — not from the act's own lane vocabulary. The ratifying act and its structured consumer legitimately speak different vocabularies (reviews-lane action=admit_covenant vs resolver enum decision=admitted; prose anchor vs typed anchor {kind, ref}); the divergence is not a defect in either surface, but an author who writes from the act's vocabulary produces a well-intended record the resolver lawfully refuses. Corollary: a fail-closed gate's precise refusal codes during a FIRST live exercise are the gate working, and the refusal→cure chain is itself the exercise's evidence — record it, never smooth it into "landed first try." Ray on conductor-score-runtime-parity-cgg-application; exercised t658→679 K1 resolver chain.

<!-- promoted from cpr_admission_act_authored_from_consumer_contract_tic655 (birth_tic 655 → /review 680 PROMOTE-as-ray, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic655-admission-act-authored-from-consumer-contract.md; audit-logs/governance/harpoon-office/harpoon-sequencer.py:550 (ADMISSION_CAPABLE_DECISIONS={'admitted'}) + :627-634 (typed anchor law). Band: COGNITIVE. Confidence_tier: tentative. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Terminal Writebacks Must Be Body-Preserving — Copy the Prior Latest Row and Patch, Never Append a Thin Status-Only Row
<a id="terminal-writeback-must-be-body-preserving"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=656 | promoted_tic=680 | relations=refines:terminal-state-valve-pattern,composes:inscribe-before-the-queue-write-that-cites-its-anchor,refinement-ray:terminal-state-change-requires-receipt-and-no-signal-goes-dark(federation) | confidence_tier=tentative -->

A terminal-verdict writeback must be BODY-PRESERVING when it lands on a row whose content is load-bearing — copy the prior latest row and patch the lifecycle fields, never append a thin status-only row — because under latest-entry-per-id the thin patch strips lesson/source from the authoritative projection even though history retains them. The sharpest failure shape: a single pass that (a) ABSORBs one id by designating a sibling as the family content-carrier and (b) writes a thin terminal row on that very sibling has let the designated carrier's signal go projection-dark in the same breath — the no-signal-goes-dark check must run against the PROJECTION the readers consume, not against append-only history. Mechanical tell: the degenerate/body-less row set grows exactly +1 per thin terminal verdict. FIRE-PROOF (per the also-ratified promotion-claim-of-automation-requires-observed-fire guard): the /review 679 verdict writeback executed body-preserving citing this discipline — an observed exercise, not merely a registration — and this very tic-680 docket's writebacks are a further live exercise.

<!-- promoted from cpr_terminal_writeback_strips_designated_carrier_body_tic656 (birth_tic 656 → /review 680 PROMOTE, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic656-terminal-writeback-strips-designated-carrier-body.md; cpr-stepper A2-656 (audit-logs/reviews/2026-07-26.jsonl): tic256 SKIP row vs t645 restored body, degenerate set 719→720 arithmetic. Band: COGNITIVE. Confidence_tier: tentative. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Attestation Predicates Must Prove Execution, Not Outcome — a Healthy-Outcome Requirement Converts a Lawful Loud-Halt Into a False Stability Failure
<a id="attestation-predicate-must-prove-execution-not-outcome"></a>
<!-- ledger-tags: authority_class=pipeline_and_integration | rung=domain | domain=context-grapple-gun | born_tic=652 | promoted_tic=680 | relations=refinement-ray:wrapper-must-discriminate-instrument-exit-code-semantics-crash-vs-verdict | confidence_tier=tentative -->

An attestation predicate that requires a healthy OUTCOME as proof of cable EXECUTION converts a lawful loud-halt tic into a false stability failure. The economy CADENCE cable attests ok only when mint_accrued > 0 (economy-heartbeat.py fire_gunslinger worker, ~line 361); at tic 652, the first full-tic mint halt of the series (zero_mint_ticks=1000/1000, trust below tau the entire tic — the DESIGNED loud breach state, surface-don't-hide) made the fully-executed CADENCE cable fail attestation, dropping it from winch dispatched and cascading to all_three_cables_committed=false → seed_stabilized=false, while 11/12 stabilization checks passed and the CADENCE trace was complete (1000 ticks ran, guards held, federal anchor frozen). Attest execution-lawfulness separately from outcome-health: a by-design breach flag must not silently flip a structural stability verdict one layer up. Ray on wrapper-must-discriminate-instrument-exit-code-semantics-crash-vs-verdict (same centroid — execution-vs-verdict conflation — one layer deeper: inside the instrument's attestation predicate rather than the wrapper's exit code). Build leg noted, not executed here: economy-heartbeat.py fire_gunslinger attest-on-execution — the orchestrator owns the backlog entry, none created by this pass.

<!-- promoted from cpr_mogul_economy_heartbeat_b9dec58f1c4c (birth_tic 652 → /review 680 PROMOTE-as-ray, Architect-ratified in-tic via question set). Source: mogul:economy_heartbeat cycle, audit-logs/economy/economy-tic-652.json detail.winch_fire + detail.stabilization_checks + detail.economy_trace.CADENCE; economy-heartbeat.py fire_gunslinger CADENCE worker (`and t.get("mint_accrued", 0) > 0`). Report-only at birth: no code change made in that cycle. Band: COGNITIVE. lesson_type: meta. Confidence_tier: tentative. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## Replay Reporting Boundary Must Declare Its Source — a Console Channel Can Diverge From the Durable Channel Silently
<a id="replay-reporting-boundary-must-declare-source"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=677 | promoted_tic=680 | relations=refinement-ray:same-tic-replay-diverges-consumers-declare-source | confidence_tier=tentative -->

A series-stateful kernel's re-fire is typed REPLAY at the ENGINE, but the divergence it creates is unguarded at the REPORTING boundary — and the reporting boundary is the one that gets read. The economy heartbeat's advance guard correctly types genesis|continue|replay: on a re-fire of an already-advanced tic it writes a sidecar snapshot, refuses to write series state, and leaves current-pointer.json anchored on the series row — that fence is sound. What is NOT fenced is the console channel: the invoke script prints g_t / mint_total / supply / breach_flags — precisely the fields a cycle report is required to carry — and on a replay run those printed values are the NON-ADMITTED replay economics, not the series truth. At tic 677 the divergence was material and silent: stdout g_t=0.494447 / mint_total=6040.60 / supply=6084.12 against the series artifact's g_t=0.748988 / mint_total=37459.99 / supply=21968.54 (a ~6x mint gap); nothing errored, both channels were internally consistent, and the only tell was an unchanged artifact mtime plus a series_mode:'replay' field in the appended invocation row. Reader-side rule: for any series-stateful producer, the cycle report MUST source its numbers from the pointer-designated artifact and MUST carry series_mode as a first-class field — never from invocation stdout, however authoritative the stdout looks. The failure fires precisely on the recovery path after a failed mandate — exactly when the system is already degraded and trusting the report most. Ray on same-tic-replay-diverges-consumers-declare-source (sibling-site completion at the REPORTING boundary): the engine fence is sound, but the console channel — the one actually read — carries no source declaration. Build leg noted, not executed here: economy-invoke console source-tag — the orchestrator owns the backlog entry, none created by this pass.

<!-- promoted from cpr_mogul_economy_heartbeat_76588aecf1b2 (birth_tic 677 → /review 680 PROMOTE-as-ray, Architect-ratified in-tic via question set). Source: mogul:economy_heartbeat cycle; economy-heartbeat.py lines 177-178 (advance_guard), 484-485 (replay sidecar path), 492-499 (pointer skipped on replay); artifact mtime unchanged (Aug 3 10:34) against invocations.jsonl row invoked_at 2026-08-04T23:02:14Z series_mode='replay'. Band: STRUCTURAL. lesson_type: reader_boundary_discriminator. Confidence_tier: tentative. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## A Park's Re-Surfacing Mechanism Must Match Its Gate Type — an Authority-Gated Park Must Not Re-Arm an Evidence-Shaped Expiry Window
<a id="park-resurfacing-mechanism-must-match-gate-type"></a>
<!-- ledger-tags: authority_class=review_and_promotion_discipline | rung=domain | domain=context-grapple-gun | born_tic=653 | promoted_tic=680 | relations=composes:obligation-lifecycle-must-be-bounded-at-both-ends(federation) | confidence_tier=reinforced -->

A park's re-surfacing mechanism must match its gate type. An authority-gated park (feedback_required, Architect decision owed) re-surfaced through an evidence-window that can only expire — and expired 3 consecutive passes without being able to discharge the gate, because window-expiry is evidence-shaped while the blocker is authority-shaped. Cure: at FIRST window expiry of an authority-gated park, route a decision-brief WAIT envelope to the authority's retrieval surface (inbox) and stop re-arming the window; the window mechanic is reserved for evidence-gated parks. Composes window-expired-is-not-condition-met with the gate-is-on-the-call-site family: the re-surfacing channel is part of the gate's design. Lived: C3 t327 parked ~35 tics (A1-657), and the tic-653 first instance of the cure — decision brief WAIT fae95ed2 delivered to ent_breyden inbound — discharged two feedback_required parks (tic256, 00c5) in 2 tics after the evidence-window mechanic had failed to discharge them for 9 tics past expiry. Cure exercised again this pass by the orchestrator at the tic-680 window (C3 decision brief routed; envelope reference lands in the same-tic commit).

<!-- promoted from cpr_authority_gated_park_resurfaces_on_authority_retrieval_surface_tic653 (birth_tic 653 → /review 680 PROMOTE, Architect-ratified in-tic via question set). Source: audit-logs/governance/borns-tic653-admission-supply-and-authority-park-resurfacing.md; stepper receipts across 651/652/653 (2nd and 3rd expiry counters). Band: COGNITIVE. Confidence_tier: reinforced. signer ent_review_execute (claude-sonnet-5), boot receipt fd504e8af50728c0. -->

---

## A Boolean-OR Breach Flag Over a High-Frequency Inner Loop Saturates — Expose the Duty Cycle or Contract the Flag as Occurrence-Only
<a id="breach-flag-or-saturation-expose-duty-cycle"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=686 | promoted_tic=689 | relations=inverse-arm:presence-observation-fallacy-guard(federation — observed-BREACH does not prove breach-SEVERITY),composes:authoritative-set-readers-must-read-the-manifest(federation),distinct-axis:economy execution_attested cure (t684),coordinates:bk-economy-reporting-contract-temporal-scope | confidence_tier=high -->

A tic-boundary breach indicator computed as a boolean OR over a high-frequency inner loop SATURATES and destroys exactly the severity signal it exists to carry — the flag must expose its duty cycle, or its consumers must read the per-tick counts instead of the flag. The c-coin economy heartbeat is the proof. Its top-level breach_flags array is IDENTICAL — ['trust_below_tau','mint_halted'] — across four consecutive tics whose underlying reality differs enormously: tic 683 breached 1000/1000 inner ticks with ZERO minting ticks (a total halt); tic 684 breached 335/1000 with 665 minting ticks (mostly healthy); tic 685 breached 592/1000 with 408 minting; tic 686 breached 729/1000 with 271 minting. A total-halt tic and a two-thirds-healthy tic present the reader with the same two strings. Because the inner loop runs 1000 ticks per governance tic, any single breached inner tick saturates the flag to true, so across the entire observed operating range the flag is non-discriminating — it is true always, and therefore carries no information beyond "the simulation ran." The discriminating measurement ALREADY EXISTS and is already emitted: detail.breach_flag_tick_counts carries 1000 / 335 / 592 / 729, and detail.ticks_with_mint carries 0 / 665 / 408 / 271. This is not a missing measurement — it is a measurement that is flattened at precisely the surface consumers read, which is the more dangerous shape because the artifact looks complete. This is the inverse of the Presence/Observation Fallacy Guard's observed-success arm: here, observed-BREACH does not prove breach-SEVERITY, and a reader that treats the flag as a severity signal will read a recovering economy and a halted one as the same state. It is also the same family as the tic-684 execution_attested cure (which fixed "seed_stabilized alone reads breach-visibility as instability") but on a distinct axis — that cure separated execution from stability; this one concerns the breach indicator's own resolution. Remedy shape: either carry a duty-cycle/severity alongside each flag at the top level (flag + breached_ticks/total_ticks), or declare in the artifact contract that breach_flags is an occurrence witness only and that every severity consumer MUST read breach_flag_tick_counts — and then sweep the consumer set, since a flag that has been non-discriminating for its whole observed history may already have silent-degrade readers treating it as severity.

FIFTH ILLUSTRATION, LIVE AT RATIFICATION (tic 689, recorded at inscription): economy-tic-689 closed with g_t 0.749035 — ABOVE tau 0.70 — while breach_flags still read ['trust_below_tau','mint_halted']: the OR saturated on inner-loop dips during a tic whose FINAL trust gate stood above the floor. The flag and the headline scalar now disagree in the artifact itself, which is precisely the shape this entry names and the reason the duty-cycle exposure (or the occurrence-only contract) is the cure rather than flag suppression — surface-don't-hide holds; the flag is not wrong, it is unresolved.

Lock line: *A boolean OR over a high-frequency loop is an occurrence witness, not a severity signal — expose the duty cycle at the surface consumers read, or contract the flag as occurrence-only and sweep its consumer set.*

<!-- promoted from cpr_mogul_economy_heartbeat_b37a95e35cb8 (birth_tic 686 → /review 689 PROMOTE-net-new, Architect-ratified in-tic question set, round 1). Source: mogul:economy_heartbeat (mogul_mandate_id tic-686-20260807T184809; no born file — the queue-row lesson is the authoritative body). Evidence-complete at birth: economy-tic-683/684/685/686.json enumerate the saturated flag array against the discriminating per-tick counts; fifth illustration economy-tic-689.json (g_t 0.749035 above tau with flags saturated) observed live at the ratifying pass. Non-derivability adjudicated at ratification: inverse-arm relation to presence-observation-fallacy-guard noted, but the instrument-design law (OR-over-inner-loop destroys severity; duty-cycle exposure is the cure) is not entailed by the guard's observer-side arms — net-new at this rung, cross-rayed rather than folded. Remedy NOT built this pass: consumer-set sweep + duty-cycle field land via bk-economy-reporting-contract-temporal-scope (MED, open — t689 illustration noted on the entry rather than duplicated as a new fix-site, per coordinate-don't-duplicate). Twin stub cpr_mogul_economy_heartbeat_33c6e7c6d0f5 ABSORBED this same pass (verify-twin judgment; hash-dedup ABSENT lane-wide — bk-cogpr-ingest-dedup-hash-unstamped filed t689). Queue: status extracted → promoted; maturity discharged (686+3=689 — queue truth over the scheduler's 690 stamp). Band: STRUCTURAL. lesson_type: net_new. confidence_tier: high. signer ent_homeskillet-f5 (claude-fable-5), boot receipt b25938b3adf8fb75. -->

---

## A Report Contract That REQUIRES Fields Its DISPATCHED PRODUCER Cannot Emit Will Be Satisfied by Inference — and the Inference Will Read as Measurement
<a id="dispatch-prescription-contract-producer-mismatch-mints-inference-as-measurement"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=690 | promoted_tic=693 | relations=refinement-ray:emitter-rows-must-match-a-reader-predicate,composes:headless-delegate-structured-emission-contract,sibling-arm:presence-observation-fallacy-guard(federation — presence-of-artifact-does-not-prove-semantic-fulfillment),fix-site:bk-mandate-cache-refresh-contract-producer-split | confidence_tier=tentative -->

A report contract that REQUIRES structured fields its DISPATCHED PRODUCER cannot emit will be satisfied by inference, and the inference will read as measurement. When a cycle/report schema names keys {A,B,C} but the invocation it prescribes structurally emits only {A} — because B and C are produced by sibling entry points joined elsewhere — the agent filling the report faces a contract it cannot honestly satisfy from the dispatched surface. The observed failure mode is not a refusal or an empty key: it is a PLAUSIBLE value derived from the one key that WAS produced, written into the artifact in the same shape a measured value would take, with no marker distinguishing the two. That synthesized value can be flatly false while looking careful, because the inference reasons from an adjacent substrate that does not actually govern the missing field. The cure is at the DISPATCH surface, not the agent: either prescribe the entry point whose result dict carries the full contract, or split the contract to match what the prescribed producer emits. A secondary cure is a per-key measured|derived provenance marker so a downstream reader can tell which keys were observed.

EVIDENCE (n=2, cross-tic): the mandate prescribes `visitor-economy-monitor.py --cache-refresh $TIC` and requires `results.cache_refresh` to carry cache_state + standing_decay + biome_health; that flag's envelope emits ONLY cache_state (the three are joined solely by `--full-cycle`, script lines 397-398). At tic 687 — the only prior cycle report carrying the key — both missing keys were filled by inference from the empty cache, recording "no standing to decay — cache carries zero entries". Measured at tic 690, that inference is FALSE on its premise: standing_decay scanned 6 visitors (visitor standing is a separate substrate from the cache) and biome_health reported 2 thresholds violated with nutrient_flow 0.0 — an inferred "healthy" would have been wrong in both directions. Re-verified live at tic 693 by the cpr-stepper: the `--cache-refresh` flag path is confirmed at `visitor-economy-monitor.py:433-434` calling `cache_refresh_cycle` ONLY (envelope operation=cache_refresh, line 137); the mechanism is unresolved and the condition is NOT resolved.

Ray on *An Emitter Whose Rows Never Match Any Reader Predicate Is Acoustically Dead While Looking Live* (sibling-site completion at the DISPATCH boundary): that entry governs the emitter whose rows no reader predicate can match; this one governs the inverse — a reader contract no prescribed producer can fill. Composes the Headless Delegate Structured Emission Contract (which obliges the delegate to emit the verifier's required key) by naming the case where the prescribed invocation makes that obligation *unsatisfiable*, so the discipline silently converts into fabrication rather than into a refusal.

Lock line: *A contract the prescribed producer cannot satisfy does not fail loudly — it gets filled by inference wearing the shape of measurement; fix the dispatch surface or split the contract, and mark each key measured|derived.*

<!-- promoted from cpr_mogul_cache_refresh_2039b17fe99f (birth_tic 690 → /review 693 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set round 1, 5/5 as recommended). Source: mogul:cache_refresh cycle (mogul_mandate_id tic-690-20260809T052045; no born file — the queue-row lesson is the authoritative body; origin cycle reports under audit-logs/mogul/cycle-reports/reports/*tic-690*). Non-derivability adjudicated at ratification: the neighbours (presence-of-artifact-does-not-prove-semantic-fulfillment; Headless Delegate Structured Emission Contract) do not name the DISPATCH-SIDE defect where the contract itself is unsatisfiable from the prescribed producer — admitted as a refinement ray, cross-rayed rather than folded. Fix-site bk-mandate-cache-refresh-contract-producer-split filed by the /review lead this pass — cited here, not duplicated. Band: STRUCTURAL. lesson_type: contract_producer_mismatch. confidence_tier: tentative. signer ent_review_execute (claude-opus-5[1m]), boot receipt e1b9d40a29e59e40. -->

**Refinement — the reciprocal sign: an agent-authored key's ABSENCE read as a machine defect (tic 715 → /review 718 PROMOTE-as-refinement-ray, ratified in-tic question set):** Inside a single `results.<cycle>` object, MACHINE-MEASURED keys and AGENT-AUTHORED keys are structurally indistinguishable — so their presence AND their absence are both misread, in opposite directions. The parent names one sign (an agent's inference filling a mandate-demanded key READS AS measurement; the t687→t714 arc and its producer-split cure). This ray is the other sign, measured at tic 715: the ABSENCE of an agent-authored narrative key was filed as a missing MACHINE measurement and carried for a full tic as an open backlog row against a script defect that does not exist (`grep -c inscribed_index mogul-runner.sh` = 0 — no producer; the machine key `inscribed_index_size` populates on EVERY path, and measured 720 on the very solo path the row named as failing). Same axis, opposite sign: one direction fabricates a measurement, the other fabricates a gap. Two teeth: (1) a cycle-report contract must NAME which keys are machine-measured and which are agent narrative — the parent's per-key measured|derived provenance marker, extended to the narrative direction; (2) a gap filed against a producer must LOCATE the producer before it is filed — if grep finds no producer, the variance is agent behavior and the fix surface is the report contract, never the script. Corollary for the receiving reader: a raw token/occurrence count is not a dedup verdict (ledger.md legitimately carried the 65c1 token twice — once as `invariant_id` metadata, once as the exactly-once provenance comment; the count alone would have read as a breach). Backlog closure ratified with this verdict: `bk-review-close-solo-cycle-inscribed-index-empty` closed FALSIFIED on this evidence.

<!-- promoted from cpr_mogul_review_close_check_caf600b532d7 (birth_tic 715 → /review 718 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set, round 1, as-recommended). Source: mogul:review_close_check (mogul_mandate_id tic-715-review-close-20260815T0239; no born file — the queue-row lesson is the authoritative body; net-new evidence = the falsification of an open backlog row by locating its named producer and finding none, plus first-party proof the allegedly-thinning path measured correctly). REFINES this entry along the SIGN axis: the parent governs inference-wearing-measurement (presence direction); this ray governs gap-fabrication (absence direction) and adds the locate-the-producer-before-filing discipline. Backlog row bk-review-close-solo-cycle-inscribed-index-empty closed falsified same pass. Queue: status extracted → promoted; maturity discharged (birth 715 + 3 = 718). Band: STRUCTURAL. lesson_type: contract_shape. confidence_tier: tentative. Confidence: 0.8. signer ent_homeskillet-f5 (claude-fable-5), boot receipt f83c4aed7bd69e3b. -->


**Refinement — the THIRD sign: a MEASURED key whose NAME over-claims — a past-tense write-verb key must report the WRITE VERDICT, and a copy-verbatim contract transmits a producer's mis-naming under the reporter's own no-inference warrant (tic 744 → /review 747, `cpr_mogul_cache_refresh_7ca94b804d78`; PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 1 Q1, recommended option verbatim; confidence measured).** The parent names inference wearing measurement (the presence direction of an AGENT-authored key); the t718 ray names absence read as a machine defect. This ray is the third sign: a key that IS measured and IS present — `signals_emitted` — whose NAME asserts a landing the producer never checked. `visitor-economy-monitor.py` `_emit_signal()` hands the signal to `dedup_signal_append()` and returns the constructed dict unconditionally (:83–84), never reading the dedup verdict; the caller appends the id to `signals_emitted` unconditionally (:230, :290) and `thresholds_violated = len(signals_emitted)` inherits it (:301). The field lists ids CONSTRUCTED, not rows WRITTEN, and reads identically in the wrote case and the suppressed-by-dedup case. Re-derived at /review 747: `sig_cache.refresh_complete_*` has ZERO rows in all 150 daily signal ledgers and the manifest while appearing in 121 producer reports from tic 387 to 747 (a fresh hash every cycle — non-deterministic AND non-landing); the biome ids' last ledger row is 2026-08-27T07:23Z (tic 743); at 747 the producer claimed both biome ids emitted and zero rows landed — and that suppression was the manifest-dedup CURE working (the ids entered the manifest at 743; cross-UTC-day dedup against the curated set is the ruled behaviour), which is the sharpest form of the finding: the SAME field value on the days the write was wrongly re-landing (08-24..08-27) and on the day it was rightly suppressed. THE INTERACTION WITH THE EXISTING CURE (the non-derivable half): the t687/t692 producer-split ruling (this entry's fix-site) REQUIRES the mandate consumer to copy `biome_health` verbatim precisely so the reporter cannot infer — so verbatim-copying a key whose NAME over-claims transmits the producer's over-claim into the governance record WITH the reporter's no-inference discipline as its warrant. Copy-verbatim protects against reporter fabrication; it does not protect against producer mis-naming; the two cures are not substitutes. Cure at the producer: return the dedup verdict and split the key — `signals_written` / `signals_suppressed_as_duplicate` (the old key a deprecated alias for one era; `thresholds_violated` follows the written set); the consumer-side interim is a separately-labelled disposition verification beside the untouched verbatim copy (`EMISSION_DISPOSITION_VERIFICATION`, first fired 744; `emission_landing_finding`, 747). Slice: the visitor-economy-monitor producer and its mandate consumer; the generalization to every `*_emitted` / `*_written` key is proposed, not measured — the census across cgg-runtime producers is the owed evidence. APO: NOT a dedup-at-write defect (the path is correct and the 744 re-emission falsifier proved it); NOT the manifest-reader doctrine; NOT the reporter-inference defect (the OPPOSITE failure — the parent's sign). Relations: composes constitution-ledger#presence-observation-fallacy-guard (an id constructed is not a row written); distinct_from constitution-ledger#authoritative-set-readers-must-read-the-manifest-not-aggregate-raw-emissions; sibling bk-mandate-cache-refresh-contract-producer-split (done 696); build row bk-write-verb-keys-report-write-verdict. The runner's b747 mint `cpr_mogul_memory_mining_4a2b38ee669e` (fence 750) re-observed this centroid from the 747 cache_refresh finding — absorb-as-reinforcement at its fence if it survives the referent test.

Lock line: *A key named for a write reports the write's VERDICT, not its attempt — and a verbatim-copy contract carries a mis-named key's claim exactly as faithfully as it carries a true one.*

<!-- promoted from cpr_mogul_cache_refresh_7ca94b804d78 (birth_tic 744 → /review 747 PROMOTE-as-refinement-ray, Architect-ratified in-tic AskUserQuestion round 1 Q1, recommended option verbatim; every headline re-derived under the row's own method before the question set). Source: mogul:cache_refresh (2026-08-27T154244-tic-744.report.json → results.cache_refresh.EMISSION_DISPOSITION_VERIFICATION; no born file — the queue-row lesson is the authoritative body). Non-derivability adjudicated: the parent and the t718 ray name the two signs of an AGENT-authored key; this ray names a MACHINE-measured key whose name over-claims, and its interaction with the copy-verbatim cure — entailed by neither. Queue: extracted → promoted (queue-lifecycle-writeback.py, row stamp split). Signer ent_homeskillet (claude-fable-5), boot receipt 2fce3c49e2a2fc4f. -->
<!-- reinforced_by: cpr_mogul_memory_mining_4a2b38ee669e (tic 750, mogul:memory_mining b747 — landing_kind=reinforce_existing; ABSORB-as-reinforcement ratified /review 750 round 1 Q1 by ent_breyden, recommended option verbatim, the SIXTEENTH consecutive tic). The referent test MEASURED by the t750 stepper (A3-750): NOT twins by referent (b744 cache_refresh vs b747 memory_mining, different reports) — which is exactly what reinforcement means: a different-referent rediscovery of ONE centroid (an action key written at the decision point records INTENT, not the store's EFFECT, and reads identically in the wrote and suppressed cases). Re-derived at 750: the tic-747 full-cycle report named three ids under signals_emitted — the two biome ids landed 08-24..27 and not 08-28, the cache id in no ledger; the field green for all three; re-observed live at 750 (signals_emitted names construction still). The federation-altitude typing rule (accepted|deduped|rejected as a contract obligation) is DERIVABLE from constitution-ledger#presence-observation-fallacy-guard + this ray until the write-verb census (bk-write-verb-keys-report-write-verdict) measures the generalization — promote a federation ray then, not before. Breadcrumb applied by the seat per the reinforced_by discipline (census 7→8). signer ent_homeskillet-f5 (claude-fable-5), boot receipt 7f106c62eb2e7388. -->

---

## A Currency Field That No Writer Updates Is Structurally Indistinguishable From a Fresh One — a Currency Assertion Must Be Written by the Act That Makes It True
<a id="currency-assertion-must-be-written-by-the-act-that-makes-it-true"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=690 | promoted_tic=693 | relations=composes:internal-memory-entries-and-governance-snapshots-must-carry-explicit-timestamps-(federation — that KI mandates the field EXISTS; this names the emit-side obligation that something must WRITE it),sibling:absorber-needs-per-tic-invoker-or-no-dark-guarantee-lapses,sibling:emitter-rows-must-match-a-reader-predicate,arm:presence-observation-fallacy-guard(federation — observed-success does not prove content-validity),fix-site:bk-agent-status-manifest-currency-writes | confidence_tier=tentative -->

A currency field that no writer updates is structurally indistinguishable from a fresh one, and it will be read as fresh. A registry entry asserting status:current, or a last_validated / last_invoked timestamp, makes a claim about the present that is only true if some act WRITES it at the moment that act occurs. When the field exists in the schema but no code path on the invocation route updates it, the field silently freezes at whatever it was seeded with while the entity it describes keeps running — so the record reports currency precisely because nothing is checking, and staleness is invisible rather than loud. The diagnostic is sharp and cheap: invoke the entity and read its own row back; if the row does not move, the field is decorative. The cure is that a currency assertion must be written by the act that makes it true, or be removed from the schema — an unwritten freshness field is worse than an absent one, because absence prompts a probe and a frozen value suppresses it.

EVIDENCE (n=2, two distinct live offices, each self-demonstrating): (1) surfaced by the civil-engineer at tic 690 — the agent-status manifest covers 19 of 25 specs, ALL 19 report status:current, and civil's OWN entry read `last_invoked_tic: 220` DURING its tic-690 invocation, so the invocation path demonstrably does not write that field. (2) the cpr-stepper reproduced the identical condition against its OWN manifest entry at tic 693: manifest re-read live shows 19 specs, all status=current, `last_validated_tic` distribution {220:14, 222:4, 543:1} — 14 frozen at tic 220, 473 tics stale — and 18 of the 19 entries carry no `last_invoked_tic` key at all while the sole entry that has it is frozen at 220. The CogPR's own diagnostic ("invoke the entity and read its own row back") was executed against the stepper itself, and the row did not move. Six dispatchable offices with live mailboxes (archivist, egress-router, estate-router, homeskillet-gk, office-videographer, unit-narrative-media) carry no validation metadata at all. CONTRAST — the healthy case exists in-tree and is the cure's exemplar: `ladderAuditAdapter.ts:8-12` explicitly refuses to fabricate a recent timestamp and surfaces the gap via stale+unresolved instead.

Composes the federation KI *Internal memory entries and governance snapshots must carry explicit timestamps or TTLs* — that invariant mandates the field EXISTS; this one names the complementary emit-side obligation that some act must WRITE it, without which the mandated field becomes an active suppressor of the probe it was added to provoke.

Lock line: *An unwritten freshness field is worse than an absent one — absence prompts a probe, a frozen value suppresses it; write the currency assertion at the act that makes it true, or delete it from the schema.*

<!-- promoted from cpr_mogul_civil_status_check_1f66077728fe (birth_tic 690 → /review 693 PROMOTE-as-refinement-ray, Architect-ratified in-tic question set round 1, 5/5 as recommended). Source: mogul:civil_status_check cycle (mogul_mandate_id tic-690-20260809T052045; no born file — the queue-row lesson is the authoritative body; origin cycle reports under audit-logs/mogul/cycle-reports/reports/*tic-690*). Second instance re-measured live at tic 693 by ent_cpr_stepper (cpr-step-693) against agent-status.manifest.json; condition NOT resolved. Non-derivability adjudicated at ratification: admitted as a refinement ray on the timestamps/TTL invariant rather than net-new law — the existence mandate and the write obligation are distinct arms. Fix-site bk-agent-status-manifest-currency-writes filed by the /review lead this pass — cited here, not duplicated. Band: STRUCTURAL. lesson_type: unwritten_currency_field. confidence_tier: tentative. signer ent_review_execute (claude-opus-5[1m]), boot receipt e1b9d40a29e59e40. -->

---

## An Evidence Window Over a Shared High-Churn Substrate Is a Global Cursor Wearing a Per-Subject Name — Anchor Evidence to What It Claims
<a id="churn-window-evidence-field-is-a-global-cursor-not-per-subject-provenance"></a>
<!-- ledger-tags: authority_class=review_close_verification | rung=domain | domain=context-grapple-gun | born_tic=720 | promoted_tic=723 | relations=arm:presence-observation-fallacy-guard(federation — evidence-surface axis: a field whose NAME promises per-subject lineage while its window measures global churn is artifact-existence dressed as semantic fulfillment),sibling:a-close-consistency-receipt-is-only-as-strong-as-its-artifact-verification,fix-site:bk-review-close-provenance-evidence-anchor | confidence_tier=tentative -->

An evidence field built from a reverse-chronological `--max-count=N` window over a shared, high-churn substrate evicts the very commit it exists to witness, and under enough churn converges to a CONSTANT across unrelated subjects — so it renders as per-subject provenance while carrying zero discriminating information. The mechanism is structural, not coincidental: when every governance batch rewrites rows bearing every subject id, the newest-first window is a GLOBAL churn cursor wearing a per-subject name, and the OLDEST commit — the subject's birth/inscription commit, the one the field exists to witness — is always the first to fall out. Raising N only delays eviction; the cure is to anchor the evidence to what its name claims: report the birth/oldest commit plus the total count (e.g. `{birth_commit, total_commits, recent_sample}`), which is both stable against churn and actually discriminating per subject.

SCOPE-HONEST APO PERIMETER: this does NOT falsify the verifier's stated claim — the docstring claims only that a non-empty trace proves a real committed artifact, and non-empty remains non-empty, so genuine=0 STANDS and no verdict was wrong. The defect lives in the evidence SURFACE a consumer reads, not in the verdict logic. It is also not a preservation-law defect — but it SPENDS the preservation law: because the window shifts on every batch commit, report content changes on every re-observation even when nothing changed semantically, minting superseded byte-copies whose only delta is the cursor shift (durable artifacts spent on a cursor rather than on signal).

EVIDENCE (measured, not inferred — review-close-check tic 719/720): `_git_pickaxe_hits()` builds `evidence.provenance_trace_commits` as `git log --all -S<cpr_id> --max-count=5`. Three sampled subjects at pickaxe depths 230, 277, and 48 total commits ALL displayed the byte-identical 5-commit window (a59ab71a, c1a6cf05, 7bed4ae4, 4e870ee1, 495dcfd6 — the five most recent queue.jsonl-touching batch commits) while each subject's actual birth commit (1f378df3, f79149fd) sat evicted and invisible. The eviction was caught in flight (a59ab71a displacing 7787e160) by diffing the preserved pre-/review baseline against the post-/review observation. Slice scope: one verifier (`review-close-check.py`), one substrate (`queue.jsonl`), three subjects — system-wide by slice at best.

Lock line: *Anchor an evidence field to what its name claims — the oldest/birth commit plus the total count discriminates; a recency window over shared churn converges to a constant.*

<!-- promoted from cpr_mogul_review_close_check_4db5bb0a46e3 (birth_tic 720 → /review 723 PROMOTE + backlog fix, Architect-ratified in-tic question set round 1, 4/4 as recommended). Source: mogul:review_close_check cycle (mandate tic-719-20260821T100644). Adjudicated as a refinement ray on the Presence/Observation-Fallacy family (evidence-surface axis), not net-new law. Fix-site bk-review-close-provenance-evidence-anchor filed this pass. Band: STRUCTURAL. lesson_type: subject. tier_refusal: exact (class_bleed, ruling review-708). signer ent_homeskillet (claude-fable-5), boot receipt 685a554bec97f9c3. -->

---

## A Text-Recurrence Measure Over a Shared-Generator Corpus Measures the Generator's Repetition Rate, Not the System's Learning
<a id="recurrence-measure-invalid-over-shared-generator-corpus"></a>
<!-- ledger-tags: authority_class=pattern_mining | rung=domain | domain=context-grapple-gun | born_tic=720 | promoted_tic=723 | relations=sibling:guarded-multi-writer-surface-carries-closed-producer-set-obligation(same live event, distinct centroid — that names WHO may write a guarded surface; this names whether the MEASURE is valid over the corpus it reads),distinct_from:guarded-multi-writer-surface-carries-closed-producer-set-obligation,fix-site:bk-pattern-miner-write-boundary-shape-predicate | confidence_tier=tentative -->

A recurrence miner that clusters records by shared TEXT will rank DEFECT BOILERPLATE as the most-reinforced pattern in its corpus — because a generator emitting the same placeholder N times is textually indistinguishable from a system learning the same lesson N times, and the defect is MORE textually identical than any genuine recurrence ever is. The metric does not merely tolerate the artifact; it actively PREFERS it, since real lessons vary in wording while boilerplate is byte-identical. Under this failure the confidence ladder INVERTS: `reinforced` is awarded for the observation COUNT of a defect, so the miner's own confidence vocabulary reads highest precisely where its evidence is worthless. A recurrence measure is only valid over a corpus whose records are independently authored; where records share a generator, recurrence measures the GENERATOR's repetition rate, not the system's learning. The discipline: a miner over machine-generated records must either (a) exclude defect-classed and template-classed text from the clustering corpus, or (b) declare its corpus's authorship-independence assumption in its output so a reader can tell a learned pattern from a generator echo.

APO PERIMETER: distinct centroid from the producer-set law (who may write a guarded surface). This hazard survives intact even if every writer were correctly fenced — the clustering would then simply lock onto whatever template the surviving generator emits. It is a MEASURE-VALIDITY law, not a write-boundary law.

EVIDENCE (measured at tic 720): all THREE patterns `pattern_miner.py` emitted that run clustered on carrier-pointer stub text — pat_74d7624c815cad73 (3 obs, `reinforced`), pat_8828c9be36105345 (2 obs, `tentative`), pat_66f77f4e60dc3c8f (3 obs, `reinforced`); their `observed_in` members are the known stub rows. Two crossed the >=3-observation threshold and were minted as `reinforced` proposals — in the same run in which 63 genuine recurrences were early-gate-suppressed as count-unchanged. Nothing in the corpus recurred; ONE generator defect appeared four times and was reported as the strongest signal found. Slice scope: one miner, one corpus (queue.jsonl), one run — the mechanism is general, the measurement is this slice.

Lock line: *A recurrence measure is valid only over independently-authored records — over a shared generator, byte-identical repetition is the generator's echo, and the measure prefers it.*

<!-- promoted from cpr_mogul_pattern_mining_86066d71a903 (birth_tic 720 → /review 723 PROMOTE, Architect-ratified in-tic question set round 1, 4/4 as recommended). Source: mogul:pattern_mining cycle (mandate tic-720-20260821T113441). Admitted as its own entry — the measure-validity centroid survives independently of writer fencing. Band: COGNITIVE. lesson_type: meta. confidence_tier: tentative. signer ent_homeskillet (claude-fable-5), boot receipt 685a554bec97f9c3. -->

**Refinement — the AGENT-CONSUMER ray: a measure-validity guard that lives in a SCRIPT does not protect the agent-executed lane that runs the same measure — for a consumer with no call site, the guard IS the dispatch text (tic 744 → /review 747, `cpr_mogul_memory_mining_fefd6a73fa3b`; PROMOTE-as-refinement-ray + first-consumer patch in-lane, Architect-ratified in-tic AskUserQuestion round 1 Q3, recommended option verbatim; confidence measured).** The parent's clause (b) — declare the corpus's authorship-independence assumption in the output — is discharged by `pattern_miner.py` as code: at tic 744 it held 6 carrier-pointer-stub lessons out of the clustering corpus and disclosed the exclusion. In the SAME mandate, `memory_mining` — whose entire contract read "scan MEMORY.md chain for recurring patterns, write findings" — was dispatched against the feedback corpus (96 files at 744, 98 at 747), which is wholly shared-generator: one lead, one authoring convention, one voice. A lexical recurrence pass over it returned a 63/96 top cluster — the signature of broad-term over-matching against a shared convention, not recurrence in the world. Nothing in the dispatch named that invalidity, so the lane's default output is a confident pattern claim over a corpus that cannot support one. At 744 the lane declared MEASURE-INVALID by hand and withheld the hit rates; at 747 — the same dispatch text, no guard — it reported a recurrence ("one-surface verification", four feedback lines t742–t746) over the same corpus WITHOUT the invalidity disclosure. n=2, opposite outcomes, one variable: a hand-fired guard is not a guard. This is the /review 743 AGENT-CONSUMER axis (`cpr_mogul_queue_refresh_d9e2a59ba0c6` — the maturity predicate restated as contract text in the queue_refresh dispatch) in its second live instance: a code census cannot close a consumer set containing agents with no call site, and the cure per consumer class is the contract text itself. FIRST-CONSUMER PATCH (this pass, CGG): the memory_mining dispatch paragraph in `mogul-runner.sh` now carries the measure-validity clause — name the corpus's shared-generator status, declare any lexical/thematic recurrence over it MEASURE-INVALID unless an instrument independent of the corpus corroborates it in the same run, withhold hit rates as findings; and the bare-caller invocation of memory-md-audit.py (tic_source resolved in-emitter — the 747 test, PAID non-vacuously, made standing). Slice: the memory_mining lane; whether deep_audit / ladder_audit run measures whose guards live only in sibling scripts is UNMEASURED — the owed census. APO: NOT a pattern_miner defect (its guard works); NOT a corpus-quality claim; NOT generic subagent-masking (composes constitution-ledger#bounded-delegation-surfaces-default-to-masking-bugs-rather-than-surfacing-them — the specific mechanism is the guard's LOCATION).

Lock line: *A guard in a script protects the script's callers; an agent lane has no caller to protect — restate the guard where the agent reads, or the measure runs unguarded and reads as found.*

<!-- promoted from cpr_mogul_memory_mining_fefd6a73fa3b (birth_tic 744 → /review 747 PROMOTE-as-refinement-ray + first-consumer patch in-lane, Architect-ratified in-tic AskUserQuestion round 1 Q3, recommended option verbatim). Source: mogul:memory_mining (2026-08-27T154244-tic-744.report.json → results.memory_mining.recurrence_pass; the n=2 counter-instance 2026-08-28T123506-tic-747.report.json → results.memory_mining.recurrence_mined; no born file — the queue-row lesson is the authoritative body). Non-derivability adjudicated: the parent names the measure's validity condition; this ray names where the guard must LIVE for a consumer class with no call site — sibling to the 743 contract-text cure, not entailed by the parent. Fix landed same pass in mogul-runner.sh (CGG commit cited in the t747 receipt). Queue: extracted → promoted. Signer ent_homeskillet (claude-fable-5), boot receipt 2fce3c49e2a2fc4f. -->


---

## A Guarded Multi-Writer Surface Carries a Closed PRODUCER-Set Obligation — a Refusal Is Scoped to a Gate, Never to a Surface
<a id="guarded-multi-writer-surface-carries-closed-producer-set-obligation"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=720 | promoted_tic=723 | relations=mirror:structural-transform-implies-closed-consumer-set-obligation(federation — the writer-direction arm of the same closed-set enumeration quiver: that law enumerates READERS of a relocated surface, this enumerates WRITERS of a guarded one),sibling:recurrence-measure-invalid-over-shared-generator-corpus(same live event, distinct centroid),composes:ingest-lane-dedup-law(id-keyed dedup guards the miner's OUTPUT ids; nothing guards payload admissibility derived from terminal INPUTS),fix-site:bk-pattern-miner-write-boundary-shape-predicate,fix-site:bk-ingest-path-lifecycle-field-parity | confidence_tier=reinforced -->

A shape-refusal cure installed at ONE intake gate protects only the producers that route through THAT gate — and a queue, registry, or ledger with MULTIPLE INDEPENDENT WRITERS carries a closed PRODUCER-set obligation exactly as a relocated surface carries a closed consumer-set obligation. The enumeration duty runs toward WRITERS, and discharging it at the writer that happened to surface the defect leaves every sibling writer minting the refused shape unimpeded. Knowing the second producer exists does not discharge the obligation — only fencing it does: the decisive evidence class is a cure whose OWN rationale names the sibling writer's harm while the fence is still built only around the writer being patched.

TWO COMPOUNDING MECHANISMS, each generalizable: (1) TERMINAL PAYLOADS ARE RESURRECTABLE BY DERIVATIVE WRITERS — an id-keyed terminal-state valve guards a writer's OUTPUT ids while nothing guards the ADMISSIBILITY of a payload derived from terminal INPUTS; a judgment that terminalized a payload must bind every derivative of that payload, or terminality is only ever local to the row it landed on. (2) DERIVED ROWS INHERIT THEIR SOURCE'S birth_tic, so a derivative writer can mint a row BORN ALREADY-MATURE — silently exempting its own output from the federation's epistemic maturity hold. The civil-engineer widening makes field-parity part of the same obligation: a writer admitted to the surface without the full lifecycle field set (`id_origin`, `tier`, `maturity_tics`, `review_tic`) mints rows born UNSCHEDULABLE — invisible to the very compiler that would otherwise hold them. Admitting a writer means admitting it to the WHOLE lifecycle contract, not just to the append.

EVIDENCE (measured end-to-end, tics 716→720): the carrier-pointer-stub defect was generator-cured at cogpr-ingest (CGG 647505a, `_CARRIER_FIELD` refusal), probe-verified 2/2-refused at 719, its historical instances terminalized at /review 719 — and at 720 `pattern_miner.py` (a second writer with its own `load_queue`/`emit_pattern_envelopes`/`atomic_append_jsonl` into the same queue.jsonl) minted TWO FRESH non-terminal rows of the same stub payload verbatim, stamped `birth_tic: 716` (inherited, zero maturity hold), carrying NO id_origin/tier/maturity_tics/review_tic (schedule_not_resolvable). The cure's own docstring (cogpr-ingest.py:173) had named the miner path explicitly. Both stubs terminal-absorbed at t722. Slice scope: one surface (queue.jsonl), two writers, one defect class traced end-to-end.

Lock line: *Admitting a new writer to a guarded surface re-opens every refusal already earned there — enumerate the producer set, fence each writer, and admit each to the whole lifecycle contract, not just the append.*

<!-- promoted from cpr_mogul_pattern_mining_4805ae1ab6a4 (birth_tic 720 → /review 723 PROMOTE + authorize cures, Architect-ratified in-tic question set round 1, 4/4 as recommended). Source: mogul:pattern_mining cycle (mandate tic-720-20260821T113441); civil-engineer independent corroboration folded into body. Gate-class cures AUTHORIZED this pass as backlog rows bk-pattern-miner-write-boundary-shape-predicate + bk-ingest-path-lifecycle-field-parity (the A1-719 precedent: gate widenings route through /review — this IS that adjudication). Band: STRUCTURAL. lesson_type: meta. confidence_tier: reinforced. signer ent_homeskillet (claude-fable-5), boot receipt 685a554bec97f9c3. -->
---

## A Boot Attestation Demand Must Be Capability-Gated to the Worldview Content the Boot Actually Carries — and Refusal-to-Fabricate Needs a Typed Declination State
<a id="boot-attestation-demand-must-be-capability-gated-to-worldview-content"></a>
<!-- ledger-tags: authority_class=boot_and_worldview | rung=domain | domain=context-grapple-gun | born_tic=720 | promoted_tic=723 | relations=arm:boot-receipt-prescriptions-must-be-capability-gated-to-agent-tool-schema(this is its worldview-CONTENT axis — the sibling gated the TOOL schema an entity boots with; this gates the CONTENT its worldview render carries),arm:presence-observation-fallacy-guard(federation — a demanded attestation the attester structurally cannot ground),fix-site:bk-worldview-ladder-retype-adjudication | confidence_tier=tentative -->

A boot sink that demands an attestation "regenerated from THIS boot's text" from EVERY entity class — while a capped-standing class's worldview render carries NO source content for that attestation at ANY budget — manufactures exactly the failure the attestation lane was built to detect: the demanded content is producible only from memory or general knowledge, which is the copy-forward / locate-don't-rehydrate shape the drift audit exists to catch. This is an EMITTER/READER MISMATCH between the sink's demand and the worldview compiler's per-class output; the prescription must be capability-gated to what the entity's boot actually carries — grant the ray to the class, exempt the class from the demand, or accept-and-mark declination as a first-class receipt state. Two coupled corpus implications: (1) without the gate, the lane manufactures either fabricated entries or silent non-compliance; (2) the explain-back corpus cannot distinguish a seat that carries the ladder THINLY from a seat that was never HANDED it — so some per-office ladder differences may be worldview-PROVISIONING differences, not comprehension differences, corrupting the drift audit at its core crux. A refusal-to-fabricate is the correct per-entity behavior but is currently invisible to the corpus; the honest cure direction is structural (per-class prescription or a typed declination state), never more diligent fabrication.

APO PERIMETER: this entry is NOT a retype verdict on the office-worldview `is_citizen` ladder-explainer gate — the t722 expression raise deliberately went AROUND that gate, and the retype question stays held on its own evidence until the class sweep (citizen/service/invoked renders) reports; adjudication carrier bk-worldview-ladder-retype-adjudication. Nor is it the tool-schema capability-gating sibling — that ray gates what an entity can EXECUTE; this gates what its boot HANDS it.

EVIDENCE (N=3, machine-measured, single-office slices): (1) tic 720 — ent_unit_narrative_media's full render (`office-worldview.py render --max-chars 0`) is 16 lines / 6,311 chars with no ladder content; the office declined to emit rather than fabricate a passing entry into the drift corpus. (2) F1-721 — the expression-lane evaluation located the gate: `office-worldview.py:1219` citizen-gates the LADDER explainer, so every non-citizen standing boots with zero ladder grammar while the sink demands the explain-back on a different predicate. (3) F1-723 — the cpr-stepper measured zero ladder content in its own tic-723 worldview body (`ladder` matching only a peer-office name; rung/hydrat/explain-back/climb all zero). Slice scope: three seats, three tics, all capped-standing or dispatched classes; citizen/service/invoked renders UNPROBED — generalization is the sweep's question, not this entry's claim.

Lock line: *Never demand an attestation the attester structurally cannot ground from its own boot — gate the prescription to the carried content, and make refusal-to-fabricate a typed, corpus-visible state.*

<!-- promoted from cpr_recognized_body_worldview_no_ladder_ray_tic720 (birth_tic 720 → /review 723 PROMOTE scoped + sweep authorized, Architect-ratified in-tic question set round 1, 4/4 as recommended; N raised 2→3 in-docket by stepper F1-723). Source: borns-tic720-recognized-body-worldview-carries-no-ladder-ray.md via cpr-extract-hook; provenance_class friction_born. The is_citizen RETYPE stays HELD pending the class sweep — around, not through. Band: COGNITIVE. lesson_type: process. confidence_tier: tentative. signer ent_homeskillet (claude-fable-5), boot receipt 685a554bec97f9c3. -->

---

## An Instance-Keyed Cure Cannot See Sibling Routes — a Boundary That Over-Admits by Design Owes DISCLOSURE PARITY on Every Route It Cannot Discriminate
<a id="instance-keyed-cures-cannot-see-sibling-routes-disclosure-parity"></a>
<!-- ledger-tags: authority_class=measurement_integrity | rung=domain | domain=context-grapple-gun | born_tic=721 | promoted_tic=724 | relations=refines:reason-coded-genuine-vs-known-verifier-split(the class/instance discriminator applied to the CURE side),arm:presence-observation-fallacy-guard(federation — receipt language claiming class coverage over an instance mechanism is presence-dressed-as-fulfillment),sibling:streak-claims-must-name-their-increment-unit-and-coverage-extent(federation — same instrument, distinct centroid: unit-delta vs admission-route) | confidence_tier=tentative -->

An enumerated DENY-LIST cure against a shape-collision class covers only the instance it was minted for, and its own receipt language makes the class read as cured. At tic 706 a non-id token (`cpr_era_tic_700_749`, an era metadata label) entered review-close-check's inscribed index and moved the counter without an inscription event. The cure was `_RESERVED_REF_PREFIXES = ("cpr_era_",)` with the in-code receipt: "Reserved tokens are excluded at every _CPR_REF_RE consumer and REPORTED as unresolved, never silently admitted." That sentence states CLASS coverage ("non-id tokens are excluded") while the mechanism delivers INSTANCE coverage (one literal prefix). At tic 721 the same class recurred through a route the deny-list is structurally blind to: `cpr_stepper_advance_tic718` — a substring of a mailbox FILENAME cited in a provenance comment's Source: clause — carries no reserved prefix, is not a CogPR id, and was admitted, inflating the token-unit index 738→740 against exactly ONE inscription event.

THE DISCRIMINATOR: does the cure key on a property of the DEFECT CLASS (this token is not a queue id) or on a property of the OBSERVED INSTANCE (this token starts with `cpr_era_`)? Instance-keyed cures cannot see sibling routes, and a receipt written in class language hides that they cannot.

THE HONEST COMPLICATION that makes this non-trivial: the class-keyed test cannot simply replace the deny-list. review-close-check ALREADY computes the general discriminator — `unresolved_against_queue` (47 tokens at the tic-721 pass, and it DOES contain the offending token, verified in-process) — but blanket-excluding that set would destroy real signal, because it legitimately contains historical inscription witnesses whose queue rows predate or left the queue. So the cure is NOT "wire the positive test to gate admission." The cure is DISCLOSURE PARITY: a boundary_rule that over-admits by design must enumerate EVERY admission route it cannot discriminate, not only the one that motivated it. The emitted unit_declaration justified over-admission solely by the sibling-id case — a genuine indistinguishability — but the tic-721 admission was a FILENAME SUBSTRING, neither a sibling id nor indistinguishable. A reader modelling the counter from its own declared rationale would not predict this inflation. The declaration's rationale must cover its mechanism's actual reach, or the disclosure is honest about the wrong thing.

Lock line: *Key the cure on the defect class or disclose every route the boundary cannot discriminate — a class-language receipt over an instance-keyed mechanism is how a cured defect recurs while reading as cured.*

<!-- promoted from cpr_mogul_review_close_check_2b98e954cf02 (birth_tic 721 → /review 724 PROMOTE as NEW cgg-ledger entry, Architect-ratified in-tic question set round 1, 4/4 as recommended; confidence 0.75). Source: mogul:review_close_check (mogul_mandate_id tic-721; no born file — the queue-row lesson is the authoritative body, inscribed verbatim reshaped into entry form). NEW-ENTRY-NOT-RAY: the class-vs-instance cure discriminator + disclosure-parity obligation are not derivable from the verifier-split family (which classifies FINDINGS, not cures) nor from the unit-declaration ray (which names the unit, not the admission routes); recurrence across two distinct routes (t706 prefix, t721 filename substring) is the claim, measured. Sibling cpr_mogul_review_close_check_ea9e46364587 adjudicated the same pass — same instrument, DISTINCT centroids, NO merge. Rider ratified with the verdict: review-close-check's unit_declaration boundary_rule extended to enumerate the filename-substring admission route — post-mandate, after the anchored checker's first production run completed. Queue: status extracted → promoted at maturity (birth 721 + 3 = 724, docket-fenced by review_tic 724). Band: COGNITIVE. lesson_type: measurement_integrity_constraint. confidence_tier: tentative. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 1af0d4e675c9ee0c. -->

---

## A Two-Valued Field Whose Second Value No Producer Ever Writes Is a PROMISSORY DEAD FIELD — a Cross-Instrument Promise Must Exist in Schema, Not Only in Prose
<a id="promissory-dead-field-cross-instrument-promise-in-schema-not-prose"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=723 | promoted_tic=726 | relations=refines:state-agreement-is-not-truth-unless-lifecycle-reachable(federation — the reachability predicate applied to a single field's declared VALUE SPACE, not cross-store state),sibling:can-it-eat-dataflow-liveness-predicate(federation — the lane is live; the FIELD is dead),distinct_from:conductor-score-runtime-parity(federation mechanism 1 — nothing is stripped; the counterpart field never existed on the seal side: a producer field with no consumer, the INVERSE shape),distinct_from:ungoverned-field-schema-reject(federation — the field is typed and admitted; its second value is unreachable) | confidence_tier=tentative -->

A two-valued field that no producer ever sets to its second value is a PROMISSORY DEAD FIELD, and a prose annotation riding beside it that promises closure at another instrument is an undischargeable promise. Measured at the cadence hook instrument pair (tic 723, authoritative stream `audit-logs/hooks/cadence-plan-submit.jsonl`, 580 rows): the interstitial-enter hook writes `plan_submission_claimed: false` 90 times with the note "seal lands at ExitPlanMode," while the seal-side hook carries no such field and shares ZERO identity keys with the promise side (promise rows carry `emission_id`/`work_tic`/`entry_tic`; seal rows carry `plan_hash`; no row carries both). The state `plan_submission_claimed: true` has no producer and is unreachable, and no reader can pair any promise with any seal — including the 490 seals that did fire.

THE TEST is not "does this lane shape downstream motion?" (the lane is live; it wrote this tic). It is two-pronged: (1) is EVERY value in the field's declared state space reachable by some producer, and (2) does the cross-instrument promise the field encodes exist in SCHEMA (a shared correlation key, a typed closure event) or only in PROSE? A promise asserted in a human-readable note across two schema-disjoint record shapes is coupling that cannot be verified, retracted, or expired — it accrues silently, one row per tic, and no consumer can ever detect that it never closed.

Scope-honest: n=1 instrument pair, observer-indexed to the Mogul memory_mining lane; NOT asserted as a global property of federation hook schemas. Explicitly NOT a claim that the 39-day seal silence is a defect — plan mode simply was not entered; incomplete-by-design is not fault. No fix is implied by the law: correlation key, field retirement, or declared-decorative annotation is a separate disposition (backlog `bk-plan-submission-claimed-field-disposition`).

Lock line: *Every declared value needs a producer, and every cross-instrument promise needs a schema — a field state reachable by no writer plus a promise living only in prose is unverifiable coupling accruing one row per tic.*

<!-- promoted from cpr_mogul_memory_mining_2af8faab40e4 (birth_tic 723 → /review 726 PROMOTE as NEW cgg-ledger entry, Architect-ratified in-tic question set, 4/4 as recommended; confidence 0.80). Source: mogul:memory_mining (mogul_mandate_id tic-723-20260821T171021), verified end-to-end against the authoritative stream, not the memory-side mirror. Field disposition deliberately NOT decided here — routed to backlog bk-plan-submission-claimed-field-disposition per the row's own NO-FIX-PROPOSED note. Queue: status extracted → promoted at maturity (birth 723 + 3 = 726, docket-fenced by review_tic 726). Band: STRUCTURAL. lesson_type: refinement. confidence_tier: tentative (inherited from reinforced parents, NOT from the single instance). signer ent_homeskillet-f5 (claude-fable-5), boot receipt d0a4b52207b5c27a. -->

---

## A Loud Counter Installed to Cure Silent-Drop Inherits the Defect's MIRROR — Cure-Time Negative-Space Lists Rot Like Positive Ones, and a Heterogeneous Diagnostic Must Publish Its Population SPLIT BY DISPOSITION
<a id="loud-counter-mirror-overadmission-split-by-disposition"></a>
<!-- ledger-tags: authority_class=measurement_integrity | rung=domain | domain=context-grapple-gun | born_tic=723 | promoted_tic=726 | relations=refines:emitter-rows-must-match-a-reader-predicate(the parent law — its vocabulary-class half is n+1 CONFIRMED here, not re-legislated),sibling:instance-keyed-cures-cannot-see-sibling-routes-disclosure-parity(same over-admission family, distinct centroid: cure-keying vs list-rot + population publication),sibling:churn-window-evidence-field-is-a-global-cursor-not-per-subject-provenance,sibling:a-close-consistency-receipt-is-only-as-strong-as-its-artifact-verification,arm:presence-observation-fallacy-guard(federation),composes:bk-streak-disclosure-population-boundary-convention | confidence_tier=tentative -->

A LOUD COUNTER installed to cure a silent-drop defect inherits the defect's MIRROR IMAGE unless its admission predicate is fenced as carefully as the matcher it watches — an open counter over a closed matcher trades under-admission for over-admission, and the cure's own instrument becomes the next unreadable surface. Measured at tic 723 on `unmatched_provenance_shaped_count` (full scanned-surface set, N=28 comments classified end-to-end, not sampled): only 10 of 28 were real inscription witnesses lost from the index; 18 (64%) were NON-WITNESSES — correctly non-matching, never design-excluded (15 born-CANDIDATE declaration blocks, 3 ledger-tags metadata blocks) — while the design-exclusion list held exactly ONE class (SKIP-heads).

TWO NON-DERIVABLE TEETH beyond the parent law: (1) NEGATIVE-SPACE LISTS AUTHORED AT CURE-TIME ROT LIKE POSITIVE ONES — the parent correctly diagnosed a closed POSITIVE list (the verb alternation) against an open authoring vocabulary, then discharged its cure with a closed NEGATIVE list (SKIP-only exclusion) against the same open vocabulary; enumerating a design-exclusion set from the instances visible at cure-time is the same sin in the reciprocal direction. (2) A CAPPED, UNSTRATIFIED SAMPLE OVER A HETEROGENEOUS POPULATION HIDES THE POPULATION'S STRUCTURE — the capped head-of-list sample showed a 6/4 split reading as "mostly a vocabulary problem" while the actual population is 64% category error, and the single-string remediation line then routes the maintainer at the 36% with no disposition for the 64%. The discipline: a diagnostic counter must publish its population SPLIT BY DISPOSITION (index-loss vs design-excludable vs unclassified), and a remediation string attached to a heterogeneous counter must name EVERY disposition its population contains, or it misroutes the reader it exists to inform.

Scope-honest APO perimeter: this does NOT weaken any /review-723 verdict (genuine=0 STANDS; all 10 lost witnesses are downstream-recovered by the reason-code classifier); NOT a claim the loud counter should be removed — it correctly prevents silent drops; NOT a retype of the SKIP-head exclusion, which remains correct by design. One verifier, one field, one observation; the mechanism is general, the measurement is this slice.

Lock line: *Fence the cure's admission predicate as carefully as the matcher it watches — and publish a heterogeneous counter's population split by disposition, never a single integer plus a head-of-list sample.*

<!-- promoted from cpr_mogul_review_close_check_8698d4d1b9bc (birth_tic 723 → /review 726 PROMOTE as refinement ray on #emitter-rows-must-match-a-reader-predicate, Architect-ratified in-tic question set, 4/4 as recommended; confidence 0.85). Source: mogul:review_close_check. The counter patch (design-exclude candidate/ledger-tags blocks + split-by-disposition output) routed to backlog bk-close-check-counter-disposition-split, NOT executed this pass — the close-check runs on the live 726 mandate. Queue: status extracted → promoted at maturity (birth 723 + 3 = 726, docket-fenced by review_tic 726). Band: STRUCTURAL. lesson_type: meta. confidence_tier: tentative. signer ent_homeskillet-f5 (claude-fable-5), boot receipt d0a4b52207b5c27a. -->

**Refinement — the JURISDICTION-ZERO consumer-carry note (tic 769 → /review 772, `cpr_mogul_review_close_check_1d0125de5ee3`; Architect-ratified in-tic round 1 Q2; the doctrine home is the federation ledger's guard-19 JURISDICTION-ZERO face — this note records the CONSUMER-side obligation on this lane's own surface).** A subset-sum headline over this counter family can go to zero by a RULING moving the partition (design-exclusion set widened 2→3 at /review 769 while population held at 20 and index_loss held 0) — a scope event, not a cure. A CONSUMER of the headline (report row, receipt, handoff sentence) carries (i) the population total, (ii) the exclusion-set cardinality, and (iii) the authority+tic that last changed the set, beside any zero it quotes; the emitter's own disclosure block does not discharge the consumer's carry. Open build half: nothing mechanizes the consumer-side carry today — this note is the ruled record, not the mechanism.
...
**Refinement — the INDEX-CONSEQUENCE face (tic 772 → /review 775, `cpr_mogul_review_close_check_3007b217a33d`; Architect-ratified in-tic round 1 Q3, PROMOTE+cure recommended verbatim).** <a id="a-loud-residue-counters-non-zero-is-typed-by-its-index-consequence"></a> A loud residue counter's NON-ZERO must be typed by its INDEX CONSEQUENCE, not by the shedding event alone. The /review-719 unmatched-provenance counter typed a shed cpr-token-bearing comment as `index_loss` — defined in the instrument's own text as "a REAL inscription witness MISSING from the index" — while that comment's token WAS in the index via another admitted comment (a member of BOTH persisted membership sets that pass): the witness COMMENT was shed; the ID was never missing; the headline reported a loss the index did not suffer, and the disposition's own definition was only half-satisfied. The cure is a discriminator at DISPOSITION time, never a matcher change: split `index_loss` into (a) `index_loss_id_absent` — at least one of the residue comment's tokens is absent from index_tokens, a real hole, the population the counter was minted for (partial presence types id_absent: a missing token IS a hole); and (b) `index_loss_comment_only` — every token already present via another admitted comment, a genuine vocabulary gap worth registering with ZERO index consequence. The sub-typing runs POST-WALK against the COMPLETE index (mid-walk membership would be order-dependent). The counter stays LOUD by construction in both cases; what changes is that its number stops meaning two different things at once. APO: NOT a widening of the design-exclusion set (both subtypes remain counted in the headline); NOT a matcher/vocabulary change (the shed comment stays shed — the split types what the shedding COST); NOT a retype of the /review-768 disposition split (that partitioned witnesses from non-witnesses; this partitions the witness-loss class by consequence). Lock line: *type a loss by what the index actually lost — a shed comment whose ID survives elsewhere is a vocabulary gap, not a hole.* Cure landed same-tic: review-close-check.py `index_loss_subtype_counts` published inside `unmatched_disposition_split`, per-sample `index_consequence` field, `_DISPOSITION_TEXT` re-truthed; fixtures 5/5 both arms + order-independence + partial-presence + loud-in-both (test_review_close_check_content_norm_and_index_consequence_tic775.py). Slice: one lived member at tic 772 (the JURISDICTION-ZERO consumer-carry note above was the shed comment), measured against both persisted membership sets; system-wide by slice at best.

<!-- PROMOTE-AS-REFINEMENT (ray on this entry's disposition-split family), /review 775, promoted from cpr_mogul_review_close_check_3007b217a33d (birth_tic 772, mogul:review_close_check checker-mint, review_tic 775; confidence_tier measured at mint — the membership-set evidence was exact; FIRST-FIRE VALIDITY humility honored at adjudication). Queue: extracted → promoted (landing_kind refinement_ray; adjudicated_at_tic 775; six-field verdict carry). Band: COGNITIVE. lesson_type: refinement. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 9d36125642298027. -->

**Refinement — the CURE-ORDERING face, inscribed in CORRECTED form (tic 774 mint → /review 777, `cpr_mogul_review_close_check_d53f1bf19ee0`; MODIFY+PROMOTE, Architect-ratified in-tic round 1 Q2, recommended option verbatim).** <a id="residue-cure-ordering-by-index-consequence-loss-on-growth"></a> When a governance instrument publishes a residue population split by remedy class, its consumers triage the classes by whether their growth is FIDELITY LOSS — never by their counts — **and loss is established by the INDEX-CONSEQUENCE subtype, not by remedy-class membership**. The mint's real insight, measured over nine check-bearing tics (766–774): wrong_object_class sat flat at 20 and head_anchor_gap at 0 while vocabulary_gap moved 0→1→2→2; count-ranking systematically defers the small class whose deferral could accrete loss, precisely because it is small. THE CORRECTED DISCRIMINATOR: under the INDEX-CONSEQUENCE face directly above (ruled /review 775, live in the instrument since the 776 artifact), a vocabulary_gap member types either `index_loss_id_absent` — a real hole, ACCRETING loss under a live authoring convention, and THIS is the subtype that outranks any count (an id_absent at n=1 outranks a wrong_object_class at n=20) — or `index_loss_comment_only` — every token present via another admitted comment, ZERO index consequence, annotate-only, and it does NOT outrank by loss. MINT-PREMISE DISCLOSURE (checker-mint humility, not retconned): the row as minted asserted "every vocabulary_gap arrival is permanent fidelity loss until a list entry is registered" — a blanket claim its own instrument's subsequent typing refuted for the ENTIRE live population: both live members typed `index_loss_comment_only` in the 776 artifact and again at the fresh 777-entry reading (vocabulary_gap 2, id_absent 0); the mint predates the subtype split's landing and could not have seen it. The ordering axis is inscribed; the blanket premise is corrected against the measurement. What survives from the mint unaltered: a design_excludable member's growth is auditable bookkeeping that no matcher change can fix; the discriminator is loss-on-growth, never movement itself (wrong_object_class grew 19→20 at t769 without loss); and the population is CONVENTION-USE-driven, not tic-driven — its arrival rate is unpredictable from cadence and its cure is always available. APO: NOT 'register the missing verb' (that is one member's typed remedy, not a lesson); NOT a reopening of the closed-list design (the closed list plus a loud counter over its complement is the RULED design; this ray prices and ORDERS the complement); NOT a liveness finding (the counter eats). Slice scope: measured on ONE instrument (review-close-check's inscribed index) over a 9-tic window plus the 776/777-entry subtype readings, observer-indexed to the mogul review_close_check lane; the generalization to other loud counters publishing multi-class residue is PROPOSED, not measured.

Lock line: *Order residue cures by what the index would actually lose — an id_absent at n=1 outranks any count; a comment_only member is annotate-only no matter how it grows; and a class's SIZE is never its RANK.*

<!-- PROMOTE-AS-REFINEMENT (MODIFY verdict — the CURE-ORDERING face on this entry's disposition-split family; head re-opened with the ADMITTED verb at the tic-777 close after the single fire typed this very comment's original MODIFY+-prefixed head index_loss_id_absent — the FIRST id_absent member, cured FIRST per this ray's own ordering, same tic, the rule demonstrating itself on its own mint), /review 777, promoted from cpr_mogul_review_close_check_d53f1bf19ee0 (birth_tic 774, mogul:review_close_check checker-mint, mogul_mandate_id tic-774-review-close-20260905T050646, source_file audit-logs/mogul/cycle-reports/reports/2026-09-05T050649-tic-774.report.json, review_tic 777; confidence_tier measured at mint — the 9-tic window verified by direct artifact reads at adjudication (772: vg1/woc20/hag0 · 773-775: vg2/woc20/hag0) — but the mint's blanket loss premise was ADJUDICATED AGAINST the A3-776 measured evidence: the /review-775-Q3 cure typed both live members comment_only in the 776 artifact and the fresh 777-entry reading (the handoff's '775 AND 776 artifacts' claim corrected here: the 775 artifact predates the subtype split; the split first appears at 776). MODIFY: the ordering axis inscribed with loss re-anchored to the index-consequence subtype; the premise disclosure carried in the body, never suppressed. Composes the INDEX-CONSEQUENCE face directly above (its first consumer-side ordering rule); refines this parent's split-by-disposition discipline with the ORDERING axis its own note said was missing ('which class to cure FIRST when several are non-empty'). The row's named concrete remedy (register 'consumer-carry' in _PROVENANCE_VERB_RE) stays routed as build-lane work, deliberately NOT minted as doctrine, per the row's own note. The row's deferred PLE facet (whether any class's growth should ever read as SIGNAL rather than cost) stays declared-absent. Queue: extracted → promoted (landing_kind refinement_ray; adjudicated_at_tic 777; six-field verdict carry; first live exercise of the A12-776-extended valve). Band: STRUCTURAL. lesson_type: refinement_ray. confidence_tier: measured (the window) / corrected (the premise). signer ent_homeskillet-f5 (claude-fable-5), boot receipt f487b09796f8aae8. -->
...
<a id="observability-embed-and-artifact-dedup-must-not-share-an-unprojected-surface"></a>
<!-- ledger-tags: authority_class=measurement_integrity | rung=domain | domain=context-grapple-gun | born_tic=723 | promoted_tic=726 | relations=refines:recursive-self-observation(federation — the dedup-boundary arm: the observer's own bookkeeping becomes part of the observed),sibling:churn-window-evidence-field-is-a-global-cursor-not-per-subject-provenance(same superseded-copy symptom, SECOND INDEPENDENT CAUSE — curing the cursor did not cure this),composes:a-close-consistency-receipt-is-only-as-strong-as-its-artifact-verification(a dedup that cannot skip produces receipts that look diligent while measuring nothing),fix-site:bk-review-close-streak-embed-defeats-skip-dedup | confidence_tier=measured_single_locus -->

A self-observation field embedded in a dedup-compared artifact — any field that changes BECAUSE observation recurred (a row count over the observation log, a same-tic re-observation counter, a streak embed) — makes the artifact's "unchanged" state STRUCTURALLY UNREACHABLE: every re-observation differs at the embed, the dedup's skip branch can never fire, and a superseded copy is minted per observation regardless of semantic change. MEASURED at review-close-check: the /review-715 `genuine_zero_streak` embed appends its log row on EVERY run including skip-decisions, so `row_count_within_streak` and `same_tic_reobservation_tics` change on each same-tic re-run and the decision is always `replace`; `test_skip_path_preserves_nothing` fails 4/5 identically at CGG HEAD pre-patch (scratch-checkout proof, lane A tic 723).

This is the SECOND independent cause of the symptom the churn-cursor law named (durable superseded artifacts spent on non-signal) — which is exactly why causes must be ENUMERATED, not patched one-and-declared-done. Cure direction (rides `bk-review-close-streak-embed-defeats-skip-dedup`): compute the dedup comparison over an embed-EXCLUDED projection of the artifact, or append the observability row only on non-skip decisions — never let the observer's own bookkeeping be part of the observed.

APO perimeter: NOT a defect in the streak's own unit or computation — the value is correct; the defect is WHERE it lives relative to the comparison projection. NOT the churn-cursor cause (cured t723; this survives the cure, proven pre-existing at HEAD). NOT an argument against observability embeds — only against sharing a dedup surface with them un-projected. Slice: one artifact family, one embed, n=1 proven by test + scratch-checkout — the law generalizes by mechanism, the measurement is this slice. Deferred facet (declared, not fabricated): no systematic sweep for OTHER artifacts carrying self-observation embeds inside deduped surfaces has been run.

Lock line: *Never let the observer's own bookkeeping be part of the observed — a dedup's comparison projection must exclude every field that changes because observation recurred.*

<!-- promoted from cpr_observability_embed_defeats_artifact_dedup_tic723 (birth_tic 723 → /review 726 PROMOTE as NEW cgg-ledger entry, Architect-ratified in-tic question set, 4/4 as recommended; confidence 0.85). Source: gunslinger wave lane A verification (tic 723), born file audit-logs/governance/borns-tic723-observability-embed-defeats-artifact-dedup.md, provenance_class friction_born. THIS ROW IS ALSO THE t723 EXTRACTOR FIX'S n=1 PRODUCTION TEST (F2-724): extracted WITH all six §4 fields + maturity_window_tics 3; the tic-726 stepper advance (extracted → tic_gated) verified all six fields survived the writeback byte-identical (per-field sha256, receipt DONE_cpr_stepper_advance_tic726.json) — the test PASSED live before this promotion terminalized the row. Queue: status tic_gated → promoted at maturity (birth 723 + 3 = 726, un-fenced, the only mature-and-unfenced row at 726). Band: STRUCTURAL. lesson_type: subject. confidence_tier: measured_single_locus. signer ent_homeskillet-f5 (claude-fable-5), boot receipt d0a4b52207b5c27a. -->

---

## A Mechanism Docstring Carrying Both an Empirical Premise and a Structural Rationale Rots at the Empirical Premise — Audit the Premise Separately from the Behavior, and Give a Tic-Keyed Identity a PHASE Axis
<a id="empirical-premise-rots-behind-structural-rationale-audit-premise-separately"></a>
<!-- ledger-tags: authority_class=measurement_integrity | rung=domain | domain=context-grapple-gun | born_tic=724 | promoted_tic=727 | relations=refines:spec-runtime-alignment-by-accident-is-structural-drift-not-victory(federation — the mismatch here is INSIDE one docstring: the false premise survives precisely because the behavior stays right),sibling:conductor-score-runtime-parity(federation — same family, different seam: score-vs-runtime there, premise-vs-rationale within one mechanism here),composes:terminal-state-change-requires-receipt-and-no-signal-goes-dark(federation — the supersession receipt is what made the two same-tic passes distinguishable at all),distinct_from:state-agreement-is-not-truth-unless-lifecycle-reachable(federation — no disagreement to adjudicate: both observations were true when taken; the defect is that the identity model cannot say WHEN),sibling:read-boundary-stamp-on-multi-writer-surface-projections(same t724 window, distinct centroid: identity-phase vs claim-interval) | confidence_tier=tentative -->

One artifact-identity file can hold TWO INCOMPATIBLE PREMISES about what a tic CONTAINS, and the mechanism will still behave correctly — which is exactly why the false premise survives. review-close-check.py asserts both: (A) at the write path, that a tic legitimately hosts MULTIPLE distinct mandates ("N=1 cardinality per tic regardless of how many distinct mandates within the tic invoke review-close-check"); and (B) at the delta-baseline path, that "a same-tic re-observation is the SAME pass re-run, not a new pass." Tic 724 falsified (B) empirically: the cadence mandate's run at 21:02 and the review-close mandate's run at 21:45 observed materially DIFFERENT substrate across the same tic — promoted 716→721, total_cprs 1196→1197, inscribed_index tokens 744→750, matched comments 789→794 — because /review 724's writebacks landed between them. The two runs were not one pass re-run; they were two passes over a MOVING substrate that happen to share a tic label.

THE DURABLE RULE, two coupled teeth: (1) the mechanism gated by (B) — excluding one's own canonical artifact from the baseline search — remains CORRECT, but for the reason stated SECOND in its own docstring (delta stability under re-measurement), not for the empirical premise stated first; when a docstring offers BOTH an empirical premise and a structural rationale, the EMPIRICAL premise is the one that rots, and it rots invisibly because the structural rationale keeps the behavior right — AUDIT THE PREMISE SEPARATELY FROM THE BEHAVIOR. (2) Where a tic can host multiple passes over a moving substrate, a tic-keyed artifact identity carries NO PHASE AXIS: a pre-writeback and a post-writeback observation of the same tic are indistinguishable by name — only the service-log row's decision code plus the supersession receipt distinguishes them, and the streak block's own field name (`same_tic_reobservation_tics`) inherits the false premise as VOCABULARY. A premise that has been falsified continues to propagate through the field names it coined.

APO perimeter: NOT a defect in the exclusion mechanism (its behavior is right; only its first-stated justification is wrong). NOT a spec-vs-runtime gap (both premises live in the runtime's own prose). NOT the read-boundary law's claim-interval defect (that sibling names the truth-interval of a PUBLISHED CLAIM; this names the phase-identity of the OBSERVING ARTIFACT). Slice scope: one verifier file, one tic's dual-mandate window, machine-measured — the mechanism generalizes to any tic-keyed artifact identity over a substrate that /review-class writers mutate mid-tic.

Lock line: *Audit a docstring's empirical premise separately from its behavior — the premise rots invisibly while the structural rationale keeps the mechanism right — and never trust two same-tic observations to be one pass until the identity carries a phase axis.*

<!-- promoted from cpr_mogul_review_close_check_0e6378d96f9f (birth_tic 724 → /review 727 PROMOTE as NEW cgg-ledger entry, Architect-ratified in-tic question set round 1, 3/3 as recommended; confidence 0.86). Source: mogul:review_close_check (t724 dual-mandate window). AgencyReceipt-lite: ratified_by Architect (ent_breyden) · basis: AskUserQuestion round-1 answer "PROMOTE (Recommended)" verbatim · scope: this inscription + queue terminalization only. Dual clamp: cost_of_action = ledger mass for a mechanism-audit discipline that fires rarely; cost_of_inaction = dual-premise docstrings keep rotting invisibly and the next phase-blind tic-keyed artifact misattributes its delta to substrate drift. Queue: status extracted → promoted at maturity (birth 724 + 3 = 727, docket-fenced by review_tic 727). Band: STRUCTURAL. lesson_type: mechanism_premise_drift. confidence_tier: tentative. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 235766d06e9ee44b. -->

---

## A State Assertion Over a Guarded Multi-Writer Surface Is Only True AS-OF a Read Boundary — Without a Read-Boundary Stamp a Snapshot Claim Silently Converts Into a Window Claim
<a id="read-boundary-stamp-on-multi-writer-surface-projections"></a>
<!-- ledger-tags: authority_class=signal_and_queue_manifold | rung=domain | domain=context-grapple-gun | born_tic=724 | promoted_tic=727 | relations=refines:guarded-multi-writer-surface-carries-closed-producer-set-obligation(the READER-direction arm of the same law — that enumerates WHO may write; this names WHEN a reader's claim was true),sibling:disagreement-as-evidence(federation — the misattribution this prevents: a later disagreement blamed on the substrate instead of the missing timestamp),composes:internal-memory-entries-and-governance-snapshots-must-carry-explicit-timestamps-(federation — the timestamp law's multi-writer sharpening: not just WHEN written but ACROSS WHAT WINDOW true),sibling:empirical-premise-rots-behind-structural-rationale-audit-premise-separately(same t724 window, distinct centroid: claim-interval vs identity-phase),adjudicated_distinct:bk-stepper-recompile-clock-stale-in-zero-advance-regime(A1-725 — ruled DISTINCT at /review 727: that is a producer/compiler trigger defect with a structural re-key cure; this is a reader/publisher disclosure law with a stamp cure; same temporal-validity quiver, opposite loci) | confidence_tier=tentative -->

The producer-side law (#guarded-multi-writer-surface-carries-closed-producer-set-obligation) enumerates WHO may write a guarded surface. Its unfilled complement runs the other direction: when a surface has multiple INDEPENDENT RESIDENT WRITERS, the surface is NOT quiescent for the duration of any one reader's mandate window — so a reader that publishes "the queue holds N pending, ID-for-ID" as a governance artifact is publishing a claim whose TRUTH-INTERVAL it never declared. Without a read-boundary stamp, a snapshot claim silently converts into a window claim.

THE FAILURE IS SPECIFICALLY SILENT because the most common concurrent motion is a lifecycle STEP, which preserves the row id and usually preserves the aggregate count while changing the COMPOSITION — so a count-only assertion passes every naive consistency check while describing a state that no longer exists. A downstream consumer (a /review docket build, a next-tic ID-for-ID prediction, a drift audit) then inherits a window claim it will verify as a snapshot claim, and any disagreement it later finds gets MISATTRIBUTED to the substrate rather than to the missing timestamp.

CURE DIRECTION IS DISCLOSURE, NOT EXCLUSION: the reader's duty is not to acquire a lock — locks would serialize independently-legitimate resident writers — it is to STAMP `read_boundary` on any multi-writer-surface projection published as evidence, and where writer motion is observed inside the window, carry it as a TYPED NON-AUTHORSHIP ATTESTATION rather than absorbing it into the snapshot.

APO perimeter: NOT the producer-set law re-stated (that fences writers; this dates readers). NOT A1-725's stale-recompile defect (adjudicated DISTINCT this pass: there the PROJECTION ITSELF is wrong at read time because the compile trigger is write-keyed in a zero-advance regime; here the projection may be perfectly fresh and the published CLAIM still lacks its interval — the cures are a trigger re-key vs a disclosure stamp, and neither substitutes for the other). NOT an argument for locking or serializing the queue. Slice scope: one surface (queue.jsonl), one mandate-window class, derived from the t724 dual-pass evidence — the mechanism generalizes to any projection over a surface with resident concurrent writers.

Lock line: *Stamp the read boundary on every multi-writer-surface projection published as evidence, and disclose writer motion as a typed non-authorship attestation — a snapshot without its interval is a window claim wearing a snapshot's name.*

<!-- promoted from cpr_mogul_queue_refresh_8bf7a0441e81 (birth_tic 724 → /review 727 PROMOTE as refinement ray on #guarded-multi-writer-surface-carries-closed-producer-set-obligation, Architect-ratified in-tic question set round 1, 3/3 as recommended; confidence 0.85). Source: mogul:queue_refresh (t724). MERGE ADJUDICATION RULED IN THE SAME ROUND: DISTINCT from A1-725 (backlog bk-stepper-recompile-clock-stale-in-zero-advance-regime) — sibling edges recorded both directions, the backlog row keeps its own structural cure lane. AgencyReceipt-lite: ratified_by Architect (ent_breyden) · basis: AskUserQuestion round-1 answers "PROMOTE (Recommended)" + "DISTINCT (Recommended)" verbatim · scope: this inscription + queue terminalization + the merge ruling only. Dual clamp: cost_of_action = a small disclosure-field obligation on projection publishers; cost_of_inaction = every cycle-report count claim remains a naive-check-passing window claim and next-tic ID-for-ID predictions inherit misattributed disagreements. Queue: status extracted → promoted at maturity (birth 724 + 3 = 727, docket-fenced by review_tic 727). Band: STRUCTURAL. lesson_type: meta. confidence_tier: tentative. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 235766d06e9ee44b. -->

---

## A Field-Set Mandatory in Doctrine but Merely Permitted at the Write Boundary Decays Silently — a Repair Is a Verdict-Class Write That Pins the Original Clock and Covers Its Own Pass
<a id="mandatory-in-doctrine-permitted-at-write-decays-silently"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=762 | promoted_tic=765 | relations=composes:constitution-ledger#when-consequence-of-error-rises-mechanisms-must-coexist-across-enforcement-layer; sibling:cpr_mogul_review_close_check_e193ae8e2af1(the ATTRIBUTION clause — surfaced this candidate via its own landing_kind read); distinct_from:cpr_mogul_review_close_check_7c6a8f3178f6(the REVISION clause — re-deriving ARMS of a bank vs re-applying a FIELD SET across a pass's own landings) -->

A field-set declared MANDATORY in doctrine but merely PERMITTED at the write boundary decays silently — and its repair pass, un-ruled, repairs the site the finding named while skipping the class and re-dating the clock. Measured at the /review 762 close over queue.jsonl latest-per-id (3,007 rows / sha256-16 3797d45d141ca6c5): the A1-739 ruled terminal writeback set {adjudicated_at_tic, absorbed_into, landing_kind} — declared 'MANDATORY' and 'apply same-pass' at /review 741 Q4 — lives ONLY in queue-lifecycle-writeback.py's LIFECYCLE_MUTABLE_FIELDS (the PERMITTED set), with zero required-set checks anywhere in the writer; the string 'MANDATORY' occurs exactly once in that file, inside a comment. Adjudication tic 761 carries ZERO stamped rows — the first tic nobody hand-carried the set is the measurement. The A1-762 repair then produced all three failure faces at once: (i) the CLASS skipped — the repairing pass's own two landings both landed bare; (ii) the SIBLING row of the repaired row, adjudicated in the same 761 pass, left bare; (iii) the CLOCK moved — the repair re-stamped adjudicated_at_tic AND review_tic 761 → 762 while the row's ratification line still reads 761, so the field introduced precisely to stop review_tic being overloaded by verdicts was itself overloaded by a repair write.

THE CLASS IS PREDICATE-INDEXED (A1-764, the evidence face): 'the bare rows' reads 2, 3, or 5 depending on the predicate — the /review 762 bare PROMOTEs (n=2), the no-field-at-all sub-shape (n=3), or the full era-scoped latest-per-id class (n=5, adding two FIELD-SELECTIVE rows that stamped the clock and skipped the kind). A ruling made on the smallest reading rules over 40 percent of the population it means; name the predicate before naming the count.

PRESENCE IS NOT SUFFICIENCY (A16-764, the second evidence face): a field-presence gate passes a value-level mis-write — the /review 764 first-stamps carried the PREVIOUS tic's verdict text and ratification line with every ruled field present and correct. A required-set check is necessary and is NOT the whole cure; value-level narration defects need their own detector, and the two gates are orthogonal.

THE RULE: a repair of an unenforced mandatory field is a VERDICT-CLASS write, so it owes (a) the ORIGINAL adjudication clock pinned explicitly — never the current tic merged in — and (b) the same field set applied to every row the repairing pass itself lands; otherwise the repair buys one cured row at the cost of one falsified clock and n fresh omissions. The durable cure is physics, not memory: a set whose compliance depends on the caller remembering it reads ZERO at the first tic nobody hand-carries it.

APO perimeter: NOT a claim the A1-762 repair was wrong to fire (the field it restored was genuinely missing and is present); NOT a proposal to widen LIFECYCLE_MUTABLE_FIELDS (the permitted set is correct; what is absent is a required-set check at the verdict branch); NOT the --review-tic re-stamp CLI shape alone (that shape is known at this seat — the measured addition is the overload of the anti-overload field itself); NOT the REVISION clause (that re-derives arms of a bank; this re-applies a field set across a pass's own landings); NOT body-preserving-writeback (#terminal-writebacks-must-be-body-preserving — that law protects the ENVELOPE from thin rows; this law makes the ruled FIELD SET physically required).

Slice: the /review verdict writeback path at the canonical seat, adjudication tics 739–765; observer-indexed to the mogul review_close_check cycle; NOT asserted over every federation field contract. The TEL blast-radius facet stays declared-absent from mint: landing_kind consumers beyond review-close-check.py:3389 were not enumerated.

Lock line: *a mandatory set the writer does not require is a permitted set wearing doctrine's name — and its repair pins the ruling's clock, never the repair's.*

Dual clamp: cost of action one required-set check at the writeback's verdict branch plus an explicit adjudication-clock pin on repair writes; cost of inaction a mandatory set that reads ZERO whenever nobody hand-carries it, plus repaired rows whose adjudication clock names the repair tic instead of the ruling tic.

<!-- promoted from cpr_mogul_review_close_check_284cdbf58189 (birth_tic 762, mogul:review_close_check, mogul_mandate_id tic-762-20260902T233619, source report audit-logs/mogul/cycle-reports/reports/2026-09-02T193632-tic-762.report.json, review_tic 765 → /review 765 PROMOTE as NEW ANCHOR + same-pass applications, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the THIRTY-FIRST consecutive tic; confidence measured at mint, upheld at adjudication w/ the stepper's independent re-measurement (writer byte-unchanged since 2026-08-28, 'MANDATORY' once in a comment at line 160, zero required-set checks). Applications ratified same-pass: required-set check at the writeback verdict branch (CGG batch t765); six back-repairs w/ ORIGINAL clocks pinned — b736 +landing_kind refinement_ray (adj 739), b737 +refinement_ray (adj 740), df535e0b7d15 +adj 762 +refinement_ray, 7c6a8f3178f6 +adj 762 +refinement_ray, a83a882105fa +adj 761 only (no skip-class landing_kind vocabulary exists in the corpus — disclosed, not invented), 496b8fe3085b landing_kind reinforce_existing → refinement_tail (typed from its own ABSORB-as-refinement-tail verdict; new value disclosed). Queue: status extracted → promoted through queue-lifecycle-writeback.py. Band: STRUCTURAL. lesson_type: refinement_ray. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 1f15c7b2d2b20a83. -->

---

## The scripts/lib Import-Form Contract — the State-Sharing Guarantee Lives on the SHARED MODULE, Never the Caller: Self-Inserting-Lib Ratified as the House Pattern
<a id="scripts-lib-import-form-contract-self-inserting-lib"></a>
<!-- ledger-tags: authority_class=pipeline_and_integration | rung=domain | domain=context-grapple-gun | born_tic=775 | promoted_tic=776 | relations=composes:#guarded-multi-writer-surface-carries-closed-producer-set-obligation; sibling:constitution-ledger#ungoverned-field-schema-reject(admission-timing kin — a convention adopted before its contract exists becomes load-bearing silently); distinct_from:#boot-seam-duality-primary-sessionstart-citizens-subagentstart(two wirings of one hook vs three import forms of one module) -->

RULED /review 776 on the wave-12 evidence (F-775-W12-1, handed up as OM-W12-2): four contract-guarded writers reach the `scripts/lib` shared surface by THREE different import mechanisms — the direct `from lib.x` form, the PEP-420 namespace form with only `scripts/` on `sys.path` (cogpr-ingest:331, a governed birth writer), and `spec_from_file_location` — and because `lib/` is a PEP-420 namespace package, the same file can become TWO DISTINCT MODULE OBJECTS with independent module-level state when callers resolve it through different `sys.path` parents. Independent state in a contract-guarded engine is a silent-fork hazard: two guards, one name, divergent tables.

THE RULING — the contract sits on the SHARED MODULE, not the caller: every `scripts/lib` module consumed by more than one writer SELF-INSERTS its own directory onto `sys.path` at module top, so ONE module object serves every caller regardless of import form. The three caller forms remain LAWFUL as typed coexistence — a caller's import form is free, because the module's self-insertion makes the forms converge on one object. This is the mechanism wave 12 proved live (the migrated tier engine: one engine object under all three forms, pinned by two subprocess tests, engine sha unchanged) and the de-facto majority the codebase already runs (cockpit_intent_emit, dsn_fragment, fragment_receipt, receipt_horizon_guard, load_doctrine_chain).

REFUSED, with reasons: (1) `lib/__init__.py` — converting the namespace package to a regular package is a global import-semantics change with an unmeasured blast radius over every live `from lib.` form and `spec_from_file_location` caller, and it does NOT by itself guarantee a single module object under divergent `sys.path` parents — it cures the name, not the state. (2) Migrating the four writers to one form — touching a governed birth writer's import is exactly the breakage class wave 12's P3 falsification caught: a premise-faithful migration would have broken cogpr-ingest WHILE PYTEST STAYED GREEN (the populated-path mask). A ruling that requires zero writer-code changes and rides the proven mechanism outranks both.

APO: NOT a license for new lib modules to skip the pattern (a NEW multi-writer module adopts self-insertion at birth — the contract is prospective and binding); NOT a claim single-writer modules must self-insert (the hazard is multi-caller state divergence; single-caller modules are out of scope); NOT an import-style preference (the ruling is about OBJECT IDENTITY, not aesthetics); NOT a retype of the closed-producer-set obligation (that closes the WRITER set over a guarded surface; this closes the MODULE-OBJECT set over a shared engine).

Slice: the CGG `scripts/lib` surface, four writers and five self-inserting modules measured at wave 12 (tic 775); other estates' lib conventions unmeasured.

Lock line: *one shared module, one module object — the caller's import form is free because the module self-inserts; a state-sharing guarantee delegated to callers is no guarantee at all.*

Dual clamp: cost of action one convention line enforced at new-module birth (existing majority already complies, zero writer changes); cost of inaction a contract-guarded engine silently forks into two module objects with divergent state the first time a caller's sys.path shape differs, invisible to pytest.

<!-- RULED /review 776 (decision item OM-W12-2, non-queue — handed up by the wave-12 build citizen in cable-receipt bk-off-enum-drift-field-generic-writer-topology-B2-wave12-tic775.json; no CPR row, so no queue writeback rides this entry). AgencyReceipt-lite: ratified_by Architect (ent_breyden) · basis: AskUserQuestion round 1 Q2 "RATIFY self-inserting-lib (Recommended)" verbatim — the FORTY-SECOND consecutive round-1 set · scope: this house contract + the two typed refusals + OM-W12-4 unblock; does NOT authorize touching any writer's import form. Dual proof carried from wave 12: structural pins across all three live import forms (two in subprocesses) + 585/585 member-exact equivalence. Consequence: OM-W12-4 (tier contract minting_authority key) stages next as a seat DATA edit gated on /review. Band: STRUCTURAL. lesson_type: house_contract_ruling. signer ent_homeskillet-f5 (claude-fable-5), boot receipt e94fb73a9fb363c8. -->

---

## A Route-Attribution Coverage Statement Measures BINDING, Never CORRECTNESS — a Negative-Fact Route Is Cross-Checked Against the Sibling Counter That Measures That Same Fact Before It May Bind
<a id="route-coverage-measures-binding-not-correctness"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=777 | promoted_tic=780 | relations=refines:the-ATTRIBUTION-clause(/review-753 — attribution owes the FIRED member; this adds that it also owes the member's VERIFICATION where the route asserts a negative fact); composes:the-INDEX-CONSEQUENCE-split(/review-775 — that split produced the very measurement that falsifies this binding); sibling:constitution-ledger#disagreement-as-evidence; distinct_from:constitution-ledger#presence-observation-fallacy-guard(guard 19 types VACUOUS agreement; this types a FALSIFIED binding under NON-vacuous disagreement) -->

A route-attribution catalog that publishes a COVERAGE statement (N/N divergent members bound to a route) measures BINDING, never CORRECTNESS — and when one of its routes asserts a NEGATIVE FACT ("this promotion landed no provenance comment"), a sibling counter on the SAME artifact may already hold the measurement that falsifies the binding. Before a coverage statement stands as evidence, every bound route whose text asserts a negative fact must be cross-checked against the sibling counter that measures that same fact; a nearest-neighbor binding onto a falsified route is WORSE than an honest `uncovered`, because coverage renders the member as RESOLVED and the divergence lane goes quiet while the residue lane is shouting.

LIVED AT TIC 777 (the t778 entry fire's SELF-CATCH — the checker critiquing its own attribution): promoted +2 / tokens +1; cross_counter_disclosure bound member d53f1bf19ee0 to `modify_and_merge_promotion_adds_no_provenance_comment` ("lands no new provenance comment") and reported coverage 1/1 — while the SAME artifact's inscribed_index_unresolved measured that member's provenance comment as PRESENT, token-bearing, and SHED by the matcher (remedy_class vocabulary_gap, index_consequence index_loss_id_absent — the FIRST id_absent fire since the /review-775 INDEX-CONSEQUENCE split was built). Verified on disk in the same run: the comment exists in this ledger head-anchored on `MODIFY+PROMOTE-AS-REFINEMENT`, the token occurs twice, and the membership sets place the member in promoted_ids while ABSENT from BOTH index_tokens and matched_comment_ids. Neither counter is wrong in isolation; the defect is the unconsulted sibling plus a coverage metric that reads binding as resolution.

CURE, landed same-pass (/review 780 Q2, the tic-780 CGG batch; NC bank audit-logs/governance/nc-predictions-tic780-coverage-binding-cure.json, suite 1,525 == 1,520+5 PREDICTED EXACT run 1): the SIXTH divergence route `promotion_witness_comment_shed_by_matcher` registered in `_DIVERGENCE_ROUTES` (append-only, indices stable); `build_inscribed_index` publishes the COMPLETE shed-comment token membership (`index_loss_member_tokens` — a set beside the count, never a capped sample); and the precedence gate in `compute_cross_counter_attribution` consults it BEFORE any negative-fact binding — an index_loss typing from the residue counter OUTRANKS the catalog route asserting comment absence for the same member; where neither binds, the member reports `uncovered` and the coverage denominator carries the honest hole. The falsifying input was already computed on the same artifact, so the cure added no new measurement. Consumer set enumerated BEFORE the run: the two exact-cardinality pins (tic753/tic754 `len(ROUTES)==5`) were adapted 5→6 per their own documented intent in the same change set — the /review-775 pin-adaptation precedent converted from run-time discovery to composition-time adaptation.

APO: NOT a hazard finding (genuine=0 held at t777; this is an attribution-quality defect, not a missing inscription); NOT a claim the t777 promotion failed (the promotion landed and the ledger carries it — only the INDEX witness was lost); NOT guard 19 (that family types VACUOUS agreement over empty observation sets; this types a FALSIFIED binding under non-vacuous disagreement); NOT a widening of index admission (the gate touches only the attribution's binding order; token admission is byte-unchanged).

Slice: observed on ONE member at ONE pass (tic 777, n=1) on the review_close_check artifact; scoped to attributing counters that publish BOTH a route catalog and a coverage statement — not generalized to all counters until measured on each.

Lock line: *coverage counts bindings; only the sibling counter that measures the asserted fact can make a binding true — cross-check every negative-fact route, and prefer an honest uncovered to a falsified resolved.*

Dual clamp: cost of action one route registration plus one precedence gate in the attributor (the falsifying input already computed on the same artifact, no new measurement); cost of inaction every future modify/merge promotion whose witness comment is shed reads as "no comment was written" at coverage 1/1 — the index hole stays invisible to the divergence lane while the residue lane shouts it, and the two lanes drift into permanent disagreement nobody reconciles.

<!-- promoted from cpr_mogul_review_close_check_4cb469459489 (birth_tic 777, mogul:review_close_check, mogul_mandate_id tic-777-20260905T184847, review_tic 780 → /review 780 PROMOTE + same-pass cure — the COVERAGE-IS-BINDING face, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FORTY-SIXTH consecutive round-1 set; confidence 0.85, the lived instance re-verified from the t777 artifact at adjudication). This row is the t778 entry fire's SELF-CATCH: the checker minted the critique of its own attribution. Typed consequence banked and expected: the tic-780 close fire stamps a producer_identity differing from the entry fire's — the INSTRUMENT-IDENTITY face's FIRST LIVE DISCRIMINATION case, lawful under the ratified writer cure. Queue: status extracted → promoted through queue-lifecycle-writeback.py w/ the valve's guard-set carry. Non-derivability: the ATTRIBUTION clause owes the fired member and the INDEX-CONSEQUENCE split types the residue; neither entails the cross-check obligation on negative-fact routes nor the precedence rule. Band: COGNITIVE. lesson_type: refinement_ray. signer ent_homeskillet-f5 (claude-fable-5), boot receipt b15cc70a86ad4d72. -->

---

## A Multi-Block Artifact Assembled Across One Run Publishes a Per-Block Read-Instant — an Artifact-Level Timestamp Cannot Keep a Self-Contradiction From Looking Like One
<a id="blocks-measured-at-different-instants-publish-their-own-read-instant"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=778 | promoted_tic=781 | relations=refines:constitution-ledger#internal-memory-entries-and-governance-snapshots-must-carry-explicit-timestamps-(the artifact-level timestamp law; this names the WITHIN-RUN per-block obligation it cannot express); composes:constitution-ledger#structural-transform-implies-closed-consumer-set-obligation(the ADDRESSING-axis ray, /review 777 — the divergence declared where the consumer reads, applied to a TEMPORAL rather than spatial divergence); composes:the-MEASUREMENT-vs-OCCURRENCE-face(/review 775 — a read-instant is occurrence-class BY TYPE, so the cure's instants are comparison-excluded under that face's own discriminator, never ad hoc); sibling:cpr_mogul_review_close_check_4cb469459489(the n=1 instance of the class, entry fire, /review 780); distinct_from:constitution-ledger#disagreement-as-evidence(no two counters disagree — one artifact contradicts itself across time) -->

A multi-block governance artifact whose blocks are measured at DIFFERENT INSTANTS of one run must publish a per-block read-instant INTO the artifact — otherwise a consumer with no call site reads the blocks as simultaneous and finds a FALSE contradiction between two fields that are each TRUE at their own write-time. An artifact-level `generated_at` cannot express intra-run ordering: it governs whether the SNAPSHOT carries a timestamp, not whether blocks measured at different instants of ONE run each carry THEIR OWN. And a disclosure of the ordering that lives only in a function docstring is addressed to a reader of the CODE — the consumer of the JSON artifact has no call site, so the disclosure must ride where the consumer reads, at the block's own altitude.

MEASURED LIVE at tic 778 on review-close-check.py's emitted artifact: `prior_same_tic_observation.earlier_preserved_same_tic_artifacts: []` beside `superseded_receipt.preserved_path: .../tic-778-check.superseded-1.json` — one field says no earlier same-tic fire is preserved while another names exactly such a file, BOTH TRUE, because the list was computed BEFORE the write block performed that same run's preservation. The instrument already modeled the correct pattern in ONE of its thirteen top-level blocks: `queue_state_tuple` publishes a `read_at` and a by-construction blind-spot sentence; the remaining twelve published neither, and the ordering was disclosed only in a docstring. RECURRENCE, not a one-off: the SECOND instance of this class on this instrument in consecutive fires (the t778 entry fire caught the same shape in tic-777-check.json — cross_counter_disclosure binding a member to "adds no provenance comment" while the SAME artifact's residue counter measured that comment present; n=2, same class, different block pair).

CURE, landed same-pass (/review 781 Q2, the tic-781 CGG batch; NC bank audit-logs/governance/nc-predictions-tic781-read-instant-cure.json): every own-read block gains a `read_at` minted by one shared helper (`_block_read_instant`) — the same-tic prior-observation block (the lived contradiction site), both delta blocks, both attribution blocks (via the shared unresolved skeleton so every path carries it), the index walk's unit declaration, and the genuine-zero streak — with `queue_state_tuple` untouched as the model; the ordering clause moved from the docstring INTO `_SAME_TIC_OBS_NOTE` (append-only, prefix byte-identical); and the sole live consumer, the skip-vs-replace predicate, was adapted IN THE SAME CHANGE SET with a recursive `read_at` strip (`_strip_read_instants`) — a read-instant is occurrence-class BY TYPE under the /review-775 discriminator, so re-observing can never manufacture its own supersession evidence, and the recursive-by-key-name shape refuses the forward-decay of a per-block registration list. BANK OUTCOMES, honest: suite 1,530 == 1,525+5 exact — on RUN 2, not run 1; THREE consumer pins fired at run 1 (same-tic content-identity tests comparing emissions for equality) because the composition-time enumeration grepped cgg-runtime/tests/ + tests/ while the named invocation's population is cgg-runtime/scripts/ — the pins were adapted per their own documented intent through the cure's own strip, and the enumeration-scope miss is recorded in the bank as the t779 pin lesson's SCOPE clause: enumerate consumer pins over the population the NAMED INVOCATION collects. FENCED OUT, honored: the same-tic block's path-citation composition (THE MUTABLE-ADDRESS face, cpr_mogul_review_close_check_55eb70c0a49b) adjudicates at ITS docket (783) — this cure adds instants and the ordering disclosure only.

APO: NOT a claim the decomposition is wrong (it is member-exact — the t778 entry fire fully named with its own +1/+1 delta, the close fire's +2/+2 zero-residual); NOT a superseded_receipt defect (the /review 747 APO holds — the receipt stays the write block's to emit, and this entry does not move it); NOT a docstring-correctness finding (the docstring is accurate, and mis-addressed for the artifact's consumer); NOT derivable from the internal-timestamps invariant alone (artifact-level staleness vs intra-run block ordering); NOT #disagreement-as-evidence (no two counters disagree — one artifact contradicts itself across time).

Slice: measured on review-close-check.py's emitted artifact at tic 778 (n=2 instances of the class on this one instrument), observer-indexed to multi-block governance artifacts assembled across a run with intervening writes; not asserted globally over all JSON emitters.

Lock line: *within one run, blocks measured at different instants each publish their own read-instant — an artifact-level generated_at cannot keep a self-contradiction from looking like one.*

Dual clamp: cost of action one note clause and one read-instant field per block in an existing read-only emitter, plus the typed comparison-exclusion (zero counter semantics move); cost of inaction a consumer auditing the artifact for internal consistency finds a false contradiction and either escalates a non-defect or, worse, learns to discount this instrument's self-contradictions — which is exactly how the tic-777 REAL contradiction would have been missed.

<!-- promoted from cpr_mogul_review_close_check_c509ebc8101d (birth_tic 778, mogul:review_close_check, mogul_mandate_id tic-778-20260905T193629, review_tic 781 → /review 781 PROMOTE + same-pass cure — THE READ-INSTANT face, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FORTY-SEVENTH consecutive round-1 set; the contradicting pair re-verified from the row's own text at adjudication under checker-mint humility). Cure landed same-pass (CGG batch t781): per-block read_at via one shared mint + ordering clause into _SAME_TIC_OBS_NOTE (append-only) + recursive occurrence-class strip in the skip-vs-replace predicate; NC bank with the run-1 pin falsifications and the enumeration-scope miss RECORDED, not smoothed; suite 1,530 == 1,525+5 exact run 2. Typed consequence banked and expected: the tic-781 close fire stamps producer_identity 7850a5b9dd87fdcd != 4e0b7fda2f52cf29 — lawful typed instrument boundary under the ratified cure (the INSTRUMENT-IDENTITY face); an EQUAL hash would now be the finding. Queue: status extracted → promoted through queue-lifecycle-writeback.py w/ the valve's guard-set carry. Non-derivability: the internal-timestamps invariant governs artifact-level staleness and the addressing-axis ray governs WHERE a divergence is declared; neither entails the per-block WITHIN-RUN instant obligation nor the occurrence-class comparison-exclusion that makes it safe to publish. Band: COGNITIVE. lesson_type: instrument_reading_discipline. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 89c48c30ced331dc. -->

---

## An Antecedent Named by a MUTABLE Address Is Not Named — Address Selection Happens at COMPOSITION, Never as a Post-Write Patch
<a id="an-antecedent-named-by-a-mutable-address-is-not-named"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=780 | promoted_tic=783 | relations=refines:constitution-ledger#terminal-state-change-requires-receipt-and-no-signal-goes-dark(the ADDRESSING half beside the inscribed RECEIPT half — a receipt that names its antecedent by an address it overwrites lets the signal go dark at the moment of supersession); sibling:the-SELF-INVALIDATING-POINTER-face(economy contract, /review 763 — same shape, different invalidator: there a LATER EXTERNAL ruling expires the address; here the invalidating write IS the naming run's own write, same instant, no external actor); composes:constitution-ledger#structural-transform-implies-closed-consumer-set-obligation(the block's readers are the closed consumer set a supersession silently obligates); sibling:cgg-ledger#blocks-measured-at-different-instants-publish-their-own-read-instant(the READ-INSTANT face fixed the TEMPORAL disclosure half of this same block at /review 781 and fenced THIS half out to its own docket; this is that half, adjudicated at its round) -->

When an instrument publishes a block whose meaning depends on a prior observation, the antecedent must be addressed by an address the publishing run itself cannot invalidate. A block that names its antecedent by the LIVE tic-keyed path and then — in the same run — overwrites that path has not named its antecedent at all: the address resolves to the successor the moment the run completes.

MEASURED LIVE at tic 780 on review-close-check.py's `prior_same_tic_observation`: the block identified its antecedent as `artifact: tic-780-check.json` — the live path — and the same run's write block then overwrote that path with its own content. Every published VALUE was correct (prior_tokens 883 / prior_matched_comments 929 / prior_generated_at all independently verified against the preserved copy) and the preservation itself was fully receipted (justification_class superseded_by_same_tic_reobservation, both producer identities stamped) — but a consumer following the NAMED address to verify 883 reads 885, and the additive decomposition (0 + 2 = 2) appears to fail arithmetic to anyone re-deriving it from the artifact alone. It fails SILENTLY: the consumer gets a plausible wrong number, never an error. The block even carries the field that would fix it — `earlier_preserved_same_tic_artifacts` — and it ships EMPTY on exactly the fire where it is needed, because it is computed BEFORE the preservation step that creates the durable path. PAR, held: the block is simultaneously honest and unusable-as-addressed — the selector `same_tic_live_artifact_pre_overwrite` truthfully declares the temporal qualifier, and declaring that your pointer will rot is not the same as making it durable.

CURE, landed same-pass (/review 783 Q2, the tic-783 CGG batch; NC bank audit-logs/governance/nc-predictions-tic783-expiry-forecast-and-mutable-address-cures.json): address selection at COMPOSITION — `_read_same_tic_prior_observation` now emits, beside the live-path handle, an `antecedent_durable_address` computed at the block's own read-instant: the antecedent's content sha256-16 + its generated_at (the unconditional durable address the publishing run cannot invalidate) + the preserved path this run's own write block WILL create if it supersedes, projected under the write block's exact naming (`superseded/<stem>.superseded-<seq>.json`, first free seq at composition), with the condition DECLARED (a skip-branch fire leaves the live path valid and the projection unconsumed — never a fabricated preservation claim). Composition-order/additive only: no counter semantics move, no baseline is re-selected, `_SAME_TIC_OBS_NOTE` extends append-only, and the skip-vs-replace strip is untouched. LIVE-ARM HONESTY: the tic-783 close fire is the tic's FIRST fire (ODD single-fire shape, present=false), so the cure's POPULATED arm is fixture-proven and its first live exercise waits for the next two-fire tic — fixture-green is not live-green, declared.

APO: NOT a data-correctness finding (every published value was right); NOT a preservation failure (the supersession copy exists with a real receipt — the terminal-state RECEIPT obligation is honored in full; this is the ADDRESSING half that obligation leaves unstated); NOT the SELF-INVALIDATING-POINTER face (there the invalidation is a later external event; here the invalidating write is the naming run's own); NOT a missing-reverse-link claim (the .superseded-by.json sidecar carries preserved→live correctly; the gap is DIRECTIONAL, forward-only — consumer → antecedent); NOT a dishonesty finding (the temporal qualifier is truthfully declared).

Slice: observed on review-close-check.py's prior_same_tic_observation block at tic 780 — the first same-tic re-observation to run under a mid-tic producer change. The class is any tic-keyed artifact writer that names a prior observation by its live path; the same supersede-and-preserve convention runs on the mandate cycle-report lane and the visitor-economy full-cycle lane. Observer-indexed to this instrument; NOT asserted globally.

Lock line: *an antecedent named by a mutable address is not named — emit the durable address (content hash + generated_at, and the projected preserved path) at composition, never as a post-write patch.*

Dual clamp: cost of action a composition-ORDER change inside one emitter (compute the durable address before emitting the block; no counter semantics move); cost of inaction every even-tic two-fire artifact ships an antecedent pointer resolving to its own successor, and the additive-decomposition audit becomes un-re-derivable from the artifact alone — failing silently, a plausible wrong number instead of an error.

<!-- promoted from cpr_mogul_review_close_check_55eb70c0a49b (birth_tic 780, mogul:review_close_check, mogul_mandate_id tic-780-20260906T003102, source report audit-logs/mogul/cycle-reports/reports/2026-09-06T003222-tic-780.report.json, review_tic 783 → /review 783 PROMOTE + same-pass cure — THE MUTABLE-ADDRESS face, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FORTY-NINTH consecutive verbatim set; the lived instance re-verified from the row's own text at adjudication under checker-mint humility; the row is the face the /review-781 READ-INSTANT cure explicitly fenced out to its own docket — the fence honored, the adjudication arriving at its round). Cure landed same-pass (CGG batch t783): antecedent_durable_address at composition (content sha256-16 + generated_at + projected preserved path under the write block's own first-free-seq naming, condition declared) + _SAME_TIC_OBS_NOTE extended append-only; NC bank composed pre-cure, zero pin falsifications predicted. Typed consequence banked and expected: the tic-783 close fire stamps a producer_identity differing from 0efdf8cf56ae76ad — the CURE branch of the doubly-conditional producer-identity bank, lawful typed instrument boundary; per the baseline-instrument-stability clause every compared arm's counting semantics are byte-identical across the cure boundary (additive fields only). Queue: status extracted → promoted through queue-lifecycle-writeback.py w/ the valve's guard-set carry. Non-derivability: the terminal-state receipt law obligates the RECEIPT and the READ-INSTANT face obligates the per-block INSTANT; neither entails the addressing rule that the antecedent's address must survive the publishing run's own write. Band: STRUCTURAL. lesson_type: addressing_discipline. signer ent_homeskillet-f5 (claude-fable-5), boot receipt be9595cda38f90d2. -->

---

## A Timestamp Compared Across an Unnamed Clock Frame Is Not a Staleness Verdict — Name Both Operands' Frames or Resolve by a Frame-Free Operand
<a id="a-timestamp-compared-across-an-unnamed-clock-frame-is-not-a-staleness-verdict"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=781 | promoted_tic=784 | relations=refines:constitution-ledger#terminal-state-change-requires-receipt-and-no-signal-goes-dark(the RENDERING-RERUN discipline's staleness test — re-read-not-re-execute turns on a staleness determination, and this ray governs the determination's operands); distinct_from:constitution-ledger#internal-memory-entries-and-governance-snapshots-must-carry-explicit-timestamps-(that law governs ABSENT timestamps; this governs two PRESENT timestamps minted in mismatched frames); sibling:cgg-ledger#an-antecedent-named-by-a-mutable-address-is-not-named(one quiver: an identifier missing its qualifying context is not the identifier it appears to be — there an address without durability, here a timestamp without its frame); distinct_from:the-CLOCK-INPUT-face(that governs WHICH clock the current tic is read from; this governs whether two time operands share a FRAME before being subtracted) -->

The re-read-not-re-execute discipline turns on a staleness determination, and that determination is a comparison between two timestamps — typically an artifact's filesystem mtime and a governing record's own time field. Those two operands are routinely minted in DIFFERENT clock frames: stat/ls render mtime in local time; mandate `started_at` / `created_at` stamp UTC with an explicit offset. Subtracting them without normalizing frames produces an error of exactly the UTC offset, and the error is DIRECTIONAL: west of UTC, a freshly written artifact reads as PREDATING the run that wrote it — a FRESH artifact masquerading as a STALE pre-existing one, which is the precise misreading that licenses the redundant same-tic re-invocation the RENDERING-RERUN clause forbids (and a second same-tic run of a tic-keyed writer supersedes its first observation and mints a content-empty supersession receipt). So: before any staleness call, NAME THE FRAME of both operands and normalize to one — or resolve the question by an operand that carries no frame ambiguity at all (the producer's own typed liveness marker, the invocations ledger row, the process table).

MEASURED LIVE at tic 781 (a near-miss, caught before it authorized anything): `audit-logs/harmony/disposition-tic-781.json` showed mtime 03:22:10 (local, UTC-4) against a mandate `started_at` of 07:19:49+00:00 — reading as ~4 hours stale and pre-existing, when it had been written 2m21s INTO the run: the artifact was the reader's own invocation's output. Frame normalization (local 03:22:10 = 07:22:10Z) inverted the verdict from "pre-existing, do not re-run" to "mine, still being written," and the typed marker probe then settled it independently.

CURE-CLASS, declared honest (/review 784 Q1 — PROMOTE as ray, inscription-only): the near-miss lived in mandate-runtime REASONING, not in a script — there is no code site whose staleness comparison silently crosses frames; the frame-unambiguous operand this ray prescribes (the typed liveness-marker probe) is ALREADY BUILT and already mandated in this lane. The ray inscribes the discipline at the altitude where the failure occurred; fabricating a code cure would reach past the evidence.

APO: NOT a clock-skew / NTP finding (both operands were correct in their own frames); NOT the CLOCK-INPUT face (which clock supplies the current tic is a different question from whether two operands share a frame); NOT a claim that mtime is an invalid staleness input — it is valid once its frame is named; NOT an incident report (the misread was caught before it authorized a re-invocation — an operational guard, minted from a near-miss).

Slice: observer-indexed to a mandate-runtime consumer comparing an artifact mtime against a UTC-stamped governing record; the failure requires (a) a staleness comparison, (b) two operands, (c) at least one operand rendered in a non-UTC frame. Not a global claim about time handling.

Lock line: *a staleness verdict requires both operands' clock frames named and normalized — or a frame-free operand; a cross-frame subtraction is not a determination.*

Dual clamp: cost of action one frame-normalization step (or one liveness-marker probe, already mandated) per staleness call — negligible; cost of inaction a fresh artifact typed as stale authorizes a redundant same-tic re-run of a tic-keyed writer, superseding a good first observation and minting a content-empty supersession receipt carrying zero governance information.

<!-- promoted from cpr_mogul_harmony_invoke_2740db8d2669 (birth_tic 781, mogul:harmony_invoke, mogul_mandate_id tic-781-20260906T005148, source report audit-logs/mogul/cycle-reports/reports/2026-09-07T071949-tic-781.report.json, review_tic 784 → /review 784 PROMOTE as refinement ray, Architect-ratified in-tic via AskUserQuestion round 1 Q1, recommended option verbatim — the FIFTIETH consecutive verbatim set; the lived near-miss re-verified from the row's own text at adjudication. Cure-class inscription-only, DECLARED: the frame-unambiguous operand (the typed liveness-marker probe) is already built and mandated in this lane — no code cure owed, none fabricated. Queue: status extracted → promoted through queue-lifecycle-writeback.py w/ the valve's guard-set carry. Non-derivability: the timestamps-TTL invariant governs ABSENT time metadata and the RENDERING-RERUN clause obligates the staleness TEST; neither entails the operand-frame rule that two present timestamps must share a named frame before subtraction. Band: STRUCTURAL. lesson_type: operational_guard. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 1c6a0b320e13483d. -->

---

## A Producer-Identity Equality Banked Across a /review Boundary Is Unsound When the Docket Names the Instrument — and the Instrument Owes Its Own Identity Delta
<a id="a-producer-identity-equality-banked-across-a-review-boundary"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=781 | promoted_tic=784 | relations=sibling:constitution-ledger#a-supersession-receipt-types-the-instrument-beside-the-reobservation(the INSTRUMENT-IDENTITY face — same field, SUPERSESSION lane; this is the PASS-SERIES lane, the one arm that lane never reached); composes:cgg-ledger per-unit-delta discipline(/review 724 RIDER 2 — the obligation-scope ray attached a delta to every counter on the artifact surface; producer_identity was the one cross-pass quantity published without one); refines:the-banked-antecedent-selector-clause(that clause governs WHICH prior reading a bank consumes; this governs whether the INSTRUMENT survived the banking interval); distinct_from:the-SAME-TIC-RE-OBSERVATION-face(that decomposes a WITHIN-tic double fire; this is a CROSS-tic instrument mutation on a single fire) -->

A producer-identity EQUALITY banked across a /review boundary is unsound at COMPOSITION whenever that /review's docket names the predicted instrument as a same-pass cure surface — and the instrument cannot warn the reader, because producer_identity is the one cross-pass quantity the artifact publishes WITHOUT a baseline. Two halves, neither alone sufficient. (1) BANK SIDE: an equality prediction asserts instrument INVARIANCE over the banking interval, so it must be checked against the docket's cure surfaces at bank time, not against the sha at fire time; when the docket names the instrument, bank INEQUALITY-BY-CURE with the cure named, or decline the arm. (2) INSTRUMENT SIDE: every other measured arm on the close-check artifact (tokens, matched_comments, promoted/deferred/skipped) carries {baseline artifact, named selector, delta}; producer_identity alone was a bare point value, so a writer change was disclosed to nobody at the fire that FIRST ran the changed writer, and every cross-pass comparison reverted to hand-carry.

MEASURED LIVE at tic 781, cause verified not inferred: a /review-781 same-pass cure edited review-close-check.py between the bank's composition and its fire; the live writer sha recomputed at the fire equaled the artifact's emitted producer_identity exactly (the artifact was honest — the BANK was overtaken), while every OTHER banked arm was EXACT (three baselines, one selector, member-exact deltas). n=2 on the mechanism: the same same-pass-cure shape one tic earlier fired AS BANKED because it was predicted WITHIN the tic; the cross-tic bank one tic later did not carry it. Since /review 783 the equality bank is retired practice: the conditional bank with both branches lawful (equality iff no ratified cure lands; typed difference under the cure branch) is the standard — this face inscribes WHY, and lands the instrument half.

CURE, landed same-pass (/review 784 Q2, the tic-784 CGG batch; NC bank audit-logs/governance/nc-predictions-tic784-producer-identity-delta-cure.json, composed before any cure byte): `compute_producer_identity_delta` beside the other delta blocks in review-close-check.py — reusing the SAME tic-keyed baseline selector resolution the other arms consume (`_find_prior_check_artifact`), publishing {baseline artifact, selector, prior_writer_sha256_16, current_writer_sha256_16, producer_identity_changed} so the instrument self-discloses its own mutation in the pass series, exactly as it already does in the supersession lane. ADDITIVE only: the block joins the comparison-volatile set (occurrence-class — it records WHICH instrument, never what was measured; skip-vs-replace untouched), the equality-flag registry stays FOUR (the /review-760 ruling is not widened at build altitude — the new flag is disclosed in the audit window's known-unregistered list, its class question routed to /review if ever load-bearing), and a pre-cure prior with no stamped identity yields an honest null (unmeasured, never inferred).

APO: NOT a claim the artifact misreported (producer_identity matched the live file sha exactly; the bank was overtaken); NOT a genuine-hazard finding (genuine stayed 0 and the streak advanced; nothing in the consistency verdict moved); NOT the supersession-receipt discrimination (that lane already carries producer identity; the uninstrumented lane was the pass series); NOT an argument against same-pass cures (the cure landing in the verifying instrument is correct and desirable; the defect was that neither the bank nor the artifact accounted for it).

Slice: review-close-check.py's own artifact + the mogul close-fire banking protocol; observer-indexed to the close fire reading a bank composed at the same tic's entry. Not asserted over other tic-keyed artifact writers without their own measurement.

Lock line: *an equality banked across a governance boundary asserts instrument invariance — check the docket's cure surfaces at bank time, and make the instrument publish its own identity delta so a writer change is disclosed at the first fire that runs it.*

Dual clamp: cost of action one delta block on an already-resolved baseline (the selector is computed once per run and consumed once more) plus a bank-time docket check on paths that already exist; cost of inaction every future producer-identity bank across a /review boundary is a coin flip whose falsification is only discoverable by out-of-band hand comparison — and a hand comparison that lands EQUAL is indistinguishable from one that was never made.

<!-- promoted from cpr_mogul_review_close_check_0feb4176272e (birth_tic 781, mogul:review_close_check, mogul_mandate_id tic-781-20260908T053730, source report audit-logs/mogul/cycle-reports/reports/2026-09-08T053734-tic-781.report.json, review_tic 784 → /review 784 PROMOTE + same-pass cure — THE PRODUCER-IDENTITY BASELINE face, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FIFTIETH consecutive verbatim set; the lived instance re-verified from the row's own text at adjudication under checker-mint humility, cause verified not inferred. Cure landed same-pass (CGG batch t784): compute_producer_identity_delta reusing the shared tic-keyed baseline selector; block joins the comparison-volatile set as occurrence-class; equality-flag registry held at four with the new flag disclosed in the known-unregistered window; NC bank composed pre-cure, zero pin falsifications predicted, additive-only composition. Typed consequence banked and expected: the tic-784 close fire stamps a producer_identity differing from c55dca32db418468 AND the new delta block's first live emission reads producer_identity_changed=true with prior c55dca32db418468 — the cure disclosing its own landing on the fire that closes its ratification, the third consecutive tic with that shape; per the baseline-instrument-stability clause every compared arm's counting semantics are byte-identical across the cure boundary. Queue: status extracted → promoted through queue-lifecycle-writeback.py w/ the valve's guard-set carry. Non-derivability: the INSTRUMENT-IDENTITY face types the instrument in the SUPERSESSION lane and the per-unit delta discipline obligates deltas on measured counters; neither entails the bank-time docket check nor that the occurrence-class identity itself owes a pass-series baseline. Band: STRUCTURAL. lesson_type: mechanism_correction. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 1c6a0b320e13483d. -->

---

## A Documented Inclusive/Exclusive Boundary Only a Rare Event Class Can Exercise Is Tested by a SELF-MEMBERSHIP Probe on a Rare-Class Instance — Common-Path Arm Agreement Is a Tautology, and This Face's Own Born Was Felled by the Probe It Prescribes

<a id="a-boundary-only-a-rare-event-class-exercises-is-tested-by-self-membership-probe"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=782 | promoted_tic=785 | relations=sibling:constitution-ledger#presence-observation-fallacy-guard(the COINCIDENT-ARMS face — arms that agree under normal conditions and discriminate only under the condition of interest; this face adds the PROBE that settles such arms, and the family enrollment ray lands at that anchor); refines:cgg-ledger#a-timestamp-compared-across-an-unnamed-clock-frame(the t763 NAME-BOUNDARY ray lineage — an identifier stripped of its qualifying context; here the stripped context is the EVENT-FAMILY of the boundary's exercising instance); distinct_from:the-WINDOW-vs-POINT-ray(that governs reading a window AS a point; this governs testing the window's own documented inclusion boundary) -->

A windowed counter whose documented inclusive-of-current/prior-only boundary can only be EXERCISED by a rare event class cannot be verified by comparing its own arms on the common path — count == len(prior_list) on every common-path tic is a TAUTOLOGY that proves nothing in either direction. The test is a SELF-MEMBERSHIP probe on an instance of the rare class: on a tic where the current run IS a qualifying member, does the inclusive arm exceed the prior-only arm by exactly one? And the probe's instance must be FAMILY-CHECKED first: an event that superficially resembles the qualifying class but belongs to a different family (an infrastructure timeout beside a validator refusal) exercises NOTHING, and reading its non-effect as a defect is the same one-pole collapse the probe exists to prevent.

MEASURED LIVE at tic 785, and the measurement FELLED THE BORN THAT PROPOSED IT: the t782 born (cpr_mogul_harmony_invoke_db4368a5f1da) claimed harmony's admission_gate_watch numerator was "prior-only in fact" from 19/19 arm-agreement including tics 765/769 — but the full packet walk (764–785) shows 765, 769, and 783 are llm_timeout_120s = INFRASTRUCTURE family, which the admission numerator CORRECTLY excludes (count==len(prior) is right there, not understated), while the one true admission-refusal instance in the corpus — t760, preserved in harmony-voice.py's own NAME-BOUNDARY comment — measured count=5 beside len(list)=4: the inclusive numerator WORKING when exercised. The born ran arm-comparison plus a family conflation where its own generalization demanded a family-checked self-membership probe; the probe, run at adjudication, falsified the instance claim and validated the law. CURE-CLASS: NO-CODE-CURE, PREMISE-FALSIFIED-BY-MEASUREMENT — harmony-voice.py sha 54c1450812453b6d untouched; fabricating a numerator "fix" would have INTRODUCED the defect the born mis-diagnosed. Reported loud per the premise-falsification discipline, never laundered (falsification receipt: audit-logs/governance/nc-predictions-tic785-arm-producer-identity-cure.json § premise_falsification).

APO: NOT a numerator defect (the code is correct; the +1 arm fires on admission-family instances and did at t760); NOT a naming quibble against the t763 ray (that ray stands — the list IS prior-only and the count IS inclusive, exactly as inscribed); NOT a claim that arm-agreement is evidence of correctness either (the tautology proves nothing in EITHER direction — the probe is the only witness); NOT license to skip minting when a probe cannot be run (an unexercisable boundary is disclosed as untested, never assumed correct).

Slice: harmony-voice.py's admission_gate_watch, packets 764–785 walked in full at tic 785; observer-indexed to this adjudication. The probe discipline generalizes; the correctness verdict on this counter does not extend past the walked corpus.

Lock line: *a boundary only a rare event class can exercise is tested by a family-checked self-membership probe on a rare-class instance — arm agreement on the common path is a tautology, and a born that skips its own probe can be felled by it.*

Dual clamp: cost of action one packet walk and one family check before a cure is fabricated; cost of inaction a "fix" that breaks a correct counter to satisfy a mis-diagnosis, plus every future arm-agreement read as verification on boundaries the common path cannot exercise.

<!-- promoted from cpr_mogul_harmony_invoke_db4368a5f1da (birth_tic 782, mogul:harmony_invoke, review_tic 785 → /review 785 PROMOTE ray + cure ratified, Architect-ratified in-tic via AskUserQuestion round 1 Q1, recommended option verbatim — the FIFTY-FIRST consecutive verbatim set; cure-class RESOLVED post-ratification to NO-CODE-CURE, PREMISE-FALSIFIED-BY-MEASUREMENT: the full packet walk (764–785) showed the born's refusal-tic instances were infrastructure-family timeouts the admission numerator correctly excludes, and the t760 lived instance (count=5 vs len=4) proves the inclusive arm works when exercised — the ray landed RE-SCOPED to the probe discipline with the falsification as its lived instance, the born itself being the instance; family enrollment ray appended at constitution-ledger#presence-observation-fallacy-guard. Queue: extracted → promoted through queue-lifecycle-writeback.py w/ the guard-set carry. Non-derivability: the COINCIDENT-ARMS face names arms that discriminate only under the condition of interest; it does not entail the self-membership probe as the settling test nor the family check on the probe's instance. Band: STRUCTURAL. lesson_type: conductor_score_runtime_parity → verification_probe_discipline. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 4c30247794c2faa6. -->

---

## A Cross-Observation Delta Declares Baseline-Instrument Stability PER ARM, at the Delta's Own Altitude — a Sibling Block Covering One Arm Is Not the Declaration

<a id="a-cross-observation-delta-declares-baseline-instrument-stability-per-arm"></a>
<!-- ledger-tags: authority_class=verification_and_proof_discipline | rung=domain | domain=context-grapple-gun | born_tic=782 | promoted_tic=785 | relations=refines:cgg-ledger#a-producer-identity-equality-banked-across-a-review-boundary(the t784 anchor — the instrument owes its own identity delta; this face lands that disclosure ON EVERY ARM the deltas compare); sibling:the-READ-INSTANT-face(same wrong-altitude-disclosure shape, different datum — t781); composes:the-RELOCATION-face(entity-stable attribution identity, t782); distinct_from:autonomous_kernel/frame-protocol-spec.md(the cure is a DECLARATION, never a hash-verified/polled cross-run state check — building it as a verified state registry is the category error that spec names) -->

A cross-observation DELTA's attribution to state is established only when the INSTRUMENT that produced each baseline is declared stable across the version boundary the comparison crosses — and that declaration must ride at the DELTA'S OWN ALTITUDE, on EVERY arm the delta compares, not in a sibling block covering one arm. A top-level identity delta (the t784 cure) discloses the CHECKER's boundary; it does not tell a reader of verdict_counts_delta whether THAT block's cross-tic baseline, or its same-tic prior, was written by the instrument now doing the comparing.

MEASURED LIVE at tic 782: review-close-check.py fired twice in one tic with the /review-782 same-pass cure committed between the fires; BOTH of the close fire's baselines — the cross-tic artifact (tic-781-check.json) and the same-tic prior (superseded-1) — carried the OLD writer's sha (7850a5f9…-era), and verdict_counts_delta published "promoted +1" with prior_same_tic_observation publishing its own attributed movement, with ZERO producer-identity declaration on either arm.

CURE, landed same-pass (/review 785 Q2; NC bank audit-logs/governance/nc-predictions-tic785-arm-producer-identity-cure.json, composed before any cure byte, suite RUN 1 EXACT 1,553==1,546+7): `_arm_producer_identity(arm_report, current_identity)` in review-close-check.py attaches {arm_writer_sha256_16, current_writer_sha256_16, arm_producer_identity_changed, reason_absent} to the cross-tic baseline arm (block.baseline.producer_identity) AND the same-tic prior arm (prior_same_tic_observation.producer_identity) on BOTH delta blocks, shape-stable on every path. ADDITIVE only: the nested declarations are occurrence-class BY TYPE and stripped recursively from the skip-vs-replace comparison view (the read-instant FORWARD-DECAY strip extended, so a future arm gaining the declaration is auto-excluded); arm_producer_identity_changed stays OUTSIDE EQUALITY_FLAG_NAMES (registry FOUR per the /review-760 ruling, disclosed in the audit window's known-unregistered list); a pre-identity arm artifact yields declared-unmeasured, never inferred.

APO: NOT a verified cross-run state check (declaration only — the frame-protocol category-error fence holds); NOT a claim the t784 block was wrong (it discloses the checker's own boundary correctly; the gap was arm coverage and altitude); NOT a skip/replace input (occurrence-class, structurally excluded); NOT retroactive (historical artifacts are never retconned; arms predating producer_identity declare absence).

Slice: review-close-check.py's delta blocks and their two arm classes; observer-indexed to the t782 two-fire boundary and this pass's composition. Not asserted over other multi-arm instruments without their own measurement.

Lock line: *a delta that compares arms declares each arm's instrument at the delta's own altitude — a disclosure a single-block reader never receives is not a disclosure.*

Dual clamp: cost of action one helper and four shape-stable sub-blocks riding baselines that already resolve; cost of inaction every cross-cure-boundary delta's attribution silently mixes instruments, discoverable only by out-of-band hand comparison of writer shas the artifact already knows.

<!-- promoted from cpr_mogul_review_close_check_9c38ebdd0b9c (birth_tic 782, mogul:review_close_check, review_tic 785 → /review 785 PROMOTE + same-pass cure — THE PER-ARM BASELINE-STABILITY face, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FIFTY-FIRST consecutive verbatim set. Cure landed same-pass: _arm_producer_identity + four arm attachments + recursive occurrence strip + known-unregistered disclosure; NC bank composed pre-cure w/ ONE composition-time enumeration correction RECORDED pre-run (banked 5 tests, composed 7 — the estimate's miss, never smoothed); suite RUN 1 EXACT 1,553==1,546+7/0F/1E-standing/593, zero pin falsifications. Typed consequence banked: the tic-785 single fire's delta blocks carry cross-tic arms reading arm sha 91d99d4337b48792 vs the post-cure writer, arm_producer_identity_changed=true — the cure disclosing its own landing per-arm on the fire that closes its ratification, the FOURTH consecutive tic with that shape. Queue: extracted → promoted through queue-lifecycle-writeback.py w/ the guard-set carry. Non-derivability: the t784 anchor obligates the instrument's own identity delta; it does not entail per-arm coverage nor the altitude law. Band: STRUCTURAL. lesson_type: refinement_ray. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 4c30247794c2faa6. -->

---

## A Freshness Predicate Whose Population Includes Caller-Side-Effect Surfaces Shares the Caller's Invocation — the Effective-Record Chain Runs in ONE Shell Call

<a id="a-freshness-predicate-spanning-caller-side-effect-surfaces-shares-the-callers-invocation"></a>
<!-- ledger-tags: authority_class=pipeline_and_integration | rung=domain | domain=context-grapple-gun | born_tic=782 | promoted_tic=785 | relations=refines:constitution-ledger — federation KI Recursive Self-Observation(the observer's own operation mutating the observed, instantiated at the invocation boundary: the caller's harness appends to the predicate's population BETWEEN its own calls); sibling:cgg-ledger#recursive-self-observation(the CGG application of the same KI); composes:cgg-ledger#close-consistency-receipt-only-as-strong-as-artifact-verification(a false-stale HOLD read as a real hold weakens the close receipt it gates) -->

The effective-record close chain (`rebuild --authority` → `check-index` → `review-gate`) MUST run inside ONE shell invocation: the index's `source_digest` spans every audit-logs JSONL (`lib/effective_record.py discover_jsonl → source_digest`), and the harness's own PostToolUse hooks append governance rows BETWEEN separate Bash calls — so a `check-index` issued in its own call reads `hold/source_digest_changed` at 0/0 unresolved, a structural false-stale minted by the CALLER'S INVOCATION SHAPE, not by the index. The false-stale reads as a governance HOLD and invites the wrong cure (re-running rebuild, suspecting the index). GENERAL FORM: when a freshness predicate's population includes surfaces the caller's own tooling appends to as a side effect of EACH call, the producer and the checker of that predicate must share one invocation — or the checker measures the caller's harness, not the store.

MEASURED at tic 782 (measured-tier born): two split-call chains held false-stale; the identical verbs in one compound invocation passed 0/0 stale=false. Exercised at every close since (t783, t784 — "chain in ONE invocation, NO interleaved Bash" in both close receipts).

FIRST-CONSUMER PATCH, landed same-pass (/review 785 Q3 — patch-the-first-consumer over inscribe-around): the ONE-INVOCATION clause now rides the /review skill's step 1.6 (cgg-runtime/skills/review/SKILL.md, runtime-synced), the exact locus where the chain fires — the law loads where it is needed, not only in handoffs that are overwritten every tic.

APO: NOT an index defect (the index is honest — its population genuinely changed between calls); NOT a reason to narrow `discover_jsonl`'s population (the digest's breadth is its value; the cure is invocation shape, not scope); NOT a general ban on split Bash calls (only predicates whose population the caller's own side effects touch are in scope); NOT satisfied by re-running rebuild in a separate call (that re-mints the same false-stale one call later).

Slice: the effective-record chain under the Claude Code harness's PostToolUse hook behavior; observer-indexed to tics 782–784. Other harnesses owe their own measurement before the clause is cited against them.

Lock line: *when the caller's own side effects feed the freshness predicate, producer and checker share one invocation — a split chain measures the harness, not the store.*

Dual clamp: cost of action one compound command where three were typed, plus one skill clause; cost of inaction a recurring false-HOLD at every close that invites re-rebuild churn and, unrecognized, could park a lawful close behind a phantom governance hold.

<!-- promoted from cpr_effective_record_chain_single_invocation_tic782 (birth_tic 782, close-instrument lane, review_tic 785 → /review 785 PROMOTE ray + skill patch, Architect-ratified in-tic via AskUserQuestion round 1 Q3, recommended option verbatim — the FIFTY-FIRST consecutive verbatim set. First-consumer patch landed same-pass at SKILL.md step 1.6 (canonical side, runtime-sync verified in the batch commit); no python surface, no test-count movement, per the NC bank's cure declaration. Queue: extracted → promoted through queue-lifecycle-writeback.py w/ the guard-set carry. Non-derivability: the federation Recursive-Self-Observation KI names the loop class; it does not entail the invocation-boundary instantiation nor the one-invocation cure shape. Band: COGNITIVE. confidence_tier: measured. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 4c30247794c2faa6. -->

---

## A Tic-Keyed Artifact That Gives Some Counters Delta Blocks and Others Bare Point-Values Licenses Recency-Attribution on the Bare Ones — Every Lone Point-Value Counter Owes a Delta Block or a Declared-Absent Entry
<a id="partial-delta-discipline-is-an-undeclared-asymmetry"></a>

**Ledger tags:** terrain_class=signal-and-manifold · lanes=review_close_check,instrument-contract · era=t783-786 · target_rung=cgg

**Body:** PARTIAL DELTA DISCIPLINE IS AN UNDECLARED ASYMMETRY, NOT A PARTIAL CURE. A tic-keyed artifact that publishes per-pass delta blocks (baseline + selector + membership attribution) for SOME of its counters and bare point-values for others silently licenses recency-attribution on the undisciplined ones: a consumer diffing two artifacts in the series cannot tell whether a missing delta means NOT-COMPARABLE or NOT-YET-BUILT, and both readings are simultaneously available from the artifact alone. So every counter published as a point-value in a tic-keyed series owes EITHER (a) a delta block carrying its own baseline and selector, OR (b) an explicit declared-absent entry naming that no membership set backs it — the exact declared-not-fabricated shape pair_coverage already uses for the deferred/skipped populations, extended from published PAIRS to lone point-value counters. LIVED AT TIC 783: the artifact attributed its +2 promoted / +2 index-token movement MEMBER-EXACT to two named ids while, in the SAME artifact, queue_state_tuple moved raw_rows 3106→3111 (+5) and unique_ids 1330→1333 (+3) against the tic-782 artifact with no delta, no selector, and no declaration; the residual WAS knowable from a different instrument (the tic-783 entry receipt's boundary read), which is the point — attribution existed in the federation while this artifact's own reader was left to recency or nothing, the exact reach the banked-antecedent clause forbids. RE-VERIFIED AT ADJUDICATION on the fresh tic-786 entry-fire artifact: queue_state_tuple still bare beside three fully-declared delta blocks (producer_identity_delta / verdict_counts_delta / inscribed_index_delta) — the asymmetry held across the t785 per-arm cure, which disciplined the arms of EXISTING delta blocks but minted no delta for the tuple. APO: NOT a hazard finding on tic 783's measured deltas (genuine=0; every published delta was member-exact); NOT a claim queue_state_tuple is mis-measured (it is measured correctly and pinned at one sha in one pass — the gap is declarative, not arithmetic); NOT a demand that the tuple's row-level membership be enumerated (the cure's membership half is DECLARED ABSENT by design — member-exact attribution lives on the membership_sets arms). Relations: refines the pair_coverage CURE-SCOPE face (/review 756 Q2 — the declared-absent duty, extended in population); sibling: the banked-antecedent clause (t771 — the consumer side of the same seam); sibling: the SAME-TIC RE-OBSERVATION face (#cgg t744→747 — disclosure living in the wrong block is disclosure a single-block reader does not receive); composes constitution-ledger#structural-transform-implies-closed-consumer-set-obligation (a delta discipline landing on some counters creates a closed set of counters owed the same discipline — ship it without enumerating them and the gap is discharged by-failure). Cure landed same-pass (ratified /review 786 Q2): compute_queue_state_tuple_delta in review-close-check.py — per-key deltas {raw_rows, unique_ids, promoted, bytes} under the shared tic-keyed selector, cross-tic baseline arm + same-tic prior arm each carrying the per-arm producer-identity declaration (the /review-785 law), and an explicit declared-absent membership entry; measurement-class under the /review-775 discriminator, read-instants and arm identities stripped by type, no skip/replace effect from re-observation alone. Slice scope: observed on review-close-check's tic-keyed artifact series (tics 782→786, by a consumer reading the artifacts); the shape plausibly generalizes to any tic-keyed artifact writer (economy, harmony, contagion siblings UNMEASURED) but is claimed here only for this series. Lock line: *a point-value counter in a tic-keyed series carries its own delta block or a declared-absent entry — never a bare number beside a disciplined one.* Dual clamp: cost of action one delta block per lone counter, no counter semantics move; cost of inaction each further pass adds another artifact in which a reader can silently recency-attribute a multi-row queue movement standing beside a fully attributed one.

<!-- promoted from cpr_mogul_review_close_check_6dfe67fd3573 (birth_tic 783, mogul:review_close_check, mogul_mandate_id tic-783-20260908T173835, source report audit-logs/mogul/cycle-reports/reports/2026-09-08T173839-tic-783.report.json, review_tic 786 → /review 786 PROMOTE + same-pass additive cure, Architect-ratified in-tic via AskUserQuestion round 1 Q2, recommended option verbatim — the FIFTY-SECOND consecutive verbatim set. Premise probe-verified at adjudication on BOTH the t783 artifacts and the fresh tic-786 entry-fire artifact per the SELF-MEMBERSHIP-PROBE discipline (/review 785 Q1). Cure: compute_queue_state_tuple_delta + assembly emission in review-close-check.py, same pass; first live emission expected at the tic-786 close fire, self-disclosing via producer_identity_delta + the per-arm declarations (the 0feb + 9c38 laws). Queue: extracted → promoted through queue-lifecycle-writeback.py w/ the seven-field guard-set carry. Non-derivability: pair_coverage names the declared-absent duty for PAIRS; the per-arm law disciplines arms of EXISTING delta blocks; neither entails the lone-point-value population nor the both-halves cure shape. Band: STRUCTURAL. lesson_type: instrument_contract_refinement. confidence_tier: tentative at mint, adjudicated 0.85. signer ent_homeskillet-f5 (claude-fable-5), boot receipt 5203d5ad5763abb6. -->
