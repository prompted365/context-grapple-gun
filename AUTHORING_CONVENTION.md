...
**`off_envelope` value selection (refined tic 166):** The value is determined by the failure mode of an undeclared argument, not by load-bearing status alone. Use `ask` for skills where undeclared-arg most likely signals caller identity confusion (e.g., /cadence vs other cadence variants, /review vs ultrareview vs review-execute). Use `proceed-with-note` for skills with long-established defaults where undeclared-arg most commonly means "apply the default" rather than caller confusion (e.g., /inbox defaulting to invoking entity = ent_homeskillet). Use `refuse` only when undeclared-arg constitutes a constitutional violation. The selection criterion is the failure mode of the undeclared-arg case, not load-bearing status per se.
