...
- **A Close/Consistency Receipt Is Only as Strong as Its Artifact Verification** — A review-close/consistency/activation receipt earns GREEN only from real artifact verification: never hide a checker via `| tail` without `set -o pipefail` or prose-relabel its `consistent:false`, never let tautologies (`len(dict)==2`, `promoted_to` non-emptiness as resolution) or a dict-constructing mounted-bear helper stand in for a round-trip — run the authoritative checker fail-loud reading its closure contract from source, prove exact cohort set-equality, resolve every credential to a real surface, and harden the close before commits make the weak proof canonical. *(tic 635→638)*
  - *Ledger:* [`cgg-ledger/ledger.md#close-consistency-receipt-only-as-strong-as-artifact-verification`](cgg-ledger/ledger.md#close-consistency-receipt-only-as-strong-as-artifact-verification)
- **Inscribe BEFORE the Queue Write That Cites Its Anchor** — A promotion's two coupled writes (the doctrine-body inscription that MINTS the anchor slug, and the queue `promoted_to: surface#anchor` credential) must sequence inscription-first, or the credential cites a guessed placeholder anchor — a dead execution credential; if the queue write must precede, treat the anchor as provisional and run a mandatory reconciliation pass re-emitting every dual-surface stamp to the REAL inscribed anchor, then verify resolution with alphanumeric-normalized matching. *(tic 635→638)*
  - *Ledger:* [`cgg-ledger/ledger.md#inscribe-before-the-queue-write-that-cites-its-anchor`](cgg-ledger/ledger.md#inscribe-before-the-queue-write-that-cites-its-anchor)
