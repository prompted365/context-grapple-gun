...
When a /cadence-emitted tic includes post-cadence operational cleanup that mutates governance state (signal triage with manifest sweeps, MEMORY.md candidate updates, queue-shaping commits), the closing /cadence should be preceded by an explicit bench-packet-prep run. The mandate's `queue_refresh` cycle updates queue indexes but does not necessarily refresh the bench-packet artifact (`bench-packets/latest.json`) — refresh depends on the mandate runner's specific cycle implementation. Without explicit bench-packet-prep, the next session's /review opens against a pre-cleanup bench packet, hiding the very state changes that motivated the cleanup. The clean-close ordering is: (1) prior /cadence emits boundary tic + mandate, (2) post-cadence cleanup mutates manifold/queue, (3) explicit bench-packet-prep captures post-cleanup state, (4) closing /cadence emits next boundary tic with fresh bench packet referenced in the handoff. Naming the protocol here as recommended close-of-session practice; promotion to cadence/SKILL.md amendment requires more validation than n=1 + 1 valve-pattern co-observation.

<!-- promoted from cpr_post_cadence_clean_close_ordering_tic187 (tic 187→188). Source: operator-asked, agent-confirmed close-of-session sequencing at tic 187. Validation evidence: bench-packet-prep at tic 187 reported 13 pending vs queue_refresh's 39 raw pending — a 26-row asymmetry that is the valve-pattern doing its job at the read boundary (FIRST PRACTICAL EXERCISE of the load_queue mirror landed at tic 186). Conservative scope: encoded as recommended close-of-session protocol in CGG/CLAUDE.md only; cadence/SKILL.md spec amendment deferred per CPR review_hints. Band: COGNITIVE. Confidence: 0.85. -->
...
cadence-ops.compute_due_cycles is the central tic-modulo cycle scheduler for Mogul mandates, but multiple doctrine surfaces name cycles that doctrine claims fire on cadence yet the runtime never schedules: (1) civil_status_check — civil-engineer.md:166 declares 'Mogul includes civil_status_check in next mandate if last report > 10 tics' but compute_due_cycles has no civil entry at any modulo (audit at audit-logs/governance/cgg-civil-cycle-detection-audit-tic224.md confirmed gap; tic 220 manual restart did not inscribe a runtime mechanism); (2) bench_packet_prep — CGG CLAUDE.md Post-Cadence Clean-Close Ordering says '/cadence should be preceded by an explicit bench-packet-prep run' yet compute_due_cycles never schedules bench_packet_prep; the Architect manually compensates each tic by invoking the trigger pre-/cadence (this tic 224→225 cadence: 'prep the bench packet via trigger? then /cadence' was a manual compensation). Both gaps survive because manual compensation works and there is no failure surface that would force the runtime fix. Same shape as Conductor-Score-Runtime Parity (federation KI) applied specifically to the cadence-ops scheduler: doctrine names a cadence, runtime mechanism (compute_due_cycles) doesn't enforce it. Mitigation: extend compute_due_cycles to schedule the named cycles (civil at tic % 10, bench_packet_prep on every cadence emit OR every n tics) OR amend doctrine to drop the cadence claim (bench-packet-prep stays manual; civil stays event-triggered). The recurrence is structural — any future doctrine surface naming a cadence will repeat the gap unless the inscription discipline includes 'verify compute_due_cycles enforces this' as a doctrine-promotion gate. Same-tic n=2 (descriptive evidence within tic 224 audit boundary). Cross-tic n=0 (this is the first cadence where both gaps were named together).
