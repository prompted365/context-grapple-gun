...
**The registered splat-conditioned interpreter lane is not yet live.** The remaining gate is the joined crossing exercised here: hydrated `CovenantSlice` + admitted covenant → `SplatInterpretationRequestV1` → proposal-band canonical-mount invocation → CGG normalization into `SplatProposalEnvelopeV1` → homeskillet temporal intake. Until cross-repository shadow parity, rollback, projection-isolation, and longitudinal-contamination tests pass, no `KERNEL_REGISTRATION.md` is warranted.
