...
## Cycle Result Contracts — the TWO-ALTITUDES disclosure (RULED /review 758 Q1)

**`ladder_audit` result contract.** One token names two instruments at different altitudes — the BASE chain scan (`ladder-audit.py` with no subcommand: CLAUDE.md rule coherence) and the ladder DOWN-LANE audit (the same script's `list-findings` / `stage-brief` / `staleness-scan` family: per-rung KI rehydration-in-spirit). A `ladder_audit` cycle result MUST name WHICH instrument ran and disclose the sibling instrument's standing count in the same result: carry the instrument's own `instrument`, `altitude`, `altitude_disclosure`, and `sibling_instruments.ladder_down_audit.open_findings_on_manifold` (+ `of_active_rays`, `predicate`, `ids`) verbatim from the base-scan JSON — never summarize a base scan as "ladder_audit: N findings" without them. A clean base result read alone converts a scope-limited clean into a false all-clear (tic 755: `{coherent: 375}` / 0 findings beside 53 open `sig_ladder_down_audit_finding_*` rays = 91.4% of the active manifold; Mogul disclosed it by hand that tic — this contract makes it the instrument's). Doctrine: `ledger.md#a-shared-instrument-name-across-two-altitudes-discloses-its-altitude-and-its-siblings-standing-count` (the presence-observation family; breadcrumb at `#disagreement-as-evidence`); source `cpr_mogul_ladder_audit_18aea9c55cf5`. The same rule already binds harmony (WINDOW-vs-POINT, /review 752 C1) and economy (FIELD QUALIFICATION, /review 733/737): an instrument whose name is shared across altitudes discloses its altitude and its sibling's standing count in the same breath.
