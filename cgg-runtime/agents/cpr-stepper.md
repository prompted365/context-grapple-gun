...
**Reinforcement must be VISIBLE (Drift-1 fix, tic 377).** When a born truth lands `reinforce_existing`, the doctrine surface it reinforces must record it — otherwise the resilience signal (a KI independently rediscovered N times = matured) is erased at inscription. The mechanism: stamp a `<!-- reinforced_by: <cpr_id> (tic N, source) -->` breadcrumb on the target ledger entry. Mechanization owner: `review-promote-writeback.py` — **the stamper is BUILT and LIVE** (`review-promote-writeback.py:757/940`; 2 real stamps on disk [predicate: literal `reinforced_by:` comment carrying a real cpr_id — `ledger.md:4421` fired tic 476, plus the b736 double-bind face fired tic 739 by review-execute per this discipline] plus 1 format TEMPLATE (a literal `<cpr_id>` placeholder in `ladder-downlane-spec.md:128`) that is NOT a stamp — figures corrected tic 739 per A3-739 under guard 18, disclosed-predicate form; the tic-709 correction had counted the template as a second breadcrumb). The REAL gap is the TRIGGER: a manual CLI flag never keyed on `landing_kind` — 15 absorbed ids carry reinforce-shaped prose with no stamp [predicate: absorbed rows whose `absorbed_into`/`review_verdict` prose reads as reinforcement with no matching `reinforced_by:` comment on the named target; measured tic 739 — the prior "≥23" was an undisclosed-predicate figure] (`bk-reinforced-by-stamper-trigger-never-keyed`). Until the trigger is keyed, the reinforcing `absorbed_reason` carries the signal and the breadcrumb is applied by review-execute when it lands the verdict. Do NOT silently `absorb` a reinforce-existing landing without recording which doctrine it reinforced.
...
FRICTION-BORN COHORT FALLBACK (A3-737, ratified /review 738): for cpr-extract-hook rows, `source` names the EVIDENCE artifact (typically a cable receipt), not the lesson's text home — the lesson text lives in a `borns-tic<N>-*.md` file that no queue field names today. The lawful Gate-1 procedure for this cohort: run existence on the leading path of `source`, then locate the lesson text by naming-convention search over `audit-logs/governance/borns-*.md` (match on the lesson head; verified 4/4 at t737). A miss on the source artifact is NOT `source_diverged` for this cohort until the born file has also been searched. The durable cure — `lesson_home` stamped at mint — rides `bk-born-lesson-home-field-at-mint` (new field, single writer, single semantics; never overload an existing field per A1-738).
