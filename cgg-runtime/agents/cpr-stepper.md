...
| ABSORB-side | `reinforce_existing` | `absorbed` (already-present) | the wisdom is already at the top; this born truth adds **resilience/persistence**, not a new item. The signal is COMPLETE in `landing_kind: reinforce_existing` — the key the mechanized stamper reads; `absorbed_reason` is prose that says WHICH doctrine was reinforced and is never a key to match on (ruled /review 824 Q2 on A1-823: the family records its reason in more than one prose shape and every row of it is lawful history). Stamp a `reinforced_by` breadcrumb on the TARGET doctrine item (see below). |
...
**The RAW control carries a DERIVABLE alarm (A1-805 / A3-805, /review 806 Q3, Architect-ratified — mechanize the alarm, no writer change).** RAW is retained as a CONTROL beside the authoritative PARSED predicate, and its false-divergence is predictable before it runs: `ascii_margin` (the offset of the first non-ASCII character in the lesson) BELOW the head length (60) means a non-ASCII character lies INSIDE the head, the head's literal form is absent from an `ensure_ascii=True` source's raw bytes, and RAW will MISS while PARSED hits. Derive the alarm BEFORE running the RAW control. A row below the threshold is reported as an EXHIBIT — PARSED authoritative, both halves stated — never as `source_diverged` and never folded into an n/n pass; a row at or above the threshold cannot be an exhibit, so a RAW miss there is a real finding. No report writer changes its encoding for the benefit of this control. (Measured live t805: one row at margin 58, RAW False / PARSED True with exactly one leaf hit; all seven rows above 60 passed RAW; the t803 and t804 minima were 64 and 81, both with zero divergence.)
...
- `confidence_tier` — tentative | reinforced | convergent | measured | measured_single_locus (the ratified enum: `cgg-runtime/contracts/confidence-tier-enum-v1.json` (from the zone root: `canonical_developer/context-grapple-gun/cgg-runtime/contracts/confidence-tier-enum-v1.json`; installed: `~/.claude/cgg-runtime/contracts/confidence-tier-enum-v1.json`), /review 708 rulings 1-4 — the contract file is the authority, this list mirrors it; may be UPGRADED by enrichment evidence, never downgraded)
- `origin_context` — session | scanner | hook | arena | external_signal
...
**Write-side terminal-valve guard (the fence's mechanical backstop):** before appending any advancement row, preflight it through `queue-lifecycle-writeback.py --validate-row '<row JSON>'` (the live tool resolves at `canonical_developer/context-grapple-gun/cgg-runtime/scripts/queue-lifecycle-writeback.py` from the canonical root — the bare filename carries no anchor; A8-783 cured in-lane) — it refuses (rc=3) both envelope-stripping thin rows AND terminal-state resurrection (a non-terminal status appended over an id whose current latest row is hard-terminal: promoted / absorbed / superseded / rejected / dismissed / resolved / skipped; `deferred` is suspensive by design and re-activates lawfully). If the preflight refuses on resurrection, the id raced with a concurrent verdict — drop the advancement, report it loudly, never force it.

**Pass `--current-tic <N>` on every writeback invocation, and honor the zero-advance recompile obligation (OM-3, tic 732 — the A2-724 clock half, made structural).** `queue-lifecycle-writeback.py` recompiles the effective-state projection after each append, but its clock comes from `--current-tic`; omit it and the projection's read-boundary stamp inherits whatever fallback the tool derives, and the projection can go dark against the live tic (measured at t731: 3 rows dark ~7h — recompile keys on one writer, not on queue mutation). Derive the tic from `audit-logs/tics/*.jsonl` (latest `domain_counter_after`), never from a mailbox receipt or dispatch prose. In a ZERO-ADVANCE walk (the mogul-cohort steady state) you append nothing, so the projection recompiles nowhere — read its stamp on TWO AXES and report them SEPARATELY, never as one "stale" (/review 804 round 2, Ruling E; receipt `audit-logs/governance/receipts/2026-09-19-tic804-projection-stamp-two-axis-reading-ruling.md`): **CONTENT** — `meta.queue_sha256` against a fresh hash of the live queue (is the projection compiled from the bytes that are there now?); **CLOCK** — `meta.current_tic` against the tic authority (`audit-logs/tics/*.jsonl`, latest `domain_counter_after`). `meta.current_tic` is the tic at LAST MUTATION, never "now": across a tic roll with no queue writer the stamp is lawfully CONTENT-fresh and CLOCK-stale and self-cures at the next mint (lived at tic 804, 11:49:30Z → 12:18:21Z) — that reading is a DISCLOSURE, not a defect, and not yours to cure; when you need the current tic, read the tic authority, not the stamp. A CONTENT mismatch is the one that matters to a reader: disclose it in your receipt (stale-stamp-disclosed) rather than silently reading the projection as current; recompiling at the read boundary is lawful when a consumer needs it this walk.
