#!/usr/bin/env python3
"""test_cpr_extract_projection_recompile_tic802.py — the mint-side projection
...
-------------------------------------------
  1. a SUCCESSFUL extraction moves `meta.queue_sha256` to the live queue's sha256
  2. the recompile targets the ZONE THE APPEND WROTE TO (meta.queue_path is the
     ...
     identically — the installed tree carries no .ticzone in its ancestry, and
     this script's zone comes from --project-dir / resolve_zone_root(), never
     from __file__
...
cpr-extract run MINTS queue rows). Fixture-green is not live-green — the first
LIVE proof is the seat's next real mint after commit + sync.

Self-Locating Artifact Test Isolation: `extract_cprs()` takes an explicit
project_dir, so no arm can reach the real zone.

Run:  python3 -m unittest test_cpr_extract_projection_recompile_tic802
"""
...
ce = _load("cpr_extract_projection_under_test", _SCRIPT)
...
BLOCK = """# Fixture surface

<!-- --agnostic-candidate
id: cpr_fixture_projection_recompile_tic802
status: pending
lesson: {lesson}
source: fixture:tic802
-->
"""


def _build_zone(td, compiler="real", lesson="A fixture lesson for the recompile."):
    """A minimal but REAL zone: .ticzone + a tic log + an empty queue + a
    CLAUDE.md carrying one Tier-1 block (+ the compiler beside the queue)."""
    zone = Path(td)
    (zone / ".ticzone").write_text(json.dumps(
        {"name": "fixturezone", "audit_logs_path": "audit-logs"}))
    (zone / ".ticignore").write_text("node_modules\nvendor\n")
    ...
    (zone / "CLAUDE.md").write_text(BLOCK.format(lesson=lesson))
    if compiler == "real":
...
def _rows(zone):
    return [l for l in (zone / "audit-logs" / "cprs" / "queue.jsonl")
            .read_text().splitlines() if l.strip()]


class SuccessfulExtractionMovesTheProjection(unittest.TestCase):
    """DUTY 2 + the zone-resolution proof."""

    def test_extraction_moves_queue_sha256_to_the_live_queue_sha(self):
        with tempfile.TemporaryDirectory() as td:
            zone = _build_zone(td)
            err = io.StringIO()
            with contextlib.redirect_stderr(err):
                entries, counters = ce.extract_cprs(str(zone), dry_run=False)

            self.assertEqual(len(entries), 1, counters)
            self.assertEqual(counters.get("written_to_queue"), 1, counters)
            self.assertEqual(counters["effective_state_recompiled"], 1,
                             f"{counters} / stderr={err.getvalue()}")
            self.assertEqual(counters["effective_state_recompile_failed"], 0)
    ...
    def test_recompile_targets_the_queue_the_append_wrote(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with contextlib.redirect_stderr(io.StringIO()):
                _entries, counters = ce.extract_cprs(str(zone), dry_run=False)
            self.assertEqual(counters["effective_state_recompiled"], 1)
            ...
            self.assertTrue(
                Path(td).resolve() in _projection(zone).resolve().parents,
...
class NoAppendFiresNoRecompile(unittest.TestCase):
    ...
    def test_dedup_hit_leaves_the_projection_byte_and_mtime_identical(self):
        with tempfile.TemporaryDirectory() as td:
            zone = _build_zone(td)
            with contextlib.redirect_stderr(io.StringIO()):
                first_entries, first = ce.extract_cprs(str(zone), dry_run=False)
            self.assertEqual(len(first_entries), 1)
            self.assertEqual(first["effective_state_recompiled"], 1)
            ...
            with contextlib.redirect_stderr(io.StringIO()):
                second_entries, second = ce.extract_cprs(str(zone), dry_run=False)

            self.assertEqual(len(second_entries), 0, second)
            self.assertEqual(second["skipped_dedup_hash_match"], 1, second)
    ...
    def test_dry_run_fires_no_recompile(self):
        with tempfile.TemporaryDirectory() as td:
            zone = _build_zone(td)
            with contextlib.redirect_stderr(io.StringIO()):
                entries, counters = ce.extract_cprs(str(zone), dry_run=True)
            self.assertEqual(len(entries), 1)
            self.assertEqual(counters["effective_state_recompiled"], 0)
            self.assertFalse(_projection(zone).exists(),
                             "a dry-run must not produce a projection")
            self.assertEqual(_rows(zone), [],
                             "a dry-run must not write a queue row")
...
class RecompileIsFailSoft(unittest.TestCase):
    ...
    def test_nonzero_compiler_still_lands_the_append_and_counts_the_failure(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(len(entries), 1, counters)
            self.assertEqual(counters.get("written_to_queue"), 1, counters)
            self.assertEqual(len(_rows(zone)), 1,
                             "the row must survive a compile failure")

            # the failure is COUNTED and LOUD
            self.assertEqual(counters["effective_state_recompiled"], 0)
            self.assertEqual(counters["effective_state_recompile_failed"], 1)
            self.assertIn("recompile_failed_rc=3",
                          counters["effective_state_recompile_detail"])
    ...
    def test_missing_compiler_is_typed_and_soft(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(counters["effective_state_recompile_failed"], 1)
            self.assertTrue(
                counters["effective_state_recompile_detail"].startswith(
                    "compiler_not_found:"),
                counters["effective_state_recompile_detail"])
    ...
    def test_process_exit_status_is_unchanged_when_the_compiler_fails(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(proc.stdout.strip(), "1",
                             "the CLI still reports its extraction count")
