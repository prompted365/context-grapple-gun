...
# half RULED LIVE; the pointer is the surface the statusline and boot
# worldview actually eat, so the constancy observability rides HERE).
# Fail-soft: a missing receipt block reads "not computed", never silence.
