...
# "Key the writer on mutation"); RETROFITTED onto the ONE shared helper at
# /review 803 round 2 (Architect-ratified, recommended option verbatim: "One
# shared helper, all five writers, today"). Ruling receipts:
...
#
# WHAT MOVED AT 803 AND WHAT DID NOT. BEHAVIOUR IS PRESERVED — same contract,
# same typed failure details, same stderr wording, same summary counters, same
# call site and the same "only after a successful append" gate. Only the OWNER
# of the body moved: what lived here as a faithful copy of queue-lifecycle-
# writeback.recompile_effective_state now lives ONCE in
# lib/effective_state_recompile.py, and five writers call it. That closes
# F-802-B4 — "the recompile helper now exists in THREE copies held together by
# diligence" — handed up by this script's own tic-802 build seat.
#
# WHY IT EXISTS AT ALL. audit-logs/cprs/effective-state/ is a DERIVED cache —
# queue.jsonl stays the sole authority — but its only invokers were queue-
# lifecycle-writeback.py (fires at a /review writeback) and bench-packet-prep.py
# (fires at a bench prep). This script is a mint-side queue APPENDER that named
# the compiler only in comments and never invoked it, so EVERY MINT left the
...
#
# DOES-NOT-SATISFY RIDER of the 801 ruling (travels verbatim): this increment does NOT add a reader-side staleness detector; a reader that trusts the projection is protected only while every queue writer recompiles. A writer added later without the call re-opens the window.
#
# DOES-NOT-SATISFY RIDER of the 803 ruling (travels verbatim): this increment does NOT add a reader-side staleness detector, does NOT prove the compiled per-id states are correct, does NOT test two writers racing, and does NOT make the projection authoritative over the queue — `queue.jsonl` latest-per-id remains the only authority; the projection is a derived convenience that is now writer-fresh for five of six writers by construction and for the sixth by its own code.
#
# ZONE LAW (CLI shape 22) is the HELPER's law, not this script's: every path the
# recompile uses — the compiler, the --out directory, the tic log — is derived
# from THE QUEUE THE APPEND ACTUALLY WROTE and never from any script's
# __file__. That is what makes the source copy and the INSTALLED copy under
# ~/.claude/cgg-runtime/scripts/ behave identically; the installed tree carries
# no .ticzone in its ancestry, so a __file__-anchored zone resolution would miss
# the real zone entirely.
# ──────────────────────────────────────────────────────────────────────────────
...
def ingest(zone_root: Path, report_path: Path, dry_run: bool):
    ...
    #
    # DOES-NOT-SATISFY RIDER (verbatim, 801): this increment does NOT add a reader-side staleness detector; a reader that trusts the projection is protected only while every queue writer recompiles. A writer added later without the call re-opens the window.
    # DOES-NOT-SATISFY RIDER (verbatim, 803): this increment does NOT add a reader-side staleness detector, does NOT prove the compiled per-id states are correct, does NOT test two writers racing, and does NOT make the projection authoritative over the queue — `queue.jsonl` latest-per-id remains the only authority; the projection is a derived convenience that is now writer-fresh for five of six writers by construction and for the sixth by its own code.
    if summary["ingested"] > 0 and not dry_run:
        _ok, _detail = recompile_effective_state(queue_file)
        summary["effective_state_recompiled"] = 1 if _ok else 0
        summary["effective_state_recompile_failed"] = 0 if _ok else 1
        summary["effective_state_recompile_detail"] = _detail
