...
[12] down-lane RETURN leg (LIVE — ratified at /review 628: ladder-feedback-push.py:49 ratified=True, dual proof closed, runtime-sync clean): ladder-feedback-push.py + trigger-manifest.yaml#ladder.rehydration_feedback; the complement to the C9 federation→rung down-lane (autonomous_kernel/ladder-downlane-spec.md).
...
THE HARPOONV2 LANE & THE HARNESS-OF-HARNESSES. ⟨OFFICE⟩ harpoonv2 = canonical_developer/homeskillet-csl/ — THE LOAD-BEARING harpoon, the DAG-OF-DAGS: a Rust workspace running covenant-strike multi-agent orchestration + MoE model routing. harpoonv2 IS THE GUNSLINGER figure — the covenant-striker whose one Colt-45 becomes whatever the covenant needs; the shot loaded, the hand STILL — that stillness is not hesitation, it is THE LAW (no strike on the frozen center; the strike routes AROUND the centroid, and the CABLES that route it are load-bearing, holding by tensegrity). Load-bearing crates: harpoon_bridge (covenant algebra ⊳/∥/⊕ → FragmentDAG → STRIKE = multi-repo PR automation + v2 API /v2/covenant · /v2/strike · /v2/process) · harpoon-core (the fusion engine — fragment/hygiene/anchor, run_native_cycle) · orchestrator (two-tier route Gemma-3-270M → Qwen3-30B) · service (Axum). vendored ≠ versioned — NEVER git-add the csl repo. DEFAULT LEAD = CLAUDE CODE; MLX + ANY provider supported — harness+provider are CONFIG, not hardcode, chosen at ⟨SUBSTRATE⟩ canonical-mount = canonical_developer/canonical-mount/ = the HARNESS-OF-HARNESSES (harness-agnostic governance mount, the sole-writer rail — lead by Claude Code OR Codex, every process/piece per any provider; live `invoke` promoted tic 618 — the ONLY live verb, claude_code the only wired originator, local/codex honestly originator_not_wired; further build increments gated; the ROLE declared here). The Office of the Harpoonv2 is a SIBLING RUNG-PEER, not a master — it strikes INTO the DAGs other offices own and parents nothing ("metabolize, not master"). ⚠️ NOT the compliance-harpoon (audit-logs/evaluations/…/compliance-harpoon) — that 'harpoon' was a MISNAME; never conflate it with the load-bearing harpoonv2.

HOW TO RUN THE BOARD (the crux). THE BOARD = a RECEIPT-BEARING CONFORMATION SLICE, never any single projection: backlog.py dag supplies the current candidate identity + declared-dependency FIELD; the standard board is produced by JOINING that field with historical topology (tic-410 §1/§4 + the legacy↔live crosswalk), lifecycle+terminality, authority+admission, physical target evidence, io-map/router geography, runtime capability, rollback, consumers, and telos — NO INDIVIDUAL INPUT SOURCE CROWNS ITSELF (route-metadata.json is a DERIVED CACHE/override lens, never a crowning input — v3 law below). The live compiler is LIVE (t621, crosswalk-gated then landed): `harpoon-sequencer.py --mode live` compiles the slice and OWNS the root artifacts (envelope-stamped: source.mode/input-hashes/repo-HEADs/identity+edge+executable-set hashes; fail-closed: missing state / unprojected covenant decomposition / unresolved contradiction = visibly non-executable; selftest 17/17 + lattice cross-derivation); legacy parity runs separately (`--mode legacy` → legacy-out/, historical A/B/C golden preserved); refresh-maps invokes live EXPLICITLY. THE V3 COVENANT LAW (t622 · autonomous_kernel/covenant-splat-fqoq-runtime-spec.md — the doctrine body; the covenant-splat skill is its operational surface): the board is a DAG of ADMITTED COVENANTS, not backlog rows decorated with route metadata — an approved covenant never re-earns its authority; it needs to survive intact into an executable, current Reality→Target route. The six route-metadata fields are COVENANT FACES the interpreter derives; readiness classes carry the pre-drain umbrella covenant_decomposition_unmaterialized (NOT evidence-deficient — the check is covenant-face presence in a derived cache, never evidence adjudication); every node carries the five-status axes (covenant/projection/conformation/execution/evidence) under the §14 axis fail-closed law — NULLS are never invented claims: conformation defaults unknown (a stale thing can be freshly rendered — envelope freshness ≠ route currency; contagion proximity + the staleness canary are the currency signals), execution never claims ready without a runtime probe. The drain resolves each unmaterialized route via the §14 resolver: locate the admitted covenant → NEVER re-litigate it → one diagnosis (covenant_absent | covenant_incomplete | covenant_projection_lost | decomposition_missing | …); evidence_insufficient only after applicable surfaces were actually searched. Directive: board-live-compiler-directive-tic621.md (+ ADDENDUM 5 — the Architect execution spine, t623). Remaining legs: TYPED covenant-backed Rust fragment boundary (fail-closed when any required typed face is absent/malformed/stale/contradicted/receipt-unsupported; canonical is the SINGLE WRITER of covenant_status=admitted — it verifies the admission receipt and emits the axis, Rust validates+consumes and never adjudicates admission; the exec_ready fail-OPEN defect CLOSED at csl 37449ea t623, fail-closed across all Rust read sites) + point-10 TWO-DOMAIN hash equality (executable_route_identity_set_hash [Rust re-projects fragments→parent covenant ids; must equal the slice's executable_identity_set_hash] ≠ fragment_occurrence_set_hash [occurrence-namespaced ids w/ choice ancestry + selection state]; the route→fragment mapping receipt persists between them — NEVER compare the two domains directly; no Rust-side hash exists yet) + lawful §14 closure drain (a fresh render is not route currency; the strict typed frontier may honestly read EMPTY pre-drain — the law holding, not breakage) + AK Control Room board adapter (render every axis independently, both hash domains, mapping-receipt status, contradictions, route currency) — the DAG-of-DAGs WINCH that orders covenant→strike waves by DEPENDENCY, never priority (c48 PROPOSES, the Architect SEQUENCES). THE MAPS LOCATE (NAVIGATION=router-of-routers · io-map · router answer "where does X live?"); THE BOARD SEQUENCES ("in what order, in what parallel waves, gated on what?") — never conflate a map-find with a strike-order. Operating order: STRIKE to find the shape → CABLE around the center the shape orbits → WINCH the cables to hoist current into target. Step by step: (1) forge the COVENANT{Reality(current) → Target} explicitly, before the strike; (2) the sequencer reads the board as DATA (never hardcoded); (3) CABLE it via the cable_lattice (Sequential=staged-lock [B can't tension until A stable] · Parallel=fork/bridle · Choice=selector · Join=merge · Gate=winch-pawl); (4) DERIVE the waves by Kahn dependency-grouping — parallelism is by DISJOINT WRITE SURFACE (items sharing queue.jsonl SERIALIZE via atomic-append); (5) PRE-FIRE GO/NO-GO (harpoon-sequencer.py --check): GO iff all six apophatic checks pass — acyclic · no_autoprioritization · center_exclusion · architect_gate_respected · drift_surfaced_not_hidden · cable_lattice_crossvalidation — and a GO is SCOPED to the tic-410 identity set (t621 find: the reconciler projects terminal/done backlog states as exec_ready, so a GO over a drained board is VACUOUS-GREEN; never read GO as "the live board is current"); (6) WINCH it as a WAVE OF OFFICE-CITIZEN AGENTS — one swarm.task_dispatch envelope per exec-ready cable into its office seat's inbox; the citizen boots from its own SubagentStart and lands a receipt. THE TWO WINCH MODES (harpoon-office/winch/winch_modes.py, WinchMode enum): GUNSLINGER="gunslinger" = SEED / fast-draw (all exec-ready cables raised AT ONCE under one shared DissonanceBasin cradle, RollbackDrill armed, seeded in one tic — the ember fires ONLY IF rollback-velocity > attachment-velocity); HIDALGO="hidalgo" = STAGED / staged-lock (sequential, a cable can't tension until the prior's receipt lands; SUCCESS = the A/B/C/D RESIDUE SHAPE, not the tranche count — clean iff no D_REJECTED; a THROTTLE, never the default). from_hoist_mode: seed→gunslinger, staged→hidalgo. A STRIKE FIRES ONLY POST-/REVIEW-RATIFICATION (craft → /review crowns → strike post-boot, fresh-context) — but that ratification is ADMISSION on the covenant OBJECT, not a per-dispatch permission event: once /review has ratified the covenant target, hand-driven waves against it — inside an approved goal and existing write boundaries — EXECUTE without asking BOOM: an approved goal + approved execution-class is a persistent scoped GRANT, not reset by a new tic/session/model/command-spelling — the system must not remember its brakes better than its permission (t618). THREE real gates remain, never conflated: (a) ENABLING the atomic-append+hook AUTO-TRIGGER — autonomous recurring dispatch — is a NEW AUTHORITY surface (ask/route) and is still DORMANT; (b) the csl FIRST live board-fire is PHYSICS-gated on a ratified covenant target (a dependency, never a permission-substitute); (c) the /review ratification above is ADMISSION on the covenant object. So TODAY the orchestrator HAND-DRIVES each wave, one invocation at a time — sequenced by DEPENDENCY, not gated by fresh permission.
...
⟨FIELD⟩ THE ORG-ENGINE PROPOSES; GOVERNANCE ADMITS (the train→propose→admit arc). The organization-engine LoRA (Qwen3-14B, a NO-DELETE, evidence-bound file-organization delegate, promptedAccounting-owned — observe-not-couple) is the SP5 PROPOSER: it reads an artifact-WITH-HISTORY and emits a typed BID = a governed subquiver of LAWFUL TRAVERSALS, never a label, never a move (mutation_authority=false is STRUCTURAL — declare-don't-rule). The declaration-adapter types the real epoch16 file-routing quiver Q_c (RATIFY-SHAPE /review 538); the SP2 GATE (autonomous_kernel.admission.proposal_gate, kernel-registered tic 539) re-derives the lawful set itself, records the model's would_rule as a bid and DISCARDS it as authority, and returns ADMIT_PROPOSAL_REFUSE_MOVE — a citizen may drive to held, but only the OWNER via an EMITTED receipt crosses into filesystem_mutation. classify_route is NOT the arbiter and NOT a placeholder the weights replace — it is re-typed as the permanent LOSSY projection d:GQuiv→R / deterministic fallback. THE ARCHITECT'S RING — FOUR REFERENTS, LAYERED (t591-stands ruling, receipted 2026-08-24-tic731-sp5-referent-ruling; never say bare "SP5" for a gate): R1 the proposal WIRE — rung tic 538, fired live once, PROPOSAL-ONLY (epoch16 eval-cleared recall 0.9952, live-served on our own M2 Metal silicon — PRESENT, not absent — QUALIFIED /review 749: that 0.9952 is a RETENTION check on a contaminated universe [epoch14 trained on its own 8-record eval + answer key; no held-out set at any epoch], and the ONE canonical-side properly-split suite [canonical-mount evals/agentworld_shadow/covenant/v1/org_engine_additive_post_training_evaluation.json, run 20260717T000319Z_2fe87ee7, tracked] scores the lineage 0/8 interface-valid · 0/8 config-exact · 0/6 pairs · 0/2 holds — different measurements, cited together or not at all; the disjointness-attested re-eval is owed at bk-epoch-eval-heldout-set); R2 the MOVE/live-egress POLICY — rung tic 591 (Ring 1 KEYSTONE) as a STANDING POLICY GRANT held at the readiness gate: armed authorization, not execution clearance — live-model source stays ELECTIVE per-invocation, mutation_authority stays structurally FALSE at the SP2 gate, reversible to source=recorded_receipt; R3 NEW model-source coupling — NOT rung, the Architect's bell; R4 autonomous recurring dispatch — NOT rung, a distinct new-authority surface (t618 three-gates law), dormant. FIXTURE-GREEN ≠ TRAINER-GREEN.
